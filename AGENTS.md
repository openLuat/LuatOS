...
### little_flash / 资源管理与 assert 模式

- **mutex 句柄的 API 名是 `luat_mutex_release`（不是 `delete`）**：`luat/include/luat_rtos_legacy.h:34-37` 声明 `luat_mutex_create/lock/unlock/release`（legacy 形态，签名是 `void*`）。`luat_rtos_mutex_delete` 是新的 `luat_rtos_*` 系列 API。`port_init` 创建 mutex 时调 `luat_mutex_create`，`device_deinit` 释放时必须用 `luat_mutex_release`（`luat_rtos_legacy.h` 的语义是「destroy + free」，不是 unlock）。
- **PC 模拟器跑 C 层 utest 必须先 `LUAT_USE_UTEST=y` 再 `bsp/pc/build_windows_32bit_msvc.bat` 重编**：否则 `lf.utest` / `socket.utest` / `http.utest` 等 C 入口函数不会被编译，运行时直接 `attempt to call a nil value` 报错。worktree 里这个 env 变量是 *构建时* 检查的，单独设置后必须触发一次完整 rebuild（哪怕代码没改），切到不设 env 后再 build 一次可关闭。
- **worktree 下 `bsp/pc` 跑 utest 的路径深度是 2 而非 3**：非 worktree 布局 `D:\github\LuatOS\bsp\pc` → `..\..\..\testcase\common\scripts\`（3 ups）；worktree 布局 `D:\github\LuatOS\.worktrees\<name>\bsp\pc` → `..\..\testcase\common\scripts\`（2 ups），因为 worktree 自身多嵌了一层 `.worktrees/<name>/`。Agent 接到任务时若模板给的 `..\..\..\testcase\...` 跑出 `opendir failed`，先确认是不是在 worktree 里，是就改 2 ups。
- **`LF_DEBUG_MODE` 应该是 release-off 而不是「#define LF_DEBUG_MODE」硬开**：`port/little_flash_config.h` 的硬 `#define` 会让 `LF_ASSERT` 在 release 仍展开成 `while(1);`（`inc/little_flash_define.h:36-52`），线上固件触发 assert 即砖机（BSP 看门狗救不回 busy loop）。正确写法：`#ifndef LF_DEBUG_MODE #define LF_DEBUG_MODE 0 #endif`，需要 dev 行为时 `-DLF_DEBUG_MODE=1`。`LF_ASSERT` 的 release 分支应退化为 `LF_ERROR(...)` 日志而不是空语句——空 `LF_ASSERT(EXPR)` 失去触发可见性，但 `while(1)` 失去可恢复性。
- **little_flash 内部资源（mutex）必须由 `device_deinit` 显式 `release`**：`port/little_flash_port.c:46-67` 的 `port_init` 在 `lf->mutex` 里塞了一个 `luat_mutex_create()` 句柄，原 `device_deinit` 只 free 了 `prog_buf` 漏了 mutex，反复 init/deinit 会泄漏 RTOS 句柄。修复模式：`device_deinit` 用 `if (lf->lock != NULL && lf->mutex != NULL)` 守护（`lock` 是 sibling 字段，port_init 写完 mutex 才会 wire lock，所以反过来就能反推 mutex 是否被 port_init 创建过），然后 `luat_mutex_release(lf->mutex); lf->mutex = NULL;`。
- **字段重命名比加注释更能解决「同名不同义」歧义**：`struct little_flash` 原本同时有 `spi.user_data`（spidev）和顶层 `user_data`（mutex），后续 C 用户极易把后者当 spidev 读。注释只能劝退、无法强制；重命名为 `mutex`（含 Doxygen 注释解释 ownership）后 grep `lf->user_data` 即可验证迁移完整。Option A（重命名）只在字段是 internal 字段、外部 C 用户较少时适用；公开 API 的字段慎重。

### `components/luat_lfs2_nand` 组件速记（NAND Flash 上的 LFS2）

**目录布局**：
- `luat_fs_lfs2_nand.c` — VFS POSIX 层（`fopen/fread/fwrite/fseek/...`），把字符串 mode 翻译成 `LFS_O_*` 标志
- `luat_fs_lfs2_nand_profile.c` — 写缓存池、metadata 扫描、空间回收；`g_lfs2_nand_write_cache[]` 是核心缓冲数组
- `luat_lfs2_nand_little_flash_lfs2.c` — 适配 little_flash（底层 NAND 抽象）
- `luat_lfs2_nand.h` — **公共 config 宏的权威源**（`LUAT_LFS2N_CACHE_POOL_BUDGET/SLOTS/CHUNK`、`LUAT_LFS2N_FILE_CACHE_LIMIT` 等），全部 `#ifndef` 守护
- `luat_lfs2.h` — littlefs 2.x 移植层

**fopen mode 翻译表（已对齐 POSIX fopen(3)）**：

| mode  | 翻译成 LFS_O_*                                     | 备注 |
|-------|----------------------------------------------------|------|
| `r`   | `RDONLY`                                            | 文件必须存在 |
| `r+`  | `RDWR`                                              | **不要 CREAT**（POSIX 规定） |
| `w`   | `WRONLY \| CREAT \| TRUNC`                          | write-only |
| `w+`  | `RDWR \| CREAT \| TRUNC`                            | read+write |
| `a`   | `WRONLY \| APPEND \| CREAT`                         | write-only 追加 |
| `a+`  | `RDWR \| APPEND \| CREAT`                           | read+write 追加 |

**fwrite 写权限判断**（必须解耦）：用 `file->flags & (LFS_O_WRONLY \| LFS_O_APPEND \| LFS_O_RDWR)` 任一非零就允许写。**不要**写成 `WRONLY != WRONLY && APPEND != APPEND` 这种"恰好正确"的表达式——一旦 fopen 那一行被改成 POSIX 正确值，fwrite 就会因为 RDWR 没有 OR 进去而误拒。

**Cache pool 宏的"权威源 + 公开 alias"模式**：
- 头 `luat_lfs2_nand.h` 定义 `LUAT_LFS2N_CACHE_POOL_BUDGET` 等，带 `#ifndef` 守护
- `luat_fs_lfs2_nand_profile.c` 内部用 `#define LFS2_NAND_WRITE_CACHE_LIMIT LUAT_LFS2N_CACHE_POOL_BUDGET` 做 alias，**不要**重新定义一个不同的数值
- `bsp/pc/xmake.lua` 用 `add_define_from_env("LUAT_LFS2N_CACHE_POOL_BUDGET")` 把环境变量串到 C 宏；测试矩阵 `testcase/lfs2n_regression/run_lfs2n_matrix.py` 就是通过这条管路 override
- 默认值 128 KiB（与 `testcase/lfs2n_regression/run_lfs2n_matrix.py` 的 `DEFAULT_KNOBS["LUAT_LFS2N_CACHE_POOL_BUDGET"] = 131072` 一致）

**审计 lfs2_nand 时必查的"硬编码 vs 公开 config"反模式**：
- 看到 `components/luat_lfs2_nand/luat_fs_lfs2_nand_profile.c` 里有 `#define XXX (256 * 1024)` 这种"硬数值 + 公开 config 宏同名/近名" 的痕迹，立刻 grep `luat_lfs2_nand.h` 和 `bsp/pc/xmake.lua` 的 `add_define_from_env`，确认是不是被 stale 数值 shadow 了公共 knob
- 同样要检查 `xmake.lua` 的 `LUAT_LFS2N_*` define_from_env 行是否齐：缺一个就少一个 override 入口

**PC 模拟器下回归链路**（参考 `bsp/pc/AGENTS.md`）：`cmd /c bsp\pc\build_windows_32bit_msvc.bat` → 跑 `testcase/lfs2n_regression/lfs2n_regression_basic/scripts/` 下的 Lua 测试；真机验证需要 `.soc` 固件 + 刷脚本，详见 S-05/S-07 任务的 deliverable.md 经验记录。
