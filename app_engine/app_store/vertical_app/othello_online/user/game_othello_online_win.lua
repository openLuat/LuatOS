--[[
@module  game_othello_online_win
@summary 联网黑白棋游戏窗口模块
@version 1.0.0
@date    2026.04.13
@author  AI
...
local function computer_move()
    ...
    if move then
        ...
        elseif not next_player_has_move then
            ...
            sys.timerStart(function() computer_move_delayed() end, 500)
        else
            current_player = next_player
            update_score_ui()
            log.info("computer_move: switch to next player", next_player)
...
local function create_ui()
    ...
    local title_bar = airui.container({ parent = main_container, x = 0, y = 0, w = 480, h = 70, color = 0x2E7D32 })
    airui.label({ parent = title_bar, x = 20, y = 20, w = 200, h = 30, text = "黑白棋", size = 48, color = 0xFFFFFF, align = airui.TEXT_ALIGN_LEFT })

    local score_bar = airui.container({ parent = main_container, x = 0, y = 70, w = 480, h = 60, color = 0xFFFFFF })

    black_label = airui.label({ parent = score_bar, x = 20, y = 10, w = 150, h = 30, text = "黑: 2", size = 42, color = 0x1A1A1A, align = airui.TEXT_ALIGN_LEFT })

    turn_label = airui.label({ parent = score_bar, x = 165, y = 10, w = 150, h = 30, text = "黑棋回合", size =42, color = 0x2E7D32, align = airui.TEXT_ALIGN_CENTER })

    white_label = airui.label({ parent = score_bar, x = 310, y = 10, w = 150, h = 30, text = "白: 2", size = 42, color = 0x1A1A1A, align = airui.TEXT_ALIGN_RIGHT })
    ...
    airui.label({ parent = hint_bar, x = 20, y = 20, w = 440, h = 40, text = "点击空白位置下棋，夹住对方棋子可翻转", size = 24, color = 0x666666, align = airui.TEXT_ALIGN_CENTER })
...
local function open_handler()
    log.info("open_handler: called, win_id =", win_id)
    if win_id then
        log.info("open_handler: window already open, focusing existing window")
        exwin.focus(win_id)
        return
    end
    log.info("open_handler: creating new window")
    win_id = exwin.open({
        on_create = on_create,
    ...
    log.info("open_handler: window created, win_id =", win_id)
end

-- 导出函数供外部调用
function open_game_window()
    log.info("open_game_window: called")
    -- 强制重置所有状态
    win_id = nil
    main_container = nil
    pass_msgbox = nil
    ...
    log.info("open_game_window: creating new window")
    win_id = exwin.open({
        on_create = on_create,
        on_destroy = on_destroy,
        on_lose_focus = on_lose_focus,
    ...
    log.info("open_game_window: window created, win_id =", win_id)
end

-- 保持消息订阅以兼容其他调用方式
sys.subscribe("OPEN_GAME_OTHELLO_ONLINE_WIN", open_handler)
