...
local function sip_task(opts)
    ...
    log.info("JQsip", "sip_task started")
    ...
    local function netCB(netc, event, param)
        log.info("JQsip", "netCB event=", event, "param=", param)
        if param ~= 0 then
        ...
        if event == socket.ON_LINE then
            ...
            local req = build_register(state, nil)
            log.info("JQsip", "send REGISTER to", state.sip_server_addr, state.sip_server_port)
    ...
    local function mark_adapter_ready_if_available(adapter, reason)
        if adapter == nil or ready_adapters[adapter] then
            return ready_adapters[adapter] and true or false
        end
        if socket.adapter(adapter) then
            ready_adapters[adapter] = true
            log.info("sip", "adapter ready by probe:", adapter, reason or "")
            return true
        end
        return false
    end
    ...
    mark_adapter_ready_if_available(state.locked_adapter, "initial_locked")
    mark_adapter_ready_if_available(socket.dft(), "initial_default")

    log.info("sip", "initial ready adapters", ready_adapters)
    ...
    while true do
        ...
        if not ready_adapters[adapter_to_use] then
            mark_adapter_ready_if_available(adapter_to_use, "loop_probe")
        end

        if not ready_adapters[adapter_to_use] then
            log.info("sip", "adapter not ready after probe, waiting for IP_READY:", adapter_to_use)
            sys.wait(1000)
            if g_stop then
                break
            end
            ...
            log.info("JQsip", "socket.connect result=", succ, "server=", state.sip_server_addr, "port=", state.sip_server_port)
            if not succ then
...
function M.start(opts)
    log.info("JQsip", "start called, g_started=", g_started, "g_stop=", g_stop)
    if g_started then
        log.warn("JQsip", "already started, skip")
        return true
    end
