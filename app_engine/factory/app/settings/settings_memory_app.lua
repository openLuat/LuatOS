...
local function get_memory_info()
    ...
    -- 获取系统内存信息 (type = "sys")
    local ok1, total1, used1, max1 = pcall(rtos.meminfo, "sys")
    if ok1 then
    ...
    -- 获取虚拟机内存信息 (type = "lua")
    local ok2, total2, used2, max2 = pcall(rtos.meminfo, "lua")
    if ok2 then
    ...
    local ok3, total3, used3, max3 = pcall(rtos.meminfo, "psram")
    if ok3 then
...
local function memory_info_handler()
    local info = get_memory_info()
    sys.publish("MEMORY_INFO", info)
    log.info("settings_memory", "上报内存信息")
end

-- 订阅内存信息查询事件
sys.subscribe("MEMORY_INFO_GET", memory_info_handler)
