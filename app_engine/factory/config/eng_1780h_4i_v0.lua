--[[
@module  config.eng_1780h_4i_v0
@summary Air1780H 引擎主机 4.3寸480x272 SPI屏(ST6201) + WiFi(airlink/6205) + ES8311 + SD + NAND 配置文件
@version 1.0
@date    2026.08.14
@author  江访
@usage
...
2. AIRLINK_IRQ/WAKEUP 引脚需在 net_manager 初始化前经 airlink.config 单独配置
   （见文件底部注释，与 evb_1601_7i_v12 一致）。
]]
return {
    -- ===== 顶层信息 =====
    name = "Engine_Air1780H_4inch_480x272_000_V000",  -- 项目命名: {类型}_{芯片}_{尺寸}_{版本}
    chip = "Air1780H",                             -- 主控芯片: 纯MCU(Air780E系列去4G) + SPI屏 + 外挂WiFi
    baseboard = "合宙引擎 AIR1780H V000 4.3寸",      -- 底板型号
    ...
    power_on = {
        { pin = 4,  dir = 1, level = 0 },  -- WIFI_EN = GPIO4 拉高使能 6205 WiFi 模组
        { pin = 18, dir = 0, level = 1 },  -- FLASH_EN = GPIO18 拉高使能 NAND Flash
        { pin = 24, dir = 0, level = 1 },  -- 8311_EN = GPIO24 拉高使能音频（功放 + ES8311 供电）
        { pin = 22, dir = 0, level = 1 },  -- PA_EN = GPIO22 拉高使能音频（功放 + ES8311 供电）
    ...
    -- ===== 硬件配置 =====
    hw = {
        -- 屏幕: ST6201 SPI 4.3寸 480×272（专用 LCD SPI 接口 HWID_0）
        lcd = {
            model = "lcd_st6201",
            params = {
        ...
        -- PA_EN=GPIO22 同时作为功放使能与音频供电（本板音频供电与功放共用一路）
        audio = {
            ...
            -- ⚠️ 15号固件(core 115)不含 audio_v2，必须用旧框架(audio)。
            -- 对齐 Air780EHM 官方录音 demo（record_amr_file.lua）：完整参数见下
            ...
            dac_delay = 6,             -- DAC 启动前冗余时间；固件≥V2026 单位100ms 填6，
                                       -- <V2026 单位1ms 填600（demo 自适应逻辑，当前固件取 6）
    ...
    --       （参考 Air8301 网关经验: 混挂不同速率设备同总线会触发 device_config 重配）
    storage = {
        ...
        nand_flash = {
            ...
            -- pin_pwr = 18,             -- FLASH_EN 已由 power_on 的 GPIO18 处理，不重复配置
        },
