...
local COLOR_USER_BG      = 0x007AFF
local COLOR_ASSISTANT_BG = 0xEEEEEE
...
local function append_msg(text, msg_type)
    ...
    local bg = msg_type == "user" and COLOR_USER_BG or msg_type == "ai" and COLOR_ASSISTANT_BG or COLOR_BG
    local fg = msg_type == "user" and COLOR_WHITE or msg_type == "ai" and COLOR_TEXT or COLOR_TEXT_SEC
    ...
    for i, line in ipairs(lines) do
        ...
        local label = airui.label({
            ...
            text = line, font_size = math.floor(14 * d),
...
local function update_last_msg(text)
    ...
    local card_w = screen_w - 2 * margin
    local label_w = card_w - 2 * pad
    local fg = entry.type == "user" and COLOR_WHITE or entry.type == "ai" and COLOR_TEXT or COLOR_TEXT_SEC
    local bg = entry.type == "user" and COLOR_USER_BG or entry.type == "ai" and COLOR_ASSISTANT_BG or COLOR_BG
    ...
    for i = 1, math.max(#lines, #old) do
        ...
        elseif i <= #lines then
            ...
            local label = airui.label({
                parent = card, x = pad, y = math.floor(3 * d), w = label_w, h = LINE_H,
                text = lines[i], font_size = math.floor(14 * d),
                color = fg, align = airui.TEXT_ALIGN_LEFT,
            })
...
local function update_send_btn()
    ...
    if is_generating then
        ...
        set_btn_style(send_btn, COLOR_DIVIDER, COLOR_TEXT_SEC)
    else
        send_btn:set_disabled(false); send_btn:set_text("发送")
        set_btn_style(send_btn, COLOR_PRIMARY, COLOR_WHITE)
    end
end
...
local function update_tts_btn()
    ...
    if tts_on then
        ...
        set_btn_style(tts_btn, COLOR_GREEN, COLOR_WHITE)
    else
        tts_btn:set_text("语音关")
        set_btn_style(tts_btn, COLOR_DIVIDER, COLOR_TEXT_SEC)
    end
end
...
local function update_voice_btn()
    ...
    if rec_state == "recording" then
        ...
        set_btn_style(voice_btn, COLOR_DANGER, COLOR_WHITE)
    else
        voice_btn:set_text("录音")
        set_btn_style(voice_btn, COLOR_PRIMARY, COLOR_WHITE)
    end
end
...
local function update_title_status()
    ...
    if ws_connected then
        ...
        title_status:set_color(COLOR_GREEN)
    else
        title_status:set_text("未连接")
        title_status:set_color(COLOR_DANGER)
    end
end
...
local function build_ui()
    update_screen_size()
    local d = _G.density_scale or 1.0
    LINE_H = math.floor(14 * d)  -- 每行 label 高度
    ...
    airui.label({ parent = titlebar, x = math.floor(70 * d), y = tb_y, w = math.floor(screen_w - 180 * d), h = math.floor(34 * d),
        text = "AI 助手", font_size = math.floor(18 * d), color = COLOR_WHITE, align = airui.TEXT_ALIGN_CENTER })

    title_status = airui.label({ parent = titlebar, x = screen_w - math.floor(65 * d) - margin, y = tb_y,
        w = math.floor(60 * d), h = math.floor(34 * d), text = "未连接",
        font_size = math.floor(12 * d), color = COLOR_DANGER, align = airui.TEXT_ALIGN_CENTER })
    ...
    status_label = airui.label({ parent = main_container, x = 0, y = status_y, w = screen_w, h = status_h,
        text = "", font_size = math.floor(12 * d), color = COLOR_TEXT_SEC, align = airui.TEXT_ALIGN_CENTER })
    ...
    textarea = airui.textarea({ parent = input_area, x = margin, y = ta_y, w = ta_w, h = ta_h,
        text = "", placeholder = "输入消息...", font_size = math.floor(16 * d), max_len = 500, keyboard = keyboard })

    send_btn = airui.button({ parent = input_area, x = margin + ta_w + gap, y = ta_y, w = send_w, h = ta_h,
        text = "发送", font_size = math.floor(13 * d),
        style = { bg_color = COLOR_PRIMARY, text_color = COLOR_WHITE, border_width = 0, radius = 8 },
    ...
    tts_btn = airui.button({ parent = input_area, x = margin, y = btn2_y, w = btn3_w, h = btn2_h,
        text = "语音开", font_size = math.floor(12 * d),
        style = { bg_color = COLOR_PRIMARY, text_color = COLOR_WHITE, border_width = 0, radius = 8 },
    ...
    voice_btn = airui.button({ parent = input_area, x = margin + btn3_w + gap, y = btn2_y, w = btn3_w, h = btn2_h,
        text = "录音", font_size = math.floor(12 * d),
        style = { bg_color = COLOR_PRIMARY, text_color = COLOR_WHITE, border_width = 0, radius = 8 },
        on_click = function() toggle_recording() end })

    airui.button({ parent = input_area, x = margin + 2 * (btn3_w + gap), y = btn2_y, w = btn3_w, h = btn2_h,
        text = "清空", font_size = math.floor(12 * d),
        style = { bg_color = COLOR_DIVIDER, text_color = COLOR_TEXT_SEC, border_width = 0, radius = 8 },
        on_click = function()
