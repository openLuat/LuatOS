...
-- theme 提前到布局参数之前 require：下面的字号/尺寸常量在模块加载期就要用它
...
local button_font_size = theme.fs("label")   -- 原为手算 math.floor(16 * density)
local info_font_size = 0
local max_description_lines = 2 -- 描述最大行数，动态计算

-- 按钮自适应参数
local card_button_height = theme.dp(32)      -- 原为手算 math.floor(32 * density)
...
local function show_progress_dialog(app_name)
    ...
    local bar = airui.bar({
        ...
        max = 100,
        value = 0,
        bg_color = CLR.line_soft,
        indicator_color = CLR.primary,
        radius = theme.r("sm")
    })
...
main_container = theme.page_bg(airui.screen, screen_w, screen_h)
    ...
    local tir = math.floor(tih / 2)   -- 搜索框仍按半高取圆角
    -- 返回键统一走 theme.iconbtn：icon="back" 有图用图、缺图退化为 "<"
    -- 注意不要写 "←"(U+2190)，固件内置字库不含该字形，渲染为空白
    theme.iconbtn(tb, {
        x = theme.dp(8), y = tiy, w = tih, h = tih,
        radius = theme.R.sm, opa = theme.OPA.fill_hi,
        border = theme.C.stroke_hi,
        icon = "back", text = "<", text_color = theme.C.primary_light,
        on_click = function()
            if window_id then
    ...
    airui.button({
        ...
        text = "搜索",
        font_size = button_font_size,
        style = { bg_color = CLR.primary, pressed_bg_color = CLR.primary_deep, text_color = CLR.white, radius = tir, border_width = 0, pad = 4 },
        on_click = function()
    ...
    sort_select = airui.dropdown({
        ...
        default_index = 0,
        style = { bg_color = CLR.panel, border_color = CLR.line_soft, radius = stbr },
    ...
    airui.button({
        ...
        text = "刷新",
        font_size = button_font_size,
        style = { bg_color = CLR.line_soft, pressed_bg_color = CLR.line_soft, text_color = CLR.t1, radius = stbr, border_width = 1, border_color = CLR.line_soft },
        on_click = function()
            current_page = 1
            publish_get_list()
        end
    ...
    for i, cat in ipairs(categories) do
        local btn = airui.button({
            ...
            font_size = button_font_size,
            style = {
                bg_color = (cat == current_category) and CLR.primary or CLR.panel,
                pressed_bg_color = CLR.primary_deep,
                text_color = (cat == current_category) and CLR.white or CLR.t1,
                radius = ctr,
                border_width = 1,
                border_color = CLR.line_soft,
                pad = 0
            ...
            on_click = function()
                ...
                for cat_idx, button_obj in ipairs(category_buttons) do
                    ...
                    button_obj:set_style({
                        bg_color = is_active and CLR.primary or CLR.panel,
                        text_color = is_active and CLR.white or CLR.t1
                    })
                end
                current_page = 1
    ...
    prev_button = airui.button({
        ...
        style = { bg_color = CLR.primary, pressed_bg_color = CLR.primary_deep, text_color = CLR.white, radius = pnr, border_width = 0 },
        on_click = function()
...
local function render_apps(apps, has_more_pages)
    ...
    for idx, app in ipairs(apps) do
        ...
        -- 原格式用了 "↓"(U+2193)，内置字库不含该字形，改成文字"下载"
        local ift = string.format("%s | 下载 %s次", szt, dlt)
        airui.label({
            parent = card,
            x = icon_size + 20,
            y = ioy,
        ...
        if app.installed then
            if app.has_update then
                ...
                airui.button({
                    ...
                    text = "更新",
                    font_size = button_font_size,
                    style = { bg_color = CLR.primary, pressed_bg_color = CLR.primary_deep, text_color = CLR.white, radius = theme.r("lg"), border_width = 0 },
                    on_click = function()
                ...
                airui.button({
                    ...
                    text = "卸载",
                    font_size = button_font_size,
                    style = { bg_color = CLR.rose, pressed_bg_color = CLR.rose, text_color = CLR.white, radius = theme.r("lg"), border_width = 0 },
                    on_click = function()
                        local msg_box = airui.msgbox({
                            w = math.min(math.floor(screen_w * 0.78), screen_w - 20),
                            h = math.max(math.floor(screen_h * 0.28), 120),
