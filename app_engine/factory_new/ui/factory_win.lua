...
local function update_controls()
    ...
    if make_link then
        ...
        set_btn_style(send_btn, CLR.green, CLR.white)
        return
    end
    rec_btn:set_disabled(false)
...
local function append_msg(text, is_status)
    ...
    msg_table:set_cell_style("row", row, {
        cell_text_color = is_status and CLR.t2 or CLR.t1,
        cell_bg_color = is_status and CLR.bg or CLR.white,
    })
    msg_rows = row + 1
    -- 滚动到底部显示最新消息
...
main_container = theme.page_bg(airui.screen, screen_w, screen_h)
    ...
    msg_table = airui.table({
        ...
        style = {
            bg_color = CLR.bg,
            cell_bg_color = CLR.bg,
            cell_border_width = 0,
            border_width = 0,
            cell_text_color = CLR.t1,
            cell_font_size = theme.fs("h2"),
    ...
    local bar = airui.container({
        ...
        --[[ 静态底栏，不是滚动容器。裸 airui.container 的 scrollable 默认是 true，
        于是任何子控件越界 1px 都会让它画出一条滑动条 —— 下面 timer_label 就是。
        这里显式关掉，作为兜底：万一还有子控件算偏，也只会裁边，不会冒出一条滚动条。]]
        scrollable = false,
    })

    local d = _G.density_scale
    --[[ 计时（录音中显示）
    宽度必须扣掉容器的左右描边：bar 的 border_width = 1，
    LVGL 的内容区 = w - 2*(pad + border) = screen_w - 2（见 lv_obj_pos.c 的
    lv_obj_get_content_width = width - space_left - space_right），
    子控件还按 screen_w 摆就会右溢 2px。]]
    timer_label = airui.label({
        parent = bar, x = 0, y = math.floor(2 * d),
        w = screen_w - 2, h = math.floor(18 * d),
        text = "", font_size = theme.fs("micro"),
    ...
    textarea = theme.input({
        ...
        font_size = theme.fs("label"), max_len = 500, keyboard = keyboard,
    })
