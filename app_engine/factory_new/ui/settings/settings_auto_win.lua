...
local function show_password_input(title, on_confirm, on_cancel)
    ...
    local win = airui.win({
        ...
        style = { bg_color = CLR.panel, header_bg_color = CLR.panel_hi, content_bg_color = CLR.panel,
            title_text_color = CLR.t1, radius = theme.r("md"), title_align = airui.TEXT_ALIGN_CENTER,
    ...
    airui.label({
        ...
        text = "请输入密码", font_size = theme.fs("label"),
    ...
    local pwd_input = theme.input({
        ...
        font_size = theme.fs("body"), keyboard = soft_keyboard,
    })
    ...
    airui.button({
        parent = win, x = item_margin, y = y_off,
        w = btn_w, h = btn_h,
        text = "取消", font_size = theme.fs("body"),
        style = { bg_color = CLR.line_soft, pressed_bg_color = CLR.line_soft, text_color = CLR.t1, radius = theme.r("xs"),
    ...
    airui.button({
        ...
        w = btn_w, h = btn_h,
        text = "确定", font_size = theme.fs("body"),
        style = { bg_color = CLR.primary, pressed_bg_color = CLR.primary_deep, text_color = CLR.white, radius = theme.r("xs"),
            border_width = 0 },
        on_click = function()
...
local function show_password_edit()
    ...
    if current_has_password then
        airui.label({ parent = win, x = item_margin, y = y_off, w = element_w, h = label_h,
            text = "原密码", font_size = theme.fs("label"), color = CLR.t1 })
        y_off = y_off + label_h + gap
        old_pwd_input = theme.input({
    ...
    airui.label({ parent = win, x = item_margin, y = y_off, w = element_w, h = label_h,
        text = "新密码（留空则清除密码）", font_size = theme.fs("label"), color = CLR.t1 })
    y_off = y_off + label_h + gap
    ...
    airui.button({
        ...
        text = "保存", font_size = theme.fs("body"),
        style = { bg_color = CLR.primary, pressed_bg_color = CLR.primary_deep, text_color = CLR.white, radius = theme.r("xs"),
            border_width = 0 },
        on_click = function()
...
local function show_app_selector()
    ...
    for _, app in ipairs(pending_apps) do
        ...
        local item = airui.container({
            ...
            color = is_selected and CLR.line_soft or CLR.panel,
            on_click = function()
        ...
        airui.label({
            ...
            text = display_text, font_size = theme.fs("h3"),
            color = is_selected and CLR.primary or CLR.t1, align = airui.TEXT_ALIGN_LEFT,
        })
...
local function show_exit_password_dialog()
    ...
    airui.label({
        parent = win, x = item_margin, y = y_off,
        w = win_w - 2 * item_margin, h = label_h,
        text = "请输入密码以退出自启APP", font_size = theme.fs("label"),
...
local function create_info_card(parent, y, label_text, value_text, on_click, dimmed)
    local r = airui.container({
        ...
        color = dimmed and CLR.line_soft or CLR.panel, radius = theme.r("xs"),
        on_click = on_click,
    })
    ...
    airui.label({
        ...
        text = label_text, font_size = theme.fs("h2"),
        color = dimmed and CLR.t2 or CLR.t1, align = airui.TEXT_ALIGN_LEFT,
    })
    local vl = airui.label({
        parent = r, x = math.floor(150 * _G.density_scale), y = ly,
        w = card_w - math.floor(190 * _G.density_scale), h = lh,
        text = value_text or "", font_size = theme.fs("h3"),
        color = CLR.t2, align = airui.TEXT_ALIGN_RIGHT, long_mode = true,
    })
    airui.label({
...
--[[开关卡片
原来这个「开机自启」是一个没有任何滑块的实心药丸（容器 + 圆角 = 一条色块），
既不像开关、也点不准 —— 现在换成 theme.switch（轨道 + 滑块的真正开关）。
另外整行也可点：LVGL 不做事件冒泡，所以点滑块走滑块回调、点空白走整行回调，
两条路径各触发一次 on_toggle，不会重复触发。]]
local function create_switch_card(parent, y, label_text, is_on, on_toggle)
    local current = is_on and true or false
    local r = airui.container({
        parent = parent, x = margin, y = y, w = card_w, h = card_h,
        color = theme.C.surface, color_opacity = theme.OPA.glass,
        border_color = theme.C.stroke, border_width = 1, radius = theme.r("md"),
        on_click = function() on_toggle(not current) end,
    })

    local lh = theme.fs("h2") + 3
    local ly = math.max(0, math.floor((card_h - lh) / 2))
    ...
    airui.label({
        parent = r, x = math.floor(20 * _G.density_scale), y = ly,
        w = sw_x - math.floor(44 * _G.density_scale), h = lh,
        text = label_text, font_size = theme.fs("h2"),
        color = CLR.t1, align = airui.TEXT_ALIGN_LEFT,
    })

    switch_bg = theme.switch(r, {
        x = sw_x, y = sw_y, w = sw_w, h = sw_h,
        checked = current, color = CLR.primary,
        on_change = function(handle)
            current = handle:get_state()
            on_toggle(current)
        end,
    })
    switch_is_on = current
end
        ...
        function()
            if not current_enabled then
                local mb = airui.msgbox({
                    ...
                    style = { text_font_size = theme.fs("body") },
