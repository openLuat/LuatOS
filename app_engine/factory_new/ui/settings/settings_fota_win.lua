...
local function update_screen_size()
    ...
    -- 小屏会异常放大、大屏反而偏小。统一改用语义档位。
    font_size  = theme.fs("h3")        -- 输入控件文字
    font_size2 = theme.fs("caption")   -- 表单标签 / 说明文字
end
...
local function show_download_prompt(message)
    ...
    local mh = math.floor(screen_h * 0.28)
    local font_size = theme.fs("label")   -- 弹窗正文（原按 screen_h 比例，本文件第三套算法）
    airui.msgbox({
        parent = main_container,
        w = mw, h = mh,
        style = { text_font_size = font_size },
...
main_container = theme.page_bg(airui.screen, screen_w, screen_h)
    ...
    airui.button({
        ...
        style = {
            bg_color = CLR.green,
            pressed_bg_color = theme.C.green_deep,
            text_color = CLR.white,
            radius = math.floor(btn_h * 0.2),
            border_width = 0
        },
