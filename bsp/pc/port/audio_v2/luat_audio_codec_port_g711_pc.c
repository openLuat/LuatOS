/* PC-local G711 adapter: the codec API takes PCM samples, not byte counts. */
#include "luat_base.h"
...
static int _g711_codec_encode(luat_audio_data_codec_t* codec,
                  const uint8_t *input, uint32_t input_size,
                  uint8_t *output, uint32_t *encoded_used_size, uint32_t *encoded_output_size) {
    if (!codec || !codec->encode_ctx || !input || !output || !encoded_used_size ||
        !encoded_output_size || !input_size || (input_size & 1U)) {
        return -LUAT_ERROR_PARAM_INVALID;
    }
    int ret = g711_encoder_get_data(codec->encode_ctx, (const int16_t *)input,
                                    input_size / sizeof(int16_t), output,
                                    encoded_output_size);
    if (ret <= 0) {
        return -LUAT_ERROR_OPERATION_FAILED;
    }
