...
sys.taskInit(function()
    ...
    local prikey = io.readFile("/luadb/privkey2.pem")
    local pubkey = io.readFile("/luadb/public2.pem")

    -- 读取公钥并马上加密数据
    local res = rsa.encrypt(pubkey, "abc")
    -- 打印结果
    ...
    if res then
        -- 读取私钥, 然后解码数据
        local dst = rsa.decrypt(prikey, res, "")
        log.info("rsa", "decrypt", dst and #dst or 0, dst and dst:toHex() or "")
    end

    -- 演示签名和验签
    local hash = crypto.sha256("1234567890"):fromHex()
    -- 签名通常很慢, 通常是服务器做
    local sig = rsa.sign(prikey, rsa.MD_SHA256, hash, "")
    log.info("rsa", "sign", sig and #sig or 0, sig and sig:toHex() or "")
    if sig then
        -- 验签是很快的
        local ret = rsa.verify(pubkey, rsa.MD_SHA256, hash, sig)
