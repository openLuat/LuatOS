 ...
 */
static int l_airui_lottie(lua_State *L)
{
    luaL_checktype(L, 1, LUA_TTABLE);
    lv_obj_t *lottie = airui_lottie_create_from_config(L, 1);
    if (lottie == NULL) {
        lua_pushnil(L);
        return 1;
    }

    airui_push_component_userdata(L, lottie, AIRUI_LOTTIE_MT);
    return 1;
}

static lv_obj_t *lottie_check(lua_State *L)
{
    return airui_check_component(L, 1, AIRUI_LOTTIE_MT);
}
...
{
    airui_lottie_play(lottie_check(L));
    return 0;
}
...
{
    airui_lottie_pause(lottie_check(L));
    return 0;
}
...
{
    airui_lottie_stop(lottie_check(L));
    return 0;
}
...
static int l_lottie_set_speed(lua_State *L)
{
    ...
    airui_lottie_set_speed(obj, speed);
    return 0;
}
...
static int l_lottie_set_loop(lua_State *L)
{
    ...
    airui_lottie_set_loop(obj, loop);
    return 0;
}
...
static int l_lottie_set_progress(lua_State *L)
{
    ...
    airui_lottie_set_progress(obj, progress);
    return 0;
}
...
static int l_lottie_set_src(lua_State *L)
{
    ...
    if (lua_type(L, 2) == LUA_TSTRING) {
        ...
        airui_lottie_set_src_file(obj, path);
        return 0;
    }
    ...
    if (lua_type(L, 2) == LUA_TTABLE) {
        ...
        if (data != NULL && len > 0) {
            airui_lottie_set_src_data(obj, data, len);
            return 0;
        }
    }

    luaL_error(L, "airui.lottie:set_src expects string path or table {path=...,data=...}");
    return 0;
}
...
int airui_lottie_create(lua_State *L)
{
    return l_airui_lottie(L);
}
