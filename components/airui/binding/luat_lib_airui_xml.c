...
typedef struct {
    lua_State *L;
    int lua_ref;
} airui_xml_event_ref_t;

typedef struct {
    lv_obj_t *keyboard;
} airui_xml_keyboard_binding_t;

// trampoline + release helper 声明
static void airui_xml_event_trampoline(lv_event_t *event);
static void airui_xml_event_release(lv_event_t *event);
static void airui_xml_keyboard_focus_cb(lv_event_t *event);
static void airui_xml_keyboard_defocus_cb(lv_event_t *event);
static void airui_xml_keyboard_release(lv_event_t *event);
/**
 * 初始化 LVGL XML 模块，保证后续可以注册/加载 XML。
 * @api airui.xml_init()
 * @return bool 永远返回 true（如果 XML 被禁用则只记录警告）。
 */
int l_airui_xml_init(lua_State *L) {
    (void)L;
    airui_xml_init();
    return 0;
}
...
/**
 * 关闭 LVGL XML 模块并释放资源。
 * @api airui.xml_deinit()
 * @return bool 永远返回 true，供 Lua 判断是否执行。
 */
int l_airui_xml_deinit(lua_State *L) {
    (void)L;
    airui_xml_deinit();
    return 0;
}
...
/**
 * Lua 层按文件注册 XML 组件定义。
 * @api airui.xml_register_from_file(path)
 * @string path XML 文件路径
 * @return bool 成功返回 true，失败返回 false（或 XML 功能不可用）。
 */
int l_airui_xml_register_from_file(lua_State *L) {
    const char *path = luaL_checkstring(L, 1);
    if (!airui_xml_register_from_file(path)) {
        lua_pushboolean(L, 0);
        return 1;
    }
    lua_pushboolean(L, 1);
 ...
 * @api airui.xml_register_from_data(name, data)
 * @string name 注册名称（可作为 `xml_create_screen` 的参数）
 * @string data XML 文本
 * @return bool 成功返回 true，失败返回 false。
 */
int l_airui_xml_register_from_data(lua_State *L) {
    const char *name = luaL_checkstring(L, 1);
    const char *data = luaL_checkstring(L, 2);
    if (!airui_xml_register_from_data(name, data)) {
        lua_pushboolean(L, 0);
        return 1;
    }
    lua_pushboolean(L, 1);
...
/**
 * 提前注册 XML 中使用的图片资源。
 * @api airui.xml_register_image(name, src)
 ...
 */
int l_airui_xml_register_image(lua_State *L) {
    const char *name = luaL_checkstring(L, 1);
    const void *src = NULL;
    ...
    if (!airui_xml_register_image(name, src)) {
        lua_pushboolean(L, 0);
 ...
 * @api airui.xml_register_font(name, font)
 * @string name XML 中使用的 font 名称
 * @lightuserdata font `airui.font_load` 返回的字体指针
 * @return bool 成功返回 true，否则返回 false + 错误信息。
 */
int l_airui_xml_register_font(lua_State *L) {
    const char *name = luaL_checkstring(L, 1);
    ...
    if (!airui_xml_register_font(name, font)) {
        lua_pushboolean(L, 0);
...
/**
 * 创建已经注册的 XML 屏幕并返回包裹的 `airui.container`。
 * @api airui.xml_create_screen(name)
 * @string name `register_from_*` 时指定的名称
 * @return userdata `airui.container` 或 nil（失败）。
 */
int l_airui_xml_create_screen(lua_State *L) {
    const char *name = luaL_checkstring(L, 1);
    lv_obj_t *screen = airui_xml_create_screen(name);
    if (screen == NULL) {
        lua_pushnil(L);
        return 1;
    }
    airui_push_component_userdata(L, screen, AIRUI_CONTAINER_MT);
    return 1;
}

/**
 * 给 XML 名称对应的控件绑定 LVGL 事件与 Lua 回调。
 * @api airui.xml_bind_event(name, event_code, callback)
 * @string name 注册时的控件名称
 * @int event_code LVGL 事件码（默认为 `LV_EVENT_CLICKED`）
 * @function callback Lua 回调
 * @return bool 成功返回 true，否则返回 false + 错误信息。
 */
int l_airui_xml_bind_event(lua_State *L) {
    const char *name = luaL_checkstring(L, 1);
    lv_event_code_t code = (lv_event_code_t)luaL_optinteger(L, 2, LV_EVENT_CLICKED);
    luaL_checktype(L, 3, LUA_TFUNCTION);

    lv_obj_t *target = airui_xml_find_object(name);
    if (target == NULL) {
        lua_pushboolean(L, 0);
    ...
    airui_xml_event_ref_t *ref = (airui_xml_event_ref_t *)luat_heap_malloc(sizeof(*ref));
    if (ref == NULL) {
        lua_pushboolean(L, 0);
    ...
    if (!airui_xml_add_event_cb(target, code, airui_xml_event_trampoline, ref, airui_xml_event_release)) {
        lua_pushboolean(L, 0);
...
int l_airui_xml_keyboard_bind(lua_State *L) {
    ...
    lv_obj_t *textarea = airui_xml_find_object(textarea_name);
    if (textarea == NULL) {
        lua_pushboolean(L, 0);
    ...
    airui_xml_keyboard_binding_t *binding = (airui_xml_keyboard_binding_t *)luat_heap_malloc(sizeof(*binding));
    if (binding == NULL) {
        lua_pushboolean(L, 0);
    ...
    if (!airui_xml_bind_keyboard_events(textarea, airui_xml_keyboard_focus_cb, airui_xml_keyboard_defocus_cb, airui_xml_keyboard_release, binding)) {
        lua_pushboolean(L, 0);
 ...
 */
static void airui_xml_event_trampoline(lv_event_t *event) {
    airui_xml_event_ref_t *ref = lv_event_get_user_data(event);
...
static void airui_xml_keyboard_focus_cb(lv_event_t *event) {
    airui_xml_keyboard_binding_t *binding = lv_event_get_user_data(event);
