/**
 * @file luat_airui_fs.c
 * @summary AIRUI 文件系统驱动实现
 * 整体流程为三层：
    *  底层是 luat_fs 的文件系统接口（读写、打开、关闭等），负责直接跟存储打交道。
    * 中间层是 AIRUI 的 internal 实现，它把 luat_fs 的 API 包装成 LVGL 需要的回调函数（比如 lv_fs_drv_t 的 read_cb、open_cb 等），并做一些适配。
 ...
 * @return 文件句柄
 */
static void *fs_open_internal(airui_ctx_t *ctx, const char *path, lv_fs_mode_t mode)
{
    if (ctx == NULL) {
        return NULL;
    }
 ...
 * @param br 实际读取字节数输出
 */
static lv_fs_res_t fs_read_internal(airui_ctx_t *ctx, void *file_p, void *buf, uint32_t btr, uint32_t *br)
{
 ...
 */
static lv_fs_res_t fs_close_internal(airui_ctx_t *ctx, void *file_p)
{
 ...
 * @param whence 参考点
 */
static lv_fs_res_t fs_seek_internal(airui_ctx_t *ctx, void *file_p, uint32_t pos, lv_fs_whence_t whence)
{
 ...
 * @param pos_p 输出当前位置
 */
static lv_fs_res_t fs_tell_internal(airui_ctx_t *ctx, void *file_p, uint32_t *pos_p)
{
...
static FILE *fs_fopen_default(const char *path, lv_fs_mode_t mode)
{
    ...
    if (fd == NULL) {
        LLOGW("airui_fs: failed to open %s (%s)", path, mode_str);
    }
    return fd;
}
...
{
    airui_ctx_t *ctx = (airui_ctx_t *)drv->user_data;
    if (ctx == NULL) {
        LLOGE("airui_fs: ctx is NULL");
        return NULL;
    }

    char file_path[AIRUI_FS_PATH_MAX] = {0};
...
bool airui_fs_load_file(airui_ctx_t *ctx, const char *path, void **out_data, size_t *out_len)
{
    ...
    if (file_p == NULL) {
        LLOGW("airui_fs_load_file: open %s failed", file_path);
        return false;
    }
 ...
 * @return 0 成功，<0 失败
 */
static int fs_register_drv(airui_ctx_t *ctx, lv_fs_drv_t *drv, char letter)
{
    ...
    LLOGD("airui_fs: registered driver '%c'", letter);
    return 0;
}
...
{
    if (ctx == NULL) {
        LLOGE("airui_fs_init failed: ctx is NULL");
        return AIRUI_ERR_INVALID_PARAM;
    }
    ...
    if (ret != 0) {
        LLOGE("airui_fs_init failed: register driver '/' failed");
        return ret;
    }

    LLOGD("airui_fs_init: file system drivers initialized");
    return 0;
}
