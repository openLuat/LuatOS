...
static luat_display_t* display_confs[LUAT_DISPLAY_CONF_COUNT] = {0};
...
const char* luat_display_name(luat_display_t *disp) {
    return disp->panel_ops ? disp->panel_ops->name : "unknown";
}

int luat_display_init_default(luat_display_t *disp) {
    disp->is_initialized = 0;

    if (disp->pin_pwr != LUAT_GPIO_NONE) {
        luat_gpio_mode(disp->pin_pwr, Luat_GPIO_OUTPUT, Luat_GPIO_DEFAULT, Luat_GPIO_LOW);
    }

    luat_gpio_mode(disp->pin_rst, Luat_GPIO_OUTPUT, Luat_GPIO_DEFAULT, Luat_GPIO_LOW);

    if (disp->pin_pwr != LUAT_GPIO_NONE) {
        luat_gpio_set(disp->pin_pwr, Luat_GPIO_LOW);
    }

    if (disp->panel_ops->user_ctrl_init) {
        int res = disp->panel_ops->user_ctrl_init(disp);
        if (res > 0) {
            goto INIT_NOT_DONE;
        }
        goto INIT_DONE;
    ...
    luat_gpio_set(disp->pin_rst, Luat_GPIO_LOW);
    luat_rtos_task_sleep(100);
    luat_gpio_set(disp->pin_rst, Luat_GPIO_HIGH);
...
int luat_display_on(luat_display_t *disp) {
    if (disp->pin_pwr != LUAT_GPIO_NONE) {
        luat_gpio_set(disp->pin_pwr, Luat_GPIO_HIGH);
    }
    if (disp->pin_bl != LUAT_GPIO_NONE) {
        luat_gpio_set(disp->pin_bl, Luat_GPIO_HIGH);
    }
    disp->power_state = LUAT_DISPLAY_POWER_ON;
    return 0;
}
