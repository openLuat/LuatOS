...
} easylvgl_lottie_data_t;

static easylvgl_lottie_data_t *lottie_get_data(easylvgl_component_meta_t *meta);
static void lottie_anim_completed_cb(lv_anim_t *anim);
static void lottie_update_speed(easylvgl_lottie_data_t *data);
static void lottie_update_loop(easylvgl_lottie_data_t *data);
static void lottie_cleanup_on_error(easylvgl_ctx_t *ctx, easylvgl_lottie_data_t *data, easylvgl_component_meta_t *meta, lv_obj_t *lottie);
static bool lottie_alloc_buffer(easylvgl_ctx_t *ctx, easylvgl_lottie_data_t *data, int w, int h);
static bool lottie_setup_anim(easylvgl_component_meta_t *meta, lv_obj_t *obj);
static bool lottie_apply_src_data(easylvgl_component_meta_t *meta, lv_obj_t *obj, const void *src, size_t size);
static bool lottie_load_from_file(easylvgl_component_meta_t *meta, lv_obj_t *obj, const char *path);
...
lv_obj_t *easylvgl_lottie_create_from_config(void *L, int idx)
{
    ...
    // 安全分配元数据和状态
    easylvgl_component_meta_t *meta = easylvgl_component_meta_alloc(ctx, lottie, EASYLVGL_COMPONENT_LOTTIE);
    if (meta == NULL) {
        lottie_cleanup_on_error(ctx, NULL, NULL, lottie);
        return NULL;
    }

    easylvgl_lottie_data_t *data = (easylvgl_lottie_data_t *)luat_heap_malloc(sizeof(easylvgl_lottie_data_t));
    if (data == NULL) {
        lottie_cleanup_on_error(ctx, NULL, meta, lottie);
        return NULL;
    }

    memset(data, 0, sizeof(easylvgl_lottie_data_t));
    data->speed = 1.0f;
    data->loop = true;
    data->auto_play = true;
    meta->user_data = data;

    data->loop = easylvgl_marshal_bool(L, idx, "loop", data->loop);
    data->auto_play = easylvgl_marshal_bool(L, idx, "auto_play", data->auto_play);
    ...
    // 绑定 Lua 回调
    int ready_ref = easylvgl_component_capture_callback(L, idx, "on_ready");
    if (ready_ref != LUA_NOREF) {
        easylvgl_component_bind_event(meta, EASYLVGL_EVENT_READY, ready_ref);
    }

    int complete_ref = easylvgl_component_capture_callback(L, idx, "on_complete");
    if (complete_ref != LUA_NOREF) {
        easylvgl_component_bind_event(meta, EASYLVGL_EVENT_COMPLETE, complete_ref);
    }
 ...
 * 从元数据中获取 Lottie 缓存数据
 * @param meta 组件元数据
 * @return 绑定的 easylvgl_lottie_data_t，或 NULL
 */
static easylvgl_lottie_data_t *lottie_get_data(easylvgl_component_meta_t *meta)
{
    if (meta == NULL) {
    ...
    return (easylvgl_lottie_data_t *)meta->user_data;
}
...
static void lottie_anim_completed_cb(lv_anim_t *anim)
{
    ...
    easylvgl_component_meta_t *meta = (easylvgl_component_meta_t *)lv_anim_get_user_data(anim);
    if (meta == NULL || meta->ctx == NULL) {
        return;
    }

    easylvgl_component_call_callback(meta, EASYLVGL_EVENT_COMPLETE, meta->ctx->L);
}
...
static bool lottie_alloc_buffer(easylvgl_ctx_t *ctx, easylvgl_lottie_data_t *data, int w, int h)
{
    ...
    void *buf = easylvgl_buffer_alloc(ctx, buf_size, EASYLVGL_BUFFER_OWNER_SYSTEM);
    if (buf == NULL) {
...
static bool lottie_setup_anim(easylvgl_component_meta_t *meta, lv_obj_t *obj)
{
    ...
    easylvgl_lottie_data_t *data = lottie_get_data(meta);
    if (data == NULL) {
        return false;
    }
...
static bool lottie_apply_src_data(easylvgl_component_meta_t *meta, lv_obj_t *obj, const void *src, size_t size)
{
    ...
    easylvgl_component_call_callback(meta, EASYLVGL_EVENT_READY, meta->ctx ? meta->ctx->L : NULL);
    return true;
}
...
static bool lottie_load_from_file(easylvgl_component_meta_t *meta, lv_obj_t *obj, const char *path)
{
    ...
    // 通过 EasyLVGL 的文件系统接口读取数据
    void *buffer = NULL;
    size_t len = 0;
    if (!easylvgl_fs_load_file(meta->ctx, path, &buffer, &len)) {
 ...
 */
static void lottie_cleanup_on_error(easylvgl_ctx_t *ctx, easylvgl_lottie_data_t *data,
                                    easylvgl_component_meta_t *meta, lv_obj_t *lottie)
{
    if (data != NULL) {
        if (data->draw_buf_data != NULL && ctx != NULL) {
            easylvgl_buffer_free(ctx, data->draw_buf_data);
            data->draw_buf_data = NULL;
        }
        luat_heap_free(data);
        if (meta != NULL) {
...
int easylvgl_lottie_play(lv_obj_t *lottie)
{
    ...
    easylvgl_component_meta_t *meta = easylvgl_component_meta_get(lottie);
    easylvgl_lottie_data_t *data = lottie_get_data(meta);
    if (data == NULL || data->anim == NULL) {
        return EASYLVGL_ERR_INVALID_PARAM;
    }
...
int easylvgl_lottie_set_src_file(lv_obj_t *lottie, const char *path)
{
    ...
    return lottie_load_from_file(meta, lottie, path) ? EASYLVGL_OK : EASYLVGL_ERR_INVALID_PARAM;
}
...
int easylvgl_lottie_set_src_data(lv_obj_t *lottie, const void *data, size_t size)
{
    ...
    return lottie_apply_src_data(meta, lottie, data, size) ? EASYLVGL_OK : EASYLVGL_ERR_INVALID_PARAM;
}
...
int easylvgl_lottie_destroy(lv_obj_t *lottie)
{
    ...
    if (data != NULL) {
        if (data->draw_buf_data != NULL && meta->ctx != NULL) {
            // 释放缓冲区
            easylvgl_buffer_free(meta->ctx, data->draw_buf_data);
            data->draw_buf_data = NULL;
        }
        luat_heap_free(data);
        meta->user_data = NULL;
...
#else
// Lottie 组件未启用时的空实现
lv_obj_t *easylvgl_lottie_create_from_config(void *L, int idx){ return NULL; }
int easylvgl_lottie_play(lv_obj_t *lottie){ return EASYLVGL_ERR_NOT_SUPPORTED; }
int easylvgl_lottie_pause(lv_obj_t *lottie){ return EASYLVGL_ERR_NOT_SUPPORTED; }
int easylvgl_lottie_stop(lv_obj_t *lottie){ return EASYLVGL_ERR_NOT_SUPPORTED; }
int easylvgl_lottie_set_src_file(lv_obj_t *lottie, const char *path){ return EASYLVGL_ERR_NOT_SUPPORTED; }
int easylvgl_lottie_set_src_data(lv_obj_t *lottie, const void *data, size_t size){ return EASYLVGL_ERR_NOT_SUPPORTED; }
int easylvgl_lottie_set_loop(lv_obj_t *lottie, bool loop){ return EASYLVGL_ERR_NOT_SUPPORTED; }
int easylvgl_lottie_set_speed(lv_obj_t *lottie, float speed){ return EASYLVGL_ERR_NOT_SUPPORTED; }
int easylvgl_lottie_set_progress(lv_obj_t *lottie, float progress){ return EASYLVGL_ERR_NOT_SUPPORTED; }
int easylvgl_lottie_destroy(lv_obj_t *lottie){ return EASYLVGL_ERR_NOT_SUPPORTED; }
#endif