...
lv_timer_t *easylvgl_msgbox_release_user_data(easylvgl_component_meta_t *meta)
{
    ...
    easylvgl_msgbox_data_t *data = (easylvgl_msgbox_data_t *)meta->user_data;
    if (data != NULL) {
...
static void easylvgl_msgbox_timeout_cb(lv_timer_t *timer) {
    ...
    easylvgl_component_meta_t *meta = easylvgl_component_meta_get(msgbox);
    if (meta != NULL) {
        easylvgl_msgbox_release_user_data(meta);
    }
...
lv_obj_t *easylvgl_msgbox_create_from_config(void *L, int idx)
{
    ...
    bool auto_center = easylvgl_marshal_bool(L, idx, "auto_center", true);
    int timeout = easylvgl_marshal_integer(L, idx, "timeout", 0);
    ...
    for (int i = 1; i <= button_count; i++) {
        const char *label_text = easylvgl_marshal_table_string_at(L, idx, "buttons", i);
        if (label_text == NULL) {
            label_text = "OK";
        }
        lv_obj_t *btn = lv_msgbox_add_footer_button(msgbox, label_text);
        if (btn != NULL) {
            lv_obj_add_event_cb(btn, easylvgl_msgbox_button_event_cb, LV_EVENT_CLICKED, msgbox);
        }
    }

    easylvgl_component_meta_t *meta = easylvgl_component_meta_alloc(
        ctx, msgbox, EASYLVGL_COMPONENT_MSGBOX);
    if (meta == NULL) {
        lv_msgbox_close(msgbox);
        return NULL;
    }

    easylvgl_msgbox_data_t *data = (easylvgl_msgbox_data_t *)luat_heap_malloc(sizeof(easylvgl_msgbox_data_t));
    if (data == NULL) {
    ...
    if (timeout > 0) {
        lv_timer_t *timer = lv_timer_create(easylvgl_msgbox_timeout_cb, timeout, msgbox);
        if (timer != NULL) {
            data->timeout_timer = timer;
        }
    }

    int callback_ref = easylvgl_component_capture_callback(L, idx, "on_action");
    if (callback_ref != LUA_NOREF) {
        easylvgl_msgbox_set_on_action(msgbox, callback_ref);
    }
...
int easylvgl_msgbox_set_on_action(lv_obj_t *msgbox, int callback_ref)
{
    ...
    if (meta->callback_refs[EASYLVGL_EVENT_ACTION] != LUA_NOREF) {
        luaL_unref(L_state, LUA_REGISTRYINDEX, meta->callback_refs[EASYLVGL_EVENT_ACTION]);
        meta->callback_refs[EASYLVGL_EVENT_ACTION] = LUA_NOREF;
    }
    ...
    meta->callback_refs[EASYLVGL_EVENT_ACTION] = callback_ref;
    return EASYLVGL_OK;
}
