/**
 * @file luat_easylvgl_time_luatos.c
 * @summary LuatOS 时基实现
 ...
 * 获取毫秒 tick
 */
static uint32_t luatos_time_get_tick(easylvgl_ctx_t *ctx)
{
    (void)ctx;
 ...
 * 毫秒延时
 */
static void luatos_time_delay_ms(easylvgl_ctx_t *ctx, uint32_t ms)
{
    (void)ctx;
    luat_rtos_task_sleep(ms);
}

static const easylvgl_time_ops_t luatos_time_ops = {
    .get_tick = luatos_time_get_tick,
    .delay_ms = luatos_time_delay_ms
};

const easylvgl_time_ops_t *easylvgl_platform_luatos_get_time_ops(void)
{
