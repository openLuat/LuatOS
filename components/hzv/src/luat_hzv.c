...
#define HZV_HEADER_SIZE        128u
#define HZV_PACKET_HEADER_SIZE 32u
#define HZV_MAX_PACKET_SIZE    (8u * 1024u * 1024u)

struct luat_hzv_reader {
    void *fp;
    luat_hzv_info_t info;
    uint64_t file_size;
...
static int hz_seek(luat_hzv_reader_t *reader, uint64_t offset)
{
    if (reader == NULL || reader->fp == NULL || offset > (uint64_t)LONG_MAX) {
        return LUAT_HZV_ERR_IO;
    }
    return HZ_FSEEK(reader->fp, (long)offset, SEEK_SET) == 0 ? LUAT_HZV_OK : LUAT_HZV_ERR_IO;
}

static int hz_read_exact(void *fp, void *buf, size_t size)
{
    return HZ_FREAD(buf, 1, size, fp) == size ? LUAT_HZV_OK : LUAT_HZV_ERR_IO;
}

static int hz_ensure_payload(luat_hzv_reader_t *reader, size_t size)
{
    size_t capacity;
    uint8_t *payload;
    ...
    if (size == 0 || size > HZV_MAX_PACKET_SIZE) {
        return LUAT_HZV_ERR_FORMAT;
    }
    capacity = reader->payload_capacity ? reader->payload_capacity : 4096u;
    while (capacity < size) {
        if (capacity > HZV_MAX_PACKET_SIZE / 2u) {
            capacity = HZV_MAX_PACKET_SIZE;
            break;
        }
        capacity *= 2u;
    }
...
static int hz_read_video_internal(luat_hzv_reader_t *reader,
                                  luat_hzv_packet_t *packet,
                                  int load_payload)
{
    uint8_t header[HZV_PACKET_HEADER_SIZE];

    if (reader == NULL || (load_payload && packet == NULL)) {
        return LUAT_HZV_ERR_PARAM;
    }
    ...
    while (reader->next_packet_offset < reader->packet_end_offset &&
        ...
        if (packet_offset + HZV_PACKET_HEADER_SIZE > reader->packet_end_offset) {
            return LUAT_HZV_ERR_FORMAT;
        }
        ret = hz_seek(reader, packet_offset);
        if (ret != LUAT_HZV_OK || hz_read_exact(reader->fp, header, sizeof(header)) != LUAT_HZV_OK) {
            return LUAT_HZV_ERR_IO;
        }
        if (memcmp(header, "HZPK", 4) != 0 || hz_le16(header + 4) != HZV_PACKET_HEADER_SIZE) {
            return LUAT_HZV_ERR_FORMAT;
        }
        ...
        next_offset = packet_offset + HZV_PACKET_HEADER_SIZE +
                      (((uint64_t)payload_size + 3u) & ~(uint64_t)3u);
        if (payload_size == 0 || payload_size > HZV_MAX_PACKET_SIZE ||
        ...
        reader->packets_seen++;
        if (stream_id != LUAT_HZV_STREAM_VIDEO) {
            continue;
        }
        ...
        if (hz_read_exact(reader->fp, reader->payload, payload_size) != LUAT_HZV_OK) {
            return LUAT_HZV_ERR_IO;
        }
        if ((reader->info.flags & LUAT_HZV_FLAG_PACKET_CRC32) != 0u &&
            hz_crc32(reader->payload, payload_size) != payload_crc) {
            return LUAT_HZV_ERR_CRC;
        }
...
{
    uint8_t header[HZV_HEADER_SIZE];
    uint8_t crc_header[HZV_HEADER_SIZE];
    uint32_t header_crc;
    uint32_t header_size;
    uint64_t index_offset;
    uint64_t index_size;
    long file_size;
    luat_hzv_reader_t *reader;
    void *fp;
    ...
    if (HZ_FSEEK(fp, 0, SEEK_END) != 0 || (file_size = HZ_FTELL(fp)) < (long)HZV_HEADER_SIZE ||
        HZ_FSEEK(fp, 0, SEEK_SET) != 0 || hz_read_exact(fp, header, sizeof(header)) != LUAT_HZV_OK) {
        HZ_FCLOSE(fp);
        return NULL;
    }
    if (memcmp(header, "HZV1", 4) != 0 || hz_le16(header + 4) != 1u) {
        HZ_FCLOSE(fp);
        return NULL;
    }
    header_size = hz_le32(header + 8);
    if (header_size != HZV_HEADER_SIZE || hz_le32(header + 16) == 0u) {
        HZ_FCLOSE(fp);
        return NULL;
    }
    ...
    reader = (luat_hzv_reader_t *)HZ_MALLOC(sizeof(luat_hzv_reader_t));
    if (reader == NULL) {
        HZ_FCLOSE(fp);
        return NULL;
    }
    ...
    if (reader->packet_data_offset < HZV_HEADER_SIZE ||
        ...
        reader->info.video_codec != LUAT_HZV_VIDEO_MJPEG ||
...
int luat_hzv_peek_video(luat_hzv_reader_t *reader, uint64_t *pts, uint32_t *duration)
{
    ...
    if (reader == NULL || pts == NULL || duration == NULL) return LUAT_HZV_ERR_PARAM;
    offset = reader->next_packet_offset;
    seen = reader->packets_seen;
    while (offset < reader->packet_end_offset && seen < reader->info.packet_count) {
        uint32_t payload_size;
        uint64_t next_offset;
        if (offset + HZV_PACKET_HEADER_SIZE > reader->packet_end_offset ||
            hz_seek(reader, offset) != LUAT_HZV_OK ||
            hz_read_exact(reader->fp, header, sizeof(header)) != LUAT_HZV_OK) {
            (void)hz_seek(reader, reader->next_packet_offset);
            return LUAT_HZV_ERR_IO;
        }
        if (memcmp(header, "HZPK", 4) != 0 || hz_le16(header + 4) != HZV_PACKET_HEADER_SIZE) {
            (void)hz_seek(reader, reader->next_packet_offset);
        ...
        payload_size = hz_le32(header + 20);
        next_offset = offset + HZV_PACKET_HEADER_SIZE +
                      (((uint64_t)payload_size + 3u) & ~(uint64_t)3u);
        if (payload_size == 0 || next_offset > reader->packet_end_offset || next_offset <= offset) {
            (void)hz_seek(reader, reader->next_packet_offset);
            return LUAT_HZV_ERR_FORMAT;
        }
        if (header[6] == LUAT_HZV_STREAM_VIDEO) {
            *pts = hz_le64(header + 8);
            *duration = hz_le32(header + 16);
            (void)hz_seek(reader, reader->next_packet_offset);
            return LUAT_HZV_OK;
...
int luat_hzv_extract_audio(luat_hzv_reader_t *reader, uint8_t **data, size_t *size)
{
    ...
    size_t capacity = 0;
    uint32_t seen = 0;
    int result = LUAT_HZV_OK;

    if (reader == NULL || data == NULL || size == NULL) {
        return LUAT_HZV_ERR_PARAM;
    }
    *data = NULL;
    *size = 0;
    if (reader->info.audio_codec == LUAT_HZV_AUDIO_NONE) {
        return LUAT_HZV_ERR_CODEC;
    }

    offset = reader->packet_data_offset;
    while (offset < reader->packet_end_offset && seen < reader->info.packet_count) {
        uint32_t payload_size;
        uint32_t payload_crc;
        uint64_t next_offset;
        ...
        if (offset + HZV_PACKET_HEADER_SIZE > reader->packet_end_offset ||
            hz_seek(reader, offset) != LUAT_HZV_OK ||
            hz_read_exact(reader->fp, header, sizeof(header)) != LUAT_HZV_OK) {
            result = LUAT_HZV_ERR_IO;
            break;
        }
        if (memcmp(header, "HZPK", 4) != 0 || hz_le16(header + 4) != HZV_PACKET_HEADER_SIZE) {
            result = LUAT_HZV_ERR_FORMAT;
            break;
        }
        payload_size = hz_le32(header + 20);
        payload_crc = hz_le32(header + 24);
        next_offset = offset + HZV_PACKET_HEADER_SIZE +
                      (((uint64_t)payload_size + 3u) & ~(uint64_t)3u);
        if (payload_size == 0 || payload_size > HZV_MAX_PACKET_SIZE ||
            next_offset > reader->packet_end_offset || next_offset <= offset) {
            result = LUAT_HZV_ERR_FORMAT;
            break;
        }
        if (header[6] == LUAT_HZV_STREAM_AUDIO) {
            size_t required = used + payload_size;
            if (required < used) {
                result = LUAT_HZV_ERR_FORMAT;
                break;
            }
            if (required > capacity) {
                size_t next_capacity = capacity ? capacity : 4096u;
                uint8_t *next_buffer;
                ...
                next_buffer = (uint8_t *)HZ_MALLOC(next_capacity);
                if (next_buffer == NULL) {
                    result = LUAT_HZV_ERR_NOMEM;
                    break;
                }
                if (buffer != NULL) {
                    memcpy(next_buffer, buffer, used);
                    HZ_FREE(buffer);
                }
                buffer = next_buffer;
                capacity = next_capacity;
            }
            if (hz_read_exact(reader->fp, buffer + used, payload_size) != LUAT_HZV_OK) {
                result = LUAT_HZV_ERR_IO;
                break;
            }
            if ((reader->info.flags & LUAT_HZV_FLAG_PACKET_CRC32) != 0u &&
                hz_crc32(buffer + used, payload_size) != payload_crc) {
                result = LUAT_HZV_ERR_CRC;
                break;
            }
            used += payload_size;
        }
    ...
    (void)hz_seek(reader, reader->next_packet_offset);
    if (result != LUAT_HZV_OK || used == 0) {
        if (buffer != NULL) HZ_FREE(buffer);
        return result != LUAT_HZV_OK ? result : LUAT_HZV_ERR_FORMAT;
    }
    *data = buffer;
    *size = used;
    return LUAT_HZV_OK;
