...
int _lvgl_struct_anim_t_newindex(lua_State *L) {
    ...
    if (!strcmp("start_value", key)) {
        anim->start_value = luaL_optinteger(L, 3, 0);
    }
    else if (!strcmp("end_value", key)) {
        anim->end_value = luaL_optinteger(L, 3, 0);
    }
    else if (!strcmp("current_value", key)) {
        anim->current_value = luaL_optinteger(L, 3, 0);
    }
...
int _lvgl_struct_draw_rect_dsc_t_newindex(lua_State *L) {
    ...
    if (!strcmp("radius", key)) {
        ...
        rect_dsc->blend_mode = luaL_optinteger(L, 3, 0);
    }
    /*Background*/
