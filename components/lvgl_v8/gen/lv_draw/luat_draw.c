...
//  lv_draw_mask_res_t lv_draw_mask_apply_ids(lv_opa_t* mask_buf, lv_coord_t abs_x, lv_coord_t abs_y, lv_coord_t len, int16_t* ids, int16_t ids_count)
int luat_lv_draw_mask_apply_ids(lua_State *L) {
    LV_DEBUG("CALL lv_draw_mask_apply_ids");
    lv_opa_t* mask_buf = (lv_opa_t*)lua_touserdata(L, 1);
    lv_coord_t abs_x = (lv_coord_t)luaL_checknumber(L, 2);
    lv_coord_t abs_y = (lv_coord_t)luaL_checknumber(L, 3);
    lv_coord_t len = (lv_coord_t)luaL_checknumber(L, 4);
    int16_t* ids = (int16_t*)lua_touserdata(L, 5);
    int16_t ids_count = (int16_t)luaL_checkinteger(L, 6);
    lv_draw_mask_res_t ret;
    ret = lv_draw_mask_apply_ids(mask_buf ,abs_x ,abs_y ,len ,ids ,ids_count);
    lua_pushboolean(L, ret == 0 ? 1 : 0);
    lua_pushinteger(L, ret);
    return 2;
}
...
int luat_lv_draw_mask_free_param(lua_State *L) {
    LV_DEBUG("CALL lv_draw_mask_free_param");
...
//  bool lv_draw_mask_is_any(lv_area_t* a)
int luat_lv_draw_mask_is_any(lua_State *L) {
    LV_DEBUG("CALL lv_draw_mask_is_any");
    lv_area_t* a = (lv_area_t*)lua_touserdata(L, 1);
    bool ret;
    ret = lv_draw_mask_is_any(a);
...
//  void lv_draw_rect(lv_area_t* coords, lv_area_t* clip, lv_draw_rect_dsc_t* dsc)
int luat_lv_draw_rect(lua_State *L) {
    LV_DEBUG("CALL lv_draw_rect");
    lv_area_t* coords = (lv_area_t*)lua_touserdata(L, 1);
    lv_area_t* clip = (lv_area_t*)lua_touserdata(L, 2);
...
int luat_lv_draw_letter(lua_State *L) {
    ...
    lv_point_t* pos_p = (lv_point_t*)lua_touserdata(L, 1);
    ...
    lv_opa_t opa = (lv_opa_t)luaL_checknumber(L, 6);
...
int luat_lv_draw_arc_get_area(lua_State *L) {
    ...
    lv_coord_t x = (lv_coord_t)luaL_checknumber(L, 1);
    lv_coord_t y = (lv_coord_t)luaL_checknumber(L, 2);
    uint16_t radius = (uint16_t)luaL_checkinteger(L, 3);
    uint16_t start_angle = (uint16_t)luaL_checkinteger(L, 4);
    uint16_t end_angle = (uint16_t)luaL_checkinteger(L, 5);
    lv_coord_t w = (lv_coord_t)luaL_checknumber(L, 6);
    bool rounded = (bool)lua_toboolean(L, 7);
    lv_area_t* area = (lv_area_t*)lua_touserdata(L, 8);
