...
map_lv_ints = ["lv_arc_type_t", "lv_style_int_t", "lv_coord_t", "lv_spinner_dir_t", "lv_drag_dir_t",
                    ...
                    "lv_slider_mode_t", "lv_arc_mode_t", "lv_text_align_t", "lv_btnmatrix_ctrl_t", "lv_cover_res_t", "lv_event_code_t",
                    "lv_flex_flow_t", "lv_grid_align_t"]
    ...
    def visit_FuncDecl(self, node):
        try :
            ...
            if method_name.startswith("_"):
                return
            if not method_name.startswith(f"lv_{self.prefix}"):
                return
            # 一些回调方法, 这些没法自动生成
            if method_name.endswith("_cb") or method_name.endswith("cb_t") or method_name.endswith("_f_t"):
                ...
                return
            if method_name in ["lv_tlsf_walker", "lv_obj_remove_event_cb_with_user_data",
            "lv_win_add_btn"]:
                return
            ...
            if method_name in ["lv_btnmatrix_set_ctrl_map", "lv_keyboard_set_ctrl_map", "lv_calendar_set_highlighted_dates",
                               ...
                               "lv_animimg_set_src", "lv_msgbox_create", "lv_obj_set_grid_dsc_array", "lv_obj_set_grid_dsc_array", "lv_style_set_grid_row_dsc_array", "lv_style_set_grid_column_dsc_array", "lv_obj_set_style_grid_row_dsc_array", "lv_obj_set_style_grid_column_dsc_array", "lv_chart_set_ext_y_array", "lv_chart_set_ext_x_array"] :
                return
...
def handle_groups(group, path):
    for name in os.listdir(path) :
        ...
        if name in ["lv_obj_style_gen.h", "lv_async.h", "lv_fs.h", "lv_log.h", "lv_mem.h",
                    ...
                    "lv_task.h", "lv_debug.h", "lv_event.h", "lv_obj_class.h", "lv_obj_style.h", "lv_obj_tree.h", "lv_obj_draw.h", "lv_obj_scroll.h",
                    "lv_indev_scroll.h", "lv_font_loader.h", "lv_tlsf.h"]:
            continue
        if name.startswith("lv_draw_") :
            continue
        #try :
            #print(">>>>>>>>>>>>" + name)
        ast = parse_file(os.path.join(path, name), use_cpp=True, cpp_path='D:\\RDA8910_V31\\prebuilts\\win32\\gcc-arm-none-eabi\\bin\\arm-none-eabi-cpp.exe', cpp_args=['-nostdinc', '-E', '-Imock', '-ID:\\RDA8910_V31\\tools\\fake_libc_include', '-I.', '-I../../lua/include', '-I../../luat/include', '-D__attribute__(x)='])
        v = FuncDefVisitor("lv_" + group, name[3:-2])
        v.visit(ast)
        #except  Exception :
        #    print("error>>>>>>>>>>>>" + name)
            #traceback.print_exc()
...
def main():
    ...
    extras = ["src/extra/layouts", "src/extra/widgets", "src/extra/themes"]
    for d in extras:
        group = list(filter(None, d.split('/')))[-1]
        dds = [f"{d}/{dd}" for dd in os.listdir(d) if os.path.isdir(f"{d}/{dd}")]
        for dd in dds:
            handle_groups(group, dd)
