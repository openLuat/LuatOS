...
static int _raw_codec_encode(luat_audio_data_codec_t *codec,
                             const uint8_t *input, uint32_t input_size,
                             uint8_t *output, uint32_t *encoded_used_size,
                             uint32_t *encoded_output_size)
{
    if (!codec || !input || !output || !encoded_used_size || !encoded_output_size ||
        input_size > codec->opts->encode_max_output_len) {
