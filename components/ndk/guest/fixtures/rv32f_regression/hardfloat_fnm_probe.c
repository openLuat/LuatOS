...
static float fnmsub_candidate(float a, float b, float c) {
    return c - a * b;
}

__attribute__((noinline))
static float fnmadd_candidate(float a, float b, float c) {
    return -(a * b) - c;
}
...
int main(void) {
    ...
    exchange[0] = f32_bits(fnmsub_candidate(a, b, c));
    exchange[1] = f32_bits(fnmadd_candidate(a, b, c));
    SYSCON = 0x5555;
    return 0;
}
