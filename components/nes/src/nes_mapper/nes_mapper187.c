...
/* https://www.nesdev.org/wiki/INES_Mapper_187
 * Mapper 187 — Waixing pirate MMC3 variant (Security + extended CHR).
 * Like MMC3 but has a security register at $5000/$8000 that must return
 * specific values, plus extended outer bank bits.
 * Simplified: treat as standard MMC3 with security bypass.
 */
...
static void nes_mapper_init(nes_t* nes) {
    if (nes->nes_mapper.mapper_register == NULL) {
        nes->nes_mapper.mapper_register = nes_malloc(sizeof(mapper187_t));
    ...
    m->bank_values[6] = 0;
    m->bank_values[7] = 1;
    m->security = 0x83u;  /* standard security return value */
...
static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    ...
    case 0x8000: m->bank_select = data; mapper187_update_banks(nes); break;
    case 0x8001: {
...
static void nes_mapper_apu(nes_t* nes, uint16_t address, uint8_t data) {
    mapper187_t* m = (mapper187_t*)nes->nes_mapper.mapper_register;
    (void)address;
    /* Security register: sets which value is returned on read */
    m->security = data;
}
