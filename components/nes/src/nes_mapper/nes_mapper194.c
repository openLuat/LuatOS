...
/* https://www.nesdev.org/wiki/INES_Mapper_194
 * Mapper 194 — Waixing MMC3 variant (CHR-RAM at banks 2/3).
 * Like mapper 74 but CHR banks 2 and 3 use CHR-RAM.
 * Simplified: treat as standard MMC3 (CHR-RAM redirect accepted).
 */
...
static void mapper194_update_banks(nes_t* nes) {
    mapper194_t* m = (mapper194_t*)nes->nes_mapper.mapper_register;
...
static void nes_mapper_init(nes_t* nes) {
    if (nes->nes_mapper.mapper_register == NULL) {
        nes->nes_mapper.mapper_register = nes_malloc(sizeof(mapper194_t));
        if (nes->nes_mapper.mapper_register == NULL) return;
    }
    mapper194_t* m = (mapper194_t*)nes->nes_mapper.mapper_register;
    nes_memset(m, 0, sizeof(mapper194_t));
    ...
    mapper194_update_banks(nes);
}

static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    mapper194_t* m = (mapper194_t*)nes->nes_mapper.mapper_register;
    switch (address & 0xE001u) {
    case 0x8000: m->bank_select = data; mapper194_update_banks(nes); break;
    case 0x8001: {
