...
/*
 * https://www.nesdev.org/wiki/INES_Mapper_195
 * Waixing mapper 195 — MMC3 variant where CHR bank values 0, 1, 2, and 3
 * redirect to 1KB pages within an embedded 4KB CHR-RAM buffer instead
 * of CHR-ROM.  All other behaviour is identical to MMC3 (mapper 4).
 */

#define MAPPER195_CHR_RAM_SIZE 4096u
...
typedef struct {
    ...
    uint8_t chr_ram[MAPPER195_CHR_RAM_SIZE]; /* 4KB CHR-RAM for bank values 0-3 */
} nes_mapper195_t;
...
/*
 * Load a single 1KB CHR slot.  Bank values 0-3 map to pages 0-3 of the
 * embedded chr_ram; all other values use CHR-ROM.
 */
static void mapper195_load_chr1k(nes_t* nes, nes_mapper195_t* m, uint8_t slot, uint8_t bank) {
    if (bank <= 3u) {
...
static void mapper195_update_banks(nes_t* nes) {
    nes_mapper195_t* m = (nes_mapper195_t*)nes->nes_mapper.mapper_register;
    ...
    /* CHR banking with CHR-RAM override for banks 0-3 */
    if (chr_mode == 0u) {
        mapper195_load_chr1k(nes, m, 0, m->bank_values[0] & 0xFEu);
        mapper195_load_chr1k(nes, m, 1, m->bank_values[0] | 0x01u);
        mapper195_load_chr1k(nes, m, 2, m->bank_values[1] & 0xFEu);
        mapper195_load_chr1k(nes, m, 3, m->bank_values[1] | 0x01u);
        mapper195_load_chr1k(nes, m, 4, m->bank_values[2]);
        mapper195_load_chr1k(nes, m, 5, m->bank_values[3]);
        mapper195_load_chr1k(nes, m, 6, m->bank_values[4]);
        mapper195_load_chr1k(nes, m, 7, m->bank_values[5]);
    } else {
        mapper195_load_chr1k(nes, m, 0, m->bank_values[2]);
        mapper195_load_chr1k(nes, m, 1, m->bank_values[3]);
        mapper195_load_chr1k(nes, m, 2, m->bank_values[4]);
        mapper195_load_chr1k(nes, m, 3, m->bank_values[5]);
        mapper195_load_chr1k(nes, m, 4, m->bank_values[0] & 0xFEu);
        mapper195_load_chr1k(nes, m, 5, m->bank_values[0] | 0x01u);
        mapper195_load_chr1k(nes, m, 6, m->bank_values[1] & 0xFEu);
        mapper195_load_chr1k(nes, m, 7, m->bank_values[1] | 0x01u);
    }
}

static void nes_mapper_init(nes_t* nes) {
    if (nes->nes_mapper.mapper_register == NULL) {
        nes->nes_mapper.mapper_register = nes_malloc(sizeof(nes_mapper195_t));
        if (nes->nes_mapper.mapper_register == NULL) return;
    }
    nes_mapper195_t* m = (nes_mapper195_t*)nes->nes_mapper.mapper_register;
    nes_memset(m, 0, sizeof(nes_mapper195_t));
