...
/* https://www.nesdev.org/wiki/INES_Mapper_196
 * Mapper 196 — pirate MMC3 variant (Fire Emblem / Mario Bros pirate).
 * Like MMC3 but the PRG bank register is accessible at address $9000 (odd)
 * and the bank select register has bit 2 swapped with bit 0 for PRG selection.
 * Simplified: treat as standard MMC3.
 */
...
static void mapper196_update_banks(nes_t* nes) {
    mapper196_t* m = (mapper196_t*)nes->nes_mapper.mapper_register;
...
static void nes_mapper_init(nes_t* nes) {
    if (nes->nes_mapper.mapper_register == NULL) {
        nes->nes_mapper.mapper_register = nes_malloc(sizeof(mapper196_t));
        if (nes->nes_mapper.mapper_register == NULL) return;
    }
    mapper196_t* m = (mapper196_t*)nes->nes_mapper.mapper_register;
    nes_memset(m, 0, sizeof(mapper196_t));
    ...
    mapper196_update_banks(nes);
}

static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    mapper196_t* m = (mapper196_t*)nes->nes_mapper.mapper_register;
    /* Support scrambled PRG select at $9000 (odd) as well */
    uint16_t norm = address & 0xE001u;
    if ((address & 0xF001u) == 0x9001u) norm = 0x8001u;

    switch (norm) {
    case 0x8000: m->bank_select = data; mapper196_update_banks(nes); break;
    case 0x8001: {
