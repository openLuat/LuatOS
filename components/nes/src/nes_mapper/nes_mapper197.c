...
/* https://www.nesdev.org/wiki/INES_Mapper_197
 * Mapper 197 — pirate MMC3 variant (Super Fighter III pirate).
 * Like MMC3 but with CHR bank layout altered:
 * CHR banks 0-1 (2KB each) go to $1000-$17FF, banks 2-5 (1KB) go to $0000-$0FFF.
 * Simplified: treat as standard MMC3 (slight CHR layout deviation accepted).
 */
...
static void mapper197_update_banks(nes_t* nes) {
    mapper197_t* m = (mapper197_t*)nes->nes_mapper.mapper_register;
    ...
    /* Variant CHR layout: 2KB banks R0/R1 at $1000/$1800, 1KB banks R2-R5 at $0000-$0C00 */
    if (chr_mode == 0u) {
...
static void nes_mapper_init(nes_t* nes) {
    if (nes->nes_mapper.mapper_register == NULL) {
        nes->nes_mapper.mapper_register = nes_malloc(sizeof(mapper197_t));
        if (nes->nes_mapper.mapper_register == NULL) return;
    }
    mapper197_t* m = (mapper197_t*)nes->nes_mapper.mapper_register;
    nes_memset(m, 0, sizeof(mapper197_t));
    ...
    mapper197_update_banks(nes);
}

static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    mapper197_t* m = (mapper197_t*)nes->nes_mapper.mapper_register;
    switch (address & 0xE001u) {
    case 0x8000: m->bank_select = data; mapper197_update_banks(nes); break;
    case 0x8001: {
