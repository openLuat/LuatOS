...
/* https://www.nesdev.org/wiki/INES_Mapper_198
 * Mapper 198 — Waixing 6MB PRG (extended MMC3).
 * Like standard MMC3 but with extra PRG bank bits.
 * Bank values for PRG can be 8-bit (supports up to 256 × 8KB = 2MB).
 * On MCU targets, banks are truncated at 255 × 8KB anyway.
...
static void nes_mapper_init(nes_t* nes) {
    if (nes->nes_mapper.mapper_register == NULL) {
        nes->nes_mapper.mapper_register = nes_malloc(sizeof(mapper198_t));
...
static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    ...
    case 0x8000: m->bank_select = data; mapper198_update_banks(nes); break;
    case 0x8001: {
