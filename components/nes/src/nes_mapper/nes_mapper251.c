...
/* https://www.nesdev.org/wiki/INES_Mapper_251
 * Mapper 251 — Waixing MMC3 + outer bank (Shen Hua Jian Yun III).
 * Like mapper 45 (Star Prolog) but with simplified outer register scheme.
 * Outer bank controlled by SRAM writes.
 */
...
static void mapper251_update_banks(nes_t* nes) {
    mapper251_t* m = (mapper251_t*)nes->nes_mapper.mapper_register;
...
static void nes_mapper_init(nes_t* nes) {
    if (nes->nes_mapper.mapper_register == NULL) {
        nes->nes_mapper.mapper_register = nes_malloc(sizeof(mapper251_t));
        if (nes->nes_mapper.mapper_register == NULL) return;
    }
    mapper251_t* m = (mapper251_t*)nes->nes_mapper.mapper_register;
    nes_memset(m, 0, sizeof(mapper251_t));
    ...
    mapper251_update_banks(nes);
}

static void nes_mapper_write(nes_t* nes, uint16_t address, uint8_t data) {
    mapper251_t* m = (mapper251_t*)nes->nes_mapper.mapper_register;
    switch (address & 0xE001u) {
    case 0x8000: m->bank_select = data; mapper251_update_banks(nes); break;
    case 0x8001: {
...
static void nes_mapper_sram(nes_t* nes, uint16_t address, uint8_t data) {
    mapper251_t* m = (mapper251_t*)nes->nes_mapper.mapper_register;
    if (address == 0x6000u) { m->outer_prg = data & 0x1Fu; mapper251_update_banks(nes); }
    else if (address == 0x6001u) { m->outer_chr = data; mapper251_update_banks(nes); }
}
