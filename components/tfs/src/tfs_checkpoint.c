...
found:
    ...
    dev->checkpt_next_block = 0;   /* page index within current block */
    return checkpt_remember_block(dev, blk);
}

static int checkpt_alloc_chunk(tfs_dev_t *dev)
{
    ...
    return dev->checkpt_cur_block * cpb + dev->checkpt_next_block++;
}
