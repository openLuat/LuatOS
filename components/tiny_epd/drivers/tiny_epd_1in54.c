...
/* A 152x152 tri-colour full refresh is specified at about 16 seconds. */
#define TINY_EPD_1IN54R_BUSY_TIMEOUT_MS 30000u
...
/*
 * Waveshare 1.54-inch B V2: 200x200 B/W/R, SSD1681 family.
 *
 * It is deliberately a separate profile from waveshare_1in54_v2_bw.  Both
 * share their resolution, but B V2 has two controller RAMs and a different
 * border/update setup.  Its BUSY signal is high while the panel is working
 * and low when it is idle.
 */
static int epd_1in54b_v2_wait_idle(tiny_epd_t *epd)
{
    return tiny_epd_wait_busy(epd,
                              TINY_EPD_1IN54_BUSY_IDLE_LEVEL,
                              TINY_EPD_1IN54B_V2_BUSY_TIMEOUT_MS);
}

static int epd_1in54b_v2_init(tiny_epd_t *epd)
{
    static const tiny_epd_reset_step_t reset_steps[] = {
        {1, 200},
        {0, 5},
    ...
    static const uint8_t driver_output[] = {0xC7, 0x00, 0x01};
    static const uint8_t x_window[] = {0x00, 0x18};
    static const uint8_t y_window[] = {0xC7, 0x00, 0x00, 0x00};
    static const uint8_t y_cursor[] = {0xC7, 0x00};
    int ret;
    ...
    ret = epd_1in54b_v2_wait_idle(epd);
    if (ret != TINY_EPD_OK) {
        return ret;
    }
    ret = tiny_epd_write_cmd(epd, 0x12); /* SWRESET */
    if (ret != TINY_EPD_OK) {
        return ret;
    }
    ret = epd_1in54b_v2_wait_idle(epd);
    ...
    ret = epd_1in54_cmd_data1(epd, 0x11, 0x01); /* data entry mode */
    if (ret != TINY_EPD_OK) {
        return ret;
    }
    ret = epd_1in54_cmd_data(epd, 0x44, x_window, sizeof(x_window));
    ...
    ret = epd_1in54_cmd_data1(epd, 0x3C, 0x05); /* B V2 border waveform */
    if (ret != TINY_EPD_OK) {
        return ret;
    }
    ret = epd_1in54_cmd_data1(epd, 0x18, 0x80); /* internal temperature */
    if (ret != TINY_EPD_OK) {
        return ret;
    }
    ret = epd_1in54_cmd_data1(epd, 0x4E, 0x00);
    ...
    return epd_1in54b_v2_wait_idle(epd);
}

/*
 * Convert a canonical indexed B/W/R surface to one controller mask.
 * selected_bit is the electrical value used for a selected pigment: the
 * 1.54R profile uses active-low masks for both colours, while 1.54B V2 uses
 * active-low black RAM (0x24) and active-high red RAM (0x26).
 */
static int epd_1in54_write_color_mask(tiny_epd_t *epd,
                                      uint8_t command,
                                      ...
                                      uint8_t selected_bit)
{
    ...
    for (y = 0; y < tiny_epd_native_height(epd); y++) {
        for (byte_x = 0; byte_x < wire_stride; byte_x++) {
            uint8_t output = selected_bit ? 0x00u : 0xFFu;
            uint8_t bit;
...
static int epd_1in54b_v2_write_frame(tiny_epd_t *epd)
{
    int ret;

    /* 0x24: BLACK=0, WHITE/RED=1. */
    ret = epd_1in54_write_color_mask(epd, 0x24, TINY_EPD_COLOR_BLACK, 0u);
    if (ret != TINY_EPD_OK) {
        return ret;
    }
    /* 0x26: RED=1, WHITE/BLACK=0. */
    return epd_1in54_write_color_mask(epd, 0x26, TINY_EPD_COLOR_RED, 1u);
}

static int epd_1in54b_v2_refresh(tiny_epd_t *epd,
                                  tiny_epd_refresh_mode_t mode,
                                  const tiny_epd_rect_t *rect)
{
    int ret;
    ...
    /* The B V2 reference sequence exposes only a full tri-colour waveform. */
    if (mode != TINY_EPD_REFRESH_FULL || rect != NULL) {
        return TINY_EPD_ERR_UNSUPPORTED_MODE;
    }
    ret = epd_1in54b_v2_write_frame(epd);
    if (ret != TINY_EPD_OK) {
        return ret;
    }
    ret = epd_1in54_cmd_data1(epd, 0x22, 0xF7);
    ...
    /* Avoid an immediate idle-low sample before BUSY has asserted. */
    tiny_epd_delay_ms(epd, TINY_EPD_1IN54_BUSY_SETTLE_MS);
    return epd_1in54b_v2_wait_idle(epd);
}

static int epd_1in54b_v2_sleep(tiny_epd_t *epd, tiny_epd_sleep_mode_t mode)
{
    int ret;
...
{
    return tiny_epd_wait_busy(epd,
                              TINY_EPD_1IN54R_BUSY_IDLE_LEVEL,
                              TINY_EPD_1IN54R_BUSY_TIMEOUT_MS);
}
...
static int epd_1in54r_init(tiny_epd_t *epd)
{
    ...
    /*
     * This 152x152 B/W/R profile uses the OTP waveform path. In particular,
     * panel setting 0x0F selects OTP LUTs; it must not be followed by the
     * old 200x200 B-series register LUT sequence.
     */
    const uint8_t booster_soft_start[] = {0x17, 0x17, 0x17};
    const uint8_t panel_setting[] = {0x0F, 0x0D};
    ...
    return epd_1in54_cmd_data1(epd, 0x50, 0x77);
}
...
static int epd_1in54r_write_frame(tiny_epd_t *epd)
{
    ...
    ret = epd_1in54_write_color_mask(epd, 0x10, TINY_EPD_COLOR_BLACK, 0u);
    if (ret != TINY_EPD_OK) {
        return ret;
    }
    tiny_epd_delay_ms(epd, 2);
    ret = epd_1in54_write_color_mask(epd, 0x13, TINY_EPD_COLOR_RED, 0u);
    if (ret != TINY_EPD_OK) {
        return ret;
    }
    tiny_epd_delay_ms(epd, 2);
...
static const tiny_epd_palette_entry_t g_tiny_epd_bwr_palette[] = {
    {TINY_EPD_COLOR_WHITE, 0u, 0x00FFFFFFu},
    {TINY_EPD_COLOR_BLACK, 1u, 0x00000000u},
    {TINY_EPD_COLOR_RED,   2u, 0x00FF0000u}
};

static const tiny_epd_surface_desc_t g_tiny_epd_bwr_surface = {
    TINY_EPD_SURFACE_INDEX2,
    2,
    1,
    (uint16_t)(sizeof(g_tiny_epd_bwr_palette) / sizeof(g_tiny_epd_bwr_palette[0])),
...
static const tiny_epd_driver_t g_tiny_epd_1in54b_v2_driver = {
    "waveshare_1in54b_v2_bwr",
    TINY_EPD_1IN54_WIDTH,
    TINY_EPD_1IN54_HEIGHT,
    2,
    1,
