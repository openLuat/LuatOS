...
int luat_pack_utest(lua_State *L, const char *case_name) {
    if (!case_name || strcmp(case_name, "pack_unpack_basic") == 0) {
        return run_lua_bool_expr(
            L,
            "local b=pack.pack('>H',0x1234) "
            "local _,v=pack.unpack(b,'>H') "
            "return v==0x1234"
        ) ? 0 : -1;
    }
    return -1;
