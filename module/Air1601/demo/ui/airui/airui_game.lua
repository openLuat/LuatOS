...
-- 屏幕与棋盘参数（适配1024*600横屏）
local SCREEN_W, SCREEN_H = 1024, 600 -- 横屏：1024宽，600高
local GRID_W, GRID_H = 20, 15        -- 调整为20列15行以适应横屏
local CELL_SIZE = 28                 -- 每格像素大小（28px）
local BOARD_W = GRID_W * CELL_SIZE   -- 560
local BOARD_H = GRID_H * CELL_SIZE   -- 420
local BOARD_X = (SCREEN_W - BOARD_W) // 2 -- 水平居中：232
...
function airui_game.create_ui()
    ...
    -- 控制按钮区域（放在棋盘下方）
    local btnY = BOARD_Y + BOARD_H + 30
    local btnW, btnH, gap = 120, 45, 20
    ...
    -- 游戏说明（放在棋盘右侧）
    local infoPanel = airui.container({
        parent = main_container,
        x = BOARD_X + BOARD_W + 30,
        y = 160,
        w = 220,
...
-- 页面生命周期函数
----------------------------------------------------------------
function airui_game.init(params)
