...
local function init_fskv()
    ...
    if result then
        ...
        for key, default_value in pairs(DEFAULT_VALUES) do
            ...
            if value == nil then
                fskv.set(key, default_value)
                log.info("settings_config_app", "设置默认值", key, default_value)
            end
        end
...
local function get_config(key, default_value)
    if not fskv_initialized then
        ...
        if not init_fskv() then
            return default_value or DEFAULT_VALUES[key]
        end
    end
    
    local value = fskv.get(key)
    if value == nil then
        value = default_value or DEFAULT_VALUES[key]
    end
    return value
end
