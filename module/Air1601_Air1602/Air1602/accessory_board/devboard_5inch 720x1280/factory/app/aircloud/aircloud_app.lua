...
function excloud_task_func()
    ...
    while true do
        ...
        if not status.is_connected then
            ...
            -- 根据不同的 BSP 类型获取信号强度和设备标识
            if rtos.bsp() == "Air1601" then
                -- Air1601 是 MCU 模组，使用 airlink_mobile_info 获取 Air780EPM 的 mobile 信息
                local csq = airlink_mobile_info.get_csq()
                local imei = airlink_mobile_info.get_imei()
                local iccid = airlink_mobile_info.get_iccid()
                
                if csq and csq >= 0 then
                    table.insert(tlv_list, { field_meaning = excloud.FIELD_MEANINGS.SIGNAL_STRENGTH_4G, data_type = excloud.DATA_TYPES.INTEGER, value = csq })
                end
                if imei and imei ~= "unknown" then
                    table.insert(tlv_list, { field_meaning = excloud.FIELD_MEANINGS.IMEI, data_type = excloud.DATA_TYPES.ASCII, value = imei })
                end
                if iccid and iccid ~= "unknown" then
                    table.insert(tlv_list, { field_meaning = excloud.FIELD_MEANINGS.SIM_ICCID, data_type = excloud.DATA_TYPES.ASCII, value = iccid })
                end
            else
                -- 其他模组使用 mobile 模块
                table.insert(tlv_list, { field_meaning = excloud.FIELD_MEANINGS.SIGNAL_STRENGTH_4G, data_type = excloud.DATA_TYPES.INTEGER, value = mobile.csq() })
                table.insert(tlv_list, { field_meaning = excloud.FIELD_MEANINGS.SIM_ICCID, data_type = excloud.DATA_TYPES.ASCII, value = mobile.imei() })
            end
            
            table.insert(tlv_list, { field_meaning = excloud.FIELD_MEANINGS.TIMESTAMP, data_type = excloud.DATA_TYPES.INTEGER, value = os.time() })
