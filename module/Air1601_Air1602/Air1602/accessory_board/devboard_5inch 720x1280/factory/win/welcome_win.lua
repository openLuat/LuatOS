...
local function move_mouse_step(target_x, target_y)
    ...
    if dist <= MOUSE_SPEED then
        ...
        mouse_img:destroy()
        sys.timerStart(on_mouse_arrived, 500)
    else
        -- 继续移动
...
local function create_ui()
    ...
    search_box = airui.container({
        ...
        color = 0xFFFFFF, radius = 30
    })

    local icon_size = 32
    local icon_padding = 12
    local icon_x = box_w - icon_size - icon_padding
    local icon_y = (box_h - icon_size) / 2

    airui.label({
        parent = search_box, x = 20, y = 14,
        w = box_w - icon_size - icon_padding - 30, h = box_h -14,
        text = "合宙turnkey开发板", font_size = 24, color = 0x666666,
