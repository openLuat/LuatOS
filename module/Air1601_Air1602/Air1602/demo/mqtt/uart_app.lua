...
本文件的对外接口有两个：
1、sys.publish("SEND_DATA_REQ", "uart", mcu.unique_id().."/uart/up", read_buf, 1)
   通知mqtt client数据发送模块，在mcu.unique_id().."/uart/up"的topic上publish数据read_buf，不关心数据发送成功还是失败；
...
local function concat_timeout_func()
    ...
    if read_buf:len() > 0 then
        sys.publish("SEND_DATA_REQ", "uart", mcu.unique_id().."/uart/up", read_buf, 1)
        read_buf = ""
    end
end
