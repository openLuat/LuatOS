...
@summary AirCAMERA_1034 USB摄像头循环拍照上传合宙云平台应用模块
@version 1.1
@date    2026.08.19
@author  江访
@usage
本demo主要使用Air1601 + AirCAMERA_1034 USB摄像头完成以下功能：
1、初始化USB主机模式，连接USB摄像头（标准UVC接口）；
2、初始化excloud库并连接合宙iot.openluat.com云平台；
3、遍历USB摄像头支持的格式和分辨率，只选择MJPEG格式 + 1280x720；
4、每10秒触发一次拍照：启动stream → 接收一帧 → 停止stream；
5、将照片保存到/ram/photo.jpg；
6、通过excloud.upload_image将照片上传到合宙云平台。
7、全程不需要LCD屏幕，纯日志输出，节省内存。

与photo_to_aircloud.lua的区别：本模块不含任何LCD显示/airui.image代码。

使用前需要：
1、确保 main.lua 中 require "netdrv_device" 已打开（在 netdrv_device 内部选择实际使用的网卡）；
2、main.lua中打开本模块后，与 audio_record 等业务模块互斥。

本文件没有对外接口，直接在main.lua中require "photo_aircloud_nolcd"就可以加载运行。
]]
...
-- AirCAMERA_1034摄像头供电控制引脚，需要拉高使能
gpio.setup(58, 1, gpio.PULLUP)

-- 全局变量
local usb_app_id = nil                       -- USB摄像头应用ID
local frame_type = camera.FORMAT_MJPG        -- 使用MJPEG格式
local sensor_w = 1280                        -- 目标分辨率宽度（期望值，摄像头不支持时自动回退到最大可用分辨率）
local sensor_h = 720                         -- 目标分辨率高度（期望值，摄像头不支持时自动回退到最大可用分辨率）
...
-- 内存检查函数
-- 作用：定期监控系统内存使用情况（无LCD显示，照片只做上传，内存需重点关注）
...
local function usb_cb(usb_id, class, app_id, event, param1, param2, param3)
    if event == usb.EV_CONNECT then
        if class == usb.CAMERA then
            log.info("photo_aircloud_nolcd", "usb摄像头已连接，app id", app_id,
                "hub地址", param1, "端口", param2, "地址", param3)
        end
    end
    if event == usb.EV_DISCONNECT then
        if class == usb.CAMERA then
            log.info("photo_aircloud_nolcd", "usb摄像头已断开，app id", app_id,
                "hub地址", param1, "端口", param2, "地址", param3)
        end
    end
end
...
local function camera_cb(app_id, event, param)
    ...
    if event == usb.EV_NEW_RX then
        if not captured then
            ...
            log.info("photo_aircloud_nolcd", "接收到图像数据，长度", data_len)
            captured = true

            -- 立即停止摄像头流，避免后续帧继续涌入
            if usb_app_id then
                log.info("photo_aircloud_nolcd", "本轮拍照完成，停止摄像头流")
                camera.stop(camera.USB)
            end

            -- 发布拍照完成事件，由capture_handler_task处理保存/上传等耗时操作
    ...
    if event == usb.EV_CONNECT then
        ...
        -- 遍历USB摄像头支持的所有格式和分辨率（优先MJPEG，选择目标分辨率或最大可用分辨率）
        local res, format_num = camera.get_usb_config(usb_app_id, camera.CONF_UVC_FORMAT)
        log.info("photo_aircloud_nolcd", "总共有", format_num, "种数据流格式")

        local found = false
        local best_fmt, best_frm, best_w, best_h = nil, nil, 0, 0  -- 最大MJPEG分辨率候选（目标匹配不到时回退用）
        for format_index = 1, format_num, 1 do
            ...
            log.info("photo_aircloud_nolcd", "格式索引", format_index, "类型", type, "帧数", frame_num)
            ...
            if type == camera.FORMAT_MJPG then
                for frame_index = 1, frame_num, 1 do
                    ...
                    log.info("photo_aircloud_nolcd", "  分辨率", w, "x", h, "fps", fps)

                    -- 精确匹配目标分辨率
                    if w == sensor_w and h == sensor_h then
                        log.info("photo_aircloud_nolcd", "找到匹配分辨率", w, "x", h, "(MJPEG)")
                    ...
                    if (w * h) > (best_w * best_h) then
                        best_fmt, best_frm, best_w, best_h = format_index, frame_index, w, h
                    end
                end
                if found then break end
            else
                log.info("photo_aircloud_nolcd", "跳过非MJPEG格式")
            end
        end

        -- 目标分辨率摄像头不支持时，自动回退到最大可用的MJPEG分辨率，保证能出图
        if not found and best_fmt then
            log.warn("photo_aircloud_nolcd", "目标分辨率", sensor_w, "x", sensor_h,
                "摄像头不支持，自动改用最大分辨率", best_w, "x", best_h)
            camera.set_usb_config(usb_app_id, camera.CONF_UVC_RESOLUTION, best_fmt, best_frm)
        end
        if best_w == 0 then
            log.error("photo_aircloud_nolcd", "未找到可用的MJPEG分辨率，拍照功能不可用")
            return
        end

        -- 创建帧数据缓冲区，按实际选择的分辨率分配大小（宽×高，容纳MJPEG压缩帧留足余量）
        collectgarbage()
        collectgarbage()
        if not frame_buff then
            local buff_size = math.ceil(best_w * best_h)
            log.info("photo_aircloud_nolcd", "创建frame_buff，当前内存状态", rtos.meminfo("lua"))
            log.info("photo_aircloud_nolcd", "按分辨率自适应缓冲区大小", buff_size, "字节 (", best_w, "x", best_h, ")")
            frame_buff = zbuff.create(buff_size)
        end
        log.info("photo_aircloud_nolcd", "frame_buff就绪，当前内存状态", rtos.meminfo("lua"))
    ...
    -- USB摄像头断开事件
    if event == usb.EV_DISCONNECT then
        log.info("photo_aircloud_nolcd", "usb摄像头 app id", app_id, "已断开")
        usb_app_id = nil
        captured = false
        return
    end
    ...
    if event == usb.EV_RX_ERR then
        log.info("photo_aircloud_nolcd", "usb摄像头接收数据异常")
        return
    end
end

-- 拍照处理任务
-- 作用：循环监听 PHOTO_CAPTURED 事件，将收到的帧数据保存到文件、上传云平台（无LCD显示）
local function capture_handler_task()
    while true do
        local _, data_len = sys.waitUntil("PHOTO_CAPTURED")
        log.info("photo_aircloud_nolcd", "开始处理本轮拍照数据，长度", data_len)
        ...
        local save_ok, save_err = pcall(function()
            ...
            if file then
                ...
                file:close()
                log.info("photo_aircloud_nolcd", "照片已保存到", save_path, "大小", data_len)
            else
                error("无法打开文件")
            end
        end)
        ...
        if not save_ok then
            log.error("photo_aircloud_nolcd", "保存照片失败", save_err)
...
local function capture_trigger_task()
    ...
    log.info("photo_aircloud_nolcd", "摄像头已就绪，开始循环拍照，间隔", capture_interval_ms, "ms")

    while true do
        if usb_app_id then
            -- 重置标志位，准备本轮拍照
            captured = false
            log.info("photo_aircloud_nolcd", "触发新一轮拍照")
            -- 启动摄像头流，等待回调收到一帧后会自动停止
            camera.stream(camera.USB, usb_app_id)
            -- 等待本轮拍照处理完成（最多等30秒，防止上传慢卡死）
            local done = sys.waitUntil("PHOTO_DONE", 30000)
            if not done then
                -- 超时未收到帧：停止摄像头流，避免流残留导致下轮重复启动
                log.warn("photo_aircloud_nolcd", "本轮拍照超时未完成，停止摄像头流")
                camera.stop(camera.USB)
            end
        else
            log.warn("photo_aircloud_nolcd", "摄像头未连接，跳过本轮拍照")
        end
        -- 等待下一轮拍照间隔
...
local function camera_app_init()
    ...
    log.info("photo_aircloud_nolcd", "USB模式设置结果", mode_result)
    -- USB上电初始化开始工作
    pm.power(pm.USB, true)
    log.info("photo_aircloud_nolcd", "USB上电完成")

    log.info("photo_aircloud_nolcd", "初始化完成，摄像头连接后将每", capture_interval_ms, "ms循环拍照并上传")
end
