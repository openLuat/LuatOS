...
local function update_mobile_icon()
    ...
    -- 移动信号等级: -1表示无效（没联网）-> 6.png
    -- 1-5: 1->1.png, 2->2.png, 3->3.png, 4->4.png, 5->5.png（信号0）
    local img_index
    if level == -1 then
        img_index = 6
    else
        img_index = level  -- 1~5 直接对应图片编号
    end
    local img_name = string.format("4Gxinhao%d.png", img_index)
    mobile_img:set_src("/luadb/" .. img_name)
    log.info("idle_win", "update mobile icon", img_name, "level", level)
end
...
local function on_status_mobile_signal_updated()
    update_mobile_icon()
end
...
local function on_create()
    ...
    -- 优化 tabview 的翻页性能（如果有这些 API）
    if tabview.set_swipe_sensitivity then
        tabview.set_swipe_sensitivity(1.5) -- 提高翻页灵敏度
    end
    ...
    sys.subscribe("STATUS_MOBILE_SIGNAL_UPDATED", on_status_mobile_signal_updated)
