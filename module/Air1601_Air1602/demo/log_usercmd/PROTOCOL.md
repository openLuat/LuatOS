...
**读片长的设备端上限（当前）= 760B**：主干把 app 构建的上行改成了
`__LOG_PORT_UNDEPENDABLE__` 的 log record 通道（`platform_define.h:104` 无条件定义该宏），
**单条记录 `__LOG_ONE_RECORD_MAX_LEN__ = 1600` 字节**，超长响应会被 `_make_log_packet_to_log_record`
在 `pos >= 1595` 处 **break —— 静默截断，没有任何信号**：

```
最坏(每字节都转义)线上长 = 1(A5) + 2*(24帧头 + 7应用头 + n) + 4(CRC) + 1(A5) <= 1600  ->  n <= 766
实测(16K 文件): read_chunk = 512/768/1024/1400/1500 全部读全; >= 1600 一律只回 ~1545B
```

截断比"丢弃"更危险：短读被上位机当成正常**短读/EOF**，`read_file` 会静默返回残缺内容
（实测 4K 文件只回 1546 字节、`stat` 却说 4096）。所以主机侧加了两道校验，把截断变成**显式报错**：

- `_read_window`：回应的 `len` 字段若大于实际数据长度（`len(body) - 7 < rlen`）→ 报"响应被截断"
- `read_file`：读完再与 `stat` 的文件大小交叉校验 → 不符即报错

故 `read_chunk` 默认取 **760**（任意内容安全值）；确认内容低转义率（≤~11%）时可上调到 ~1500。
...
同步主干后（log record 路径）实测：`read_chunk=760` 时读 64K **267KB/s**、读 4K 209KB/s，
36 例真机用例全通过。
