...
- 转义最坏 1→2 字节：线上 ≤ 2×(36+476)+1 = 1025 ≤ 1056 ✓（长度预检不会因此丢帧）
...
设备端 ISR 用固定 **512B** 缓冲抽一帧（`Uart_NoBlockRxFrame`，见 §4），
**线上帧长超过它就会被拆到两次中断里，连发时丢帧**。而 usercmd 的 chunk=476 时线上长是 518B：
...
实测拐点精确落在 512B（chunk=470 线上 512 仍不稳、466 线上 508 干净）：
...
转义会把 0xA5/0xA6 各膨胀成 2 字节，所以**固定片长要么不安全要么浪费**：
按"整帧最坏 2 倍"保守估算只能取 219（内容全是 0xA5 时安全），代价是只剩 138KB/s。

因此 host 端按**实际内容的转义长度**逐片求解最大片长（`wire_budget` 默认 508B，
二分求解、以长度精确生成的帧为准，另留 4B 余量）：

- 随机/普通内容 → 片长取满 ~470B → **199KB/s**
- 内容全是 0xA5 → 片长自动收到 ~240B → 仍不超限、不丢帧
- **任何内容都不会超 512B**，故不会因内容触发丢帧
...
| 读窗口 W | 1 |
| 写窗口 W | 1 |
| 读片长 | **4096B**（host 侧参数；设备端响应上限见下） |
| 协商写片长 chunk | 476B（设备上界，实际片长由 `wire_budget` 按内容收缩） |
| wire_budget | 508B（单帧线上长上限；设备 ISR 抽帧缓冲 512B） |

**读片长的设备端上限曾是 32732B**（旧固件）：`soc_cmd_response` 在 cache 模式下按 `2*len` 估算
64KB log fifo 余量，且该分支**没有 `else`** —— 余量不足时整条响应被静默丢弃（不发也不报错）。

```
2*len <= 64K - 2*sizeof(tx_head)  ->  响应 len <= 32744  ->  read_chunk <= 32732
实测: read_chunk=16384 正常(432KB/s), 32768 稳定超时(两轮复现), 且设备仍健康
```

**2026-09-17 已修**：余量改按最坏转义重算，装不下时回退 `soc_log_to_device` 直发，
不再静默丢弃。故该上限只对旧固件成立。同时 `soc_rx` 改为一次中断内循环抽干，
下行单帧不再被拆到两次中断（修掉了 `chunk=476` 线上 518B 超过 512B 抽帧缓冲导致的丢帧）。
...
4096→16384 只多 11% 但余量从 8 倍掉到 2 倍，故默认取 4096。
...
| 写 64K 到 /ram/ | **~170KB/s** | 自适应分片 + W=1；分片过大时（chunk=476，线上 518B）掉到 13KB/s |
...
吞吐瓶颈不在协议开销，而在**单帧线上长度**：设备端 ISR 用固定 512B 缓冲抽帧，
超过它就会被拆到两次中断里并在连发时丢帧。修好 host 读串口 + 按内容收缩片长后，
单帧往返只有 1~2ms。

剩余空间：写方向要再提速，方向是**加大每帧载荷**（把设备 ISR 抽帧缓冲和 `rx_cache1` 一起放大，
或降低转义膨胀），而不是开大窗口（实测开大只会更快丢帧）。
附带发现两个设备端问题：`soc_cmd_response` 的静默丢包（见上），
以及 `Uart_NoBlockRxFrame` 的 512B 缓冲装不下一帧 chunk=476（线上 518B）。
