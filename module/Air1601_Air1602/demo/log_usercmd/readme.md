...
| `read_chunk` | 760B | 任意内容安全值：设备端单帧响应有 1600B log record 上限（见 PROTOCOL.md §6），超出会被截断；主机侧现在会**显式报错**而不是静默返回残缺内容。实测读 64K 267KB/s |
...
因此：`read_chunk` 默认降到 **760**（任意内容安全值），并由主机侧把两类异常都变成**显式报错**——
`_read_window` 校验"声明的 len 是否大于实到数据"，`read_file` 再与 `stat` 的大小交叉校验。
