...
本文件的对外接口有一个：
1、sys.publish("SEND_DATA_REQ", "timer", mcu.unique_id().."/timer/up", payload, 0, {func=send_data_cbfunc, para="timer"..payload})
...
-- para：回调参数，sys.publish("SEND_DATA_REQ", "timer", mcu.unique_id().."/timer/up", payload, 0, {func=send_data_cbfunc, para="timer"..payload})中携带的para
...
function send_data_req_timer_cbfunc()
    ...
    -- 携带的第二个参数mcu.unique_id().."/timer/up"为要publish的topic
    ...
    sys.publish("SEND_DATA_REQ", "timer", mcu.unique_id().."/timer/up", payload, 0, {func=send_data_cbfunc, para="timer"..payload})
    payload = payload+1
end
