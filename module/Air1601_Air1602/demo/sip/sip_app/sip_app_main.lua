...
local SIP_CONFIG = {
    ...
    sip_username = "8647930800115830",
    sip_password = "a.123456",
    sip_transport = exsip.TRANSPORT_UDP,
    
    remote_sip_uri = "sip:8647930800115831@180.152.6.34",
    auto_answer = true,
...
local function stop()
    ...
    exsip.stop()
    if exsip.is_voip_busy and exsip.is_voip_busy() then
        log.warn("stop", "等待 VoIP 停止完成，由 C 层释放音频")
        return
    end
    local ok, err = pcall(exaudio.pm, audio and audio.SHUTDOWN or 2)
    if not ok then
        log.warn("stop", "音频休眠失败", err)
    end
end
    ...
    elseif event == "voip" then
        ...
        if sub_event == "state" then
            ...
            if data == "starting" then
                sys.publish("SIP_APP_MAIN_VOIP_STARTING")
            elseif data == "started" then
                sys.publish("SIP_APP_MAIN_VOIP_STARTED")
            elseif data == "stopped" or data == "idle" then
