...
-- SIP disconnected 可能早于 C 层释放音频，挂断播报延后到 stopped。
local pending_hangup = false
local pending_hangup_reason
--前正在播放的 TTS 优先级
...
local function play_tts(text, priority)
    ...
    if voip_running or (exsip.is_voip_busy and exsip.is_voip_busy()) then
        log.info("tts_speaker", "VoIP 正在启动、通话或停止，跳过 TTS 播报")
        return false
    end
...
sys.subscribe("SIP_APP_MAIN_VOIP_STARTING", voip_start_tts)
