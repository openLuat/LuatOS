--[[
@module  netdrv_device
@summary 网络驱动设备功能模块(Air1601版本)
@version 1.0
@date    2026.03.16
@author  朱天华
@usage
本文件为网络驱动设备功能模块，核心业务逻辑为：根据项目需求，选择并且配置合适的网卡(网络适配器)
1、netdrv_airlink：socket.LWIP_USER0，通过airlink连接Air780EPM实现4G联网；
2、netdrv_eth_spi：socket.LWIP_ETH，通过SPI外挂CH390H芯片的以太网卡；
3、netdrv_multiple：可以配置多种网卡的优先级，按照优先级配置，使用其中一种网卡连接外网；
4、netdrv_pc：pc模拟器上的网卡

根据自己的项目需求，只需要require以上几种中的一种即可；
]]


-- 根据自己的项目需求，只需要require以下几种中的一种即可；

-- Air1601 MCU模式，通过airlink连接Air780EPM实现4G联网
-- 加载4G网络驱动
require "netdrv_4g"

-- 加载"通过SPI外挂CH390H芯片的以太网卡"驱动模块
-- 如果需要使用以太网，取消以下注释
-- require "netdrv_eth_spi"

-- 加载"可以配置优先级的多种网卡"驱动模块
-- 如果需要配置多网卡优先级，取消以下注释
-- require "netdrv_multiple"

-- 加载"pc模拟器网卡"驱动模块
-- require "netdrv_pc"
