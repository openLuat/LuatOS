...
local function parse_frames(s)
    ...
    while #rx_buf >= 6 do
        ...
        if pos > 1 then
            rx_buf = rx_buf:sub(pos)
            if #rx_buf < 6 then return end
        end
...
function exfacecam.open(param)
    ...
    if param.face_rst then
        ...
        sys.wait(100)  -- 拉低复位保持100ms
        gpio.setup(param.face_rst, 1)
        sys.wait(500)  -- 释放复位后等500ms模组启动
    end
    ...
    log.info(TAG, "准备初始化 UART" .. uart_id .. " @115200 8N1")
    uart.setup(uart_id, 115200, 8, uart.PAR_NONE, uart.STOP_1)
    log.info(TAG, "UART" .. uart_id .. " setup 完成")
    sys.wait(300)  -- 等UART建立稳定后再开始探测模组
    ...
    local function uart_recv_cb(_, len)
        ...
        if data and #data > 0 then
            local ok, err = pcall(parse_frames, data)
            if not ok then
                log.error(TAG, "parse err:", err)
                rx_buf = ""
            end
        end
    end
