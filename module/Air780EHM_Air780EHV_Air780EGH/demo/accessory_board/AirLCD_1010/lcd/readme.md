...
6. **camera_preview_page.lua** - 摄像头预览页面，支持预览→拍照→保存/删除

### 1.3 驱动模块
6. **lcd_drv.lua** - LCD显示驱动模块，基于lcd核心库，lcd_drv和exlcd_drv二选一使用

7. **exlcd_drv.lua** - LCD显示驱动模块，基于exlcd扩展库，lcd_drv和exlcd_drv二选一使用
8. **tp_drv.lua** - 触摸驱动模块，基于tp核心库，tp_drv和extp_drv二选一使用
9. **extp_drv.lua** - 触摸驱动模块，基于extp扩展库，tp_drv和extp_drv二选一使用
10. **customer_font_drv.lua** - 自定义外部字体驱动功能模块
11. **hzfont_drv.lua** - 合宙软件矢量字库（开发中）
...
- Air780EHM/Air780EHV/Air780EGH 核心板和 AirLCD_1010 配件板的硬件接线方式为
  ...
  - 为了演示方便，所以 Air780EHM/Air780EHV/Air780EGH 核心板上电后直接通过 VDD_EXT 引脚给 AirLCD_1010 配件板供电；
...
-- 加载字库驱动管理功能模块，有以下两种：
-- 1、使用1hzfont核心库驱动内核固件中支持的软件矢量字库的hzfont_drv.lua（正在开发中，后续补充）
-- 2、使用自定义字体
...
#### 7.4.1 主页面操作
1. 设备启动后显示主页面，包含三个功能按钮（摄像头预览、LCD演示、自定义字体）
2. 查看系统功能概览信息
3. 点击各功能按钮进入对应演示页面
...
#### 7.4.4 摄像头预览页面
1. 进入页面即开始摄像头预览，点击屏幕任意位置拍照
2. 拍照后进入照片显示页面，触摸保存/删除/返回按钮操作
3. 点击保存后文字变绿显示"已保存"
4. 点击删除后重新进入预览模式
...
- **摄像头预览页面**：预览画面正常显示，拍照响应及时，照片保存/删除功能正常
