...
说明：
成功执行extp_drv_init()之后，在触摸面板上有触摸动作时，在extp扩展库内，默认会发布以下消息：
...
local function extp_drv_init()
    ...
    if result==0 then
        log.error("extp_drv_init i2c.setup error")
        return false
    end
    ...
    result = extp.init({tp_model = "AirLCD_1010", i2c_id = 1, pin_rst = 0xFF, pin_int = pin_int, w = 320, h = 480})
