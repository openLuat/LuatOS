...
@author  拓毅恒
@usage
本模块演示在VoLTE通话中给对方播放音频文件、TTS文本或实时推送裸PCM流。
通话接通后在AUDIO_START/SPEECH_START事件中自动触发播放。

工作流程：
- 系统启动后初始化音频设备和CC
- 由宏IS_DIAL决定是主动拨号还是等待来电接听
- 通话建立（AUDIO_START）后，按 PLAY_SOURCE 配置播放音频
...
3. 播放流式：        cc.extern_source(true, true, audio_v2.DATA_CODEC_TYPE_RAW, true, 16000, 16, 1, true)
