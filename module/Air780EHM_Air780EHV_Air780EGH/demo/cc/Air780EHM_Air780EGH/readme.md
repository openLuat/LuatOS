...
- play_audio_during_cc.lua: 通话中播放外部音频（文件/TTS/流式）模块
...
### 场景四：通话中播放外部音频（play_audio_during_cc.lua）
...
3. **使用方式**：
   ...
   - 播放流式：PLAY_SOURCE = true
               PCM_STREAM_FILE = "/luadb/test.pcm"
...
3、根据需求配置演示功能：
   ...
   - 功能一（基础通话功能模块）：取消注释 `require "cc_app"`，注释掉其他三个
   - 功能二（TTS循环播放与通话处理功能模块）：取消注释 `require "cc_tts_app"`，注释掉其他三个
   - 功能三（通话录音功能模块）：取消注释 `require "cc_record_save"`，注释掉其他三个
   - 功能四（通话中播放外部音频功能模块）：取消注释 `require "play_audio_during_cc"`，注释掉其他三个
