--[[
@module  bl0939_app
@summary BL0939 双路免校准电能计量数据读取业务模块（适配 BL0939V1.4 板 + Air780EHV）
@version 1.6
@date    2026.08.21
@author  李明
@history
  1.6（2026.08.21）：增加 SPI 模式支持，MODE 变量可在 "uart"/"spi" 间切换（Air780EHV 用 SPI0：SCLK=PIN86/GPIO11、MOSI=PIN85/GPIO9、MISO=PIN84/GPIO10）
...
--   Air780EHV 3.3V                    -> P2-1 V   （光耦次级供电）
--   Air780EHV GND                      -> P2-4 G
--   Air780EHV UART1_TXD(GPIO18/PIN18)  -> P2-2 RXD
--   Air780EHV UART1_RXD(GPIO17/PIN17)  <- P2-3 TXD
--
-- SPI 模式接线（⚠️ 无光耦隔离，信号为热侧直连，只能冷态测试，禁止接市电！）：
--   1) BL0939 板由外部 3.3V 供电：P3-1(+3V3)、P3-2(GND)，SEL(U1-16) 拉高到 3.3V
--   2) Air780EHV 使用 SPI0（默认 SPI；EHV 因 ES8311 占用 I2C0，不支持 SPI1）：
--      SCLK=PIN86/GPIO11、MOSI=PIN85/GPIO9、MISO=PIN84/GPIO10；CS=PIN83/GPIO8 仅作占位，BL0939 无 CS 不接
...
-- SPI 模式专用：Air780EHV 使用 SPI0
local SPI_ID = 0
local SPI_CS = 8   -- PIN83/GPIO8 占位，不接 BL0939
