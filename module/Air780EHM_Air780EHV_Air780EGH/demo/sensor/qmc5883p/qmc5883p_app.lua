...
通过 MODE 变量选择通信模式（改 MODE 的值即可，无需注释/取消注释代码）：
  ...
  MODE = 2  硬件 I2C（指定 i2c_id，占用 CPU 少，Air780EHM 系列用 I2C1）
]]
...
local init_config
if MODE == 1 then
    -- 软件 I2C：SCL/SDA 用任意 GPIO，Air780EHM 接线示例用 GPIO31/GPIO30
    init_config = {scl = 31, sda = 30}
elseif MODE == 2 then
    -- 硬件 I2C：i2c_id 为总线号（Air780EHM 系列用 1），不传 scl/sda 时无总线恢复能力
