--[[
@module  airui_sleep
@summary AirUI 休眠管理示例，演示 AirUI + LCD + TP + 模组四级联动休眠与唤醒
@version 1.0
@date    2026.05.26
@author  江访
@usage
...
2、监测触摸空闲时间，超时后自动执行四级休眠：AirUI休眠 → LCD休眠 → TP休眠 → 模组休眠
3、唤醒顺序：先唤醒模组 → 再唤醒TP → 再唤醒LCD → 最后唤醒AirUI

休眠类型说明：
- AIRUI_SLEEP：仅 AirUI 休眠，禁用触摸，用户代码仍可运行
- AIRUI_SLEEP_TOUCH_WAKE：AirUI 休眠，触摸可唤醒（TP INT 引脚配置为中断）
- AIRUI_LCD_SLEEP：AirUI + LCD 休眠，禁用触摸
...
唤醒类型说明：
- TOUCH_WAKE：依赖 TP INT 引脚硬件中断唤醒模组（WAKEUP0 引脚），GT911 触摸时自动退出休眠
- TIMER_WAKE：通过定时器唤醒（低功耗模式下用 sys.timerStart，PSM+ 下用 pm.dtimerStart）
- KEY_WAKE：按键唤醒（PWR_KEY/BOOT 键），本模块内部注册 GPIO 中断并通过 KEY_EVENT 全局消息触发
- 按键作为通用唤醒源：无论 wake_type 配置为哪种类型，按键按下均会触发唤醒，独立于 wake_type 配置
- 禁用触摸：通过 airui.touch_unsubscribe() 取消 airui 层触摸订阅，airui.sleep() 同时停止渲染和输入处理

互斥关系说明：
- 触摸唤醒与 PSM+ 模式互斥（PSM+ 唤醒后重启）
- 触摸唤醒要求未禁用 airui 触摸功能
...
4、按键驱动已内置到本模块，airui_sleep.init() 内自动初始化 GPIO0 / gpio.PWR_KEY 中断
...
local SLEEP_TYPE_AIRUI_LCD = 3             -- AirUI + LCD 休眠
local SLEEP_TYPE_MODULE_LOWPOWER = 4       -- 模组常规低功耗模式
local SLEEP_TYPE_MODULE_PSM_PLUS = 5       -- 模组超低功耗模式（PSM+）
local SLEEP_TYPE_TP_ONLY = 6               -- 仅 TP 休眠

-- 唤醒类型常量
local WAKE_TYPE_TOUCH = 1                  -- 触摸唤醒
local WAKE_TYPE_TIMER = 2                  -- 定时唤醒
local WAKE_TYPE_KEY = 3                    -- 按键唤醒（PWR_KEY/BOOT）
local WAKE_TYPE_TOUCH_AND_TIMER = 4        -- 触摸 + 定时唤醒
local WAKE_TYPE_KEY_AND_TIMER = 5          -- 按键 + 定时唤醒
local WAKE_TYPE_NONE = 6                   -- 不禁用外部唤醒（依赖底层默认行为）

-- 默认配置参数
local DEFAULT_SLEEP_ENABLE = true                       -- 默认开启休眠
local DEFAULT_SLEEP_TIMEOUT = 30                        -- 默认30秒无操作自动休眠
local DEFAULT_SLEEP_TYPE = SLEEP_TYPE_AIRUI_TOUCH_WAKE  -- 默认 AirUI 休眠（触摸唤醒）
local DEFAULT_WAKE_TYPE = WAKE_TYPE_TOUCH               -- 默认触摸唤醒
local DEFAULT_TIMER_WAKE_SEC = 300                      -- 默认定时唤醒间隔300秒
local DEFAULT_LCD_SLEEP_ENABLE = false                  -- 默认不开启 LCD 单独休眠
...
local lcd_sleep_enable = DEFAULT_LCD_SLEEP_ENABLE

-- 运行时状态
local is_sleeping = false                   -- 是否处于休眠状态
local tp_is_sleeping = false                -- TP 是否已休眠
local tp_slept = false                      -- tp.sleep() 是否已调用（影响唤醒恢复策略）
local lcd_is_sleeping = false               -- LCD 是否已休眠
...
-- Air780E 硬件配置
local I2C_PORT = 1                          -- 硬件 I2C 端口号（与 tp_drv.lua 保持一致）

-- 按键定义（按键唤醒使用）
local KEY_BOOT = 0                          -- GPIO0 按键（BOOT 键）
...
-- 按键中断处理：PWR 键（gpio.PWR_KEY），双边沿触发
...
-- Air780EHM 上 TP INT 连接到 WAKEUP0 引脚（101号引脚）
local function setup_touch_wakeup()
    log.info("airui_sleep", "配置触摸唤醒，WAKEUP0 引脚")
    gpio.debounce(gpio.WAKEUP0, 50)
    gpio.setup(gpio.WAKEUP0, function(level, pin_id)
...
local function execute_sleep_sequence()
    ...
    local need_touch_wake = (wake_type == WAKE_TYPE_TOUCH or wake_type == WAKE_TYPE_TOUCH_AND_TIMER)
    ...
    -- 步骤2：LCD 休眠（配置开启时调用 lcd.sleep()）
    if lcd_sleep_enable and not lcd_is_sleeping then
        log.info("airui_sleep", "步骤2：LCD 休眠")
        lcd.sleep()
        lcd_is_sleeping = true
        log.info("airui_sleep", "LCD 休眠成功")
    elseif lcd_is_sleeping then
        log.info("airui_sleep", "步骤2：LCD 已在休眠状态，跳过")
    else
        log.info("airui_sleep", "步骤2：LCD 休眠未开启，跳过")
    end

    -- 步骤3：TP 触摸芯片休眠
    -- 重要：触摸唤醒模式下不能休眠 GT911，否则 INT 引脚状态异常导致误唤醒且 I2C 不可访问
    if need_touch_wake then
        log.info("airui_sleep", "步骤3：触摸唤醒已开启，跳过 TP 休眠")
        tp_slept = false
    elseif _G.tp_sleep_device and not tp_is_sleeping then
        log.info("airui_sleep", "步骤3：TP 触摸芯片休眠（GT911）")
        ...
        log.info("airui_sleep", "步骤3：无 TP 设备或已在休眠，跳过")
    end
    ...
    -- 硬件层：根据唤醒类型配置唤醒源
    if need_touch_wake then
        -- GT911 保持唤醒，INT 引脚正常，配置 WAKEUP0 下降沿中断检测触摸
        setup_touch_wakeup()
    end
    if wake_type == WAKE_TYPE_TIMER
        or wake_type == WAKE_TYPE_TOUCH_AND_TIMER
        or wake_type == WAKE_TYPE_KEY_AND_TIMER then
        setup_timer_wakeup()
    else
        disable_timer_wakeup()
    end
    -- 按键唤醒不需要额外配置硬件中断，key_drv 已注册 PWR_KEY/BOOT 中断
    -- 按下时会发布 KEY_EVENT 消息，本模块在 key_event_task 中转换为 AIRUI_SLEEP_WAKEUP

    -- 步骤5：模组进入低功耗模式
    if sleep_type == SLEEP_TYPE_MODULE_LOWPOWER then
        log.info("airui_sleep", "步骤5：模组进入常规低功耗模式（pm.WORK_MODE = 1）")
        pm.power(pm.WORK_MODE, 1)

    elseif sleep_type == SLEEP_TYPE_MODULE_PSM_PLUS then
        log.info("airui_sleep", "步骤5：模组进入超低功耗模式（pm.WORK_MODE = 3）")

        -- PSM+ 模式下触摸唤醒互斥，定时唤醒必须使用 pm.dtimerStart
        if wake_type == WAKE_TYPE_TIMER
            or wake_type == WAKE_TYPE_TOUCH_AND_TIMER
            or wake_type == WAKE_TYPE_KEY_AND_TIMER then
    ...
    elseif sleep_type == SLEEP_TYPE_TP_ONLY then
        log.info("airui_sleep", "步骤5：仅 TP 休眠，模组保持常规模式")

    else
        log.info("airui_sleep", "步骤5：仅 AirUI/LCD 休眠，模组保持常规模式")
    end

    log.info("airui_sleep", "========== 休眠流程完成 ==========")
end

-- 唤醒流程：先唤醒模组 → 再唤醒TP → 再唤醒LCD → 最后唤醒AirUI
-- 严格按照此顺序执行，确保各层级正确恢复
local function execute_wakeup_sequence(wake_source)
    ...
    -- 步骤2：恢复 TP
    if tp_slept then
        -- GT911 已通过 tp.sleep() 休眠，I2C 不可访问，tp_reinit 必定失败
        -- 保留 _G.tp_sleep_device 引用，下次休眠可再次调用 tp.sleep()
        log.info("airui_sleep", "步骤2：TP 已休眠，跳过恢复（触摸需重启后重新初始化）")
    else
        -- GT911 保持唤醒，先释放 WAKEUP0 中断，再恢复 TP 驱动
    ...
    -- 步骤3：唤醒 LCD
    if lcd_is_sleeping then
        log.info("airui_sleep", "步骤3：唤醒 LCD")
        lcd.wakeup()
        lcd_is_sleeping = false
        log.info("airui_sleep", "LCD 唤醒成功")
    else
        log.info("airui_sleep", "步骤3：LCD 未休眠，跳过")
    end

    -- 步骤4：唤醒 AirUI，恢复渲染
    log.info("airui_sleep", "步骤4：唤醒 AirUI")
...
function airui_sleep.init()
    ...
    log.info("airui_sleep", "LCD休眠:", lcd_sleep_enable)
    if wake_type == WAKE_TYPE_TIMER
        or wake_type == WAKE_TYPE_TOUCH_AND_TIMER
        or wake_type == WAKE_TYPE_KEY_AND_TIMER then
    ...
    -- 初始化按键 GPIO（BOOT/PWR_KEY），对外发布 KEY_EVENT 消息
    key_drv_init()
...
--   sleep_type: number 休眠类型（1-6）
--   wake_type: number 唤醒类型（1-6）
--   timer_wake_sec: number 定时唤醒间隔秒数
--   lcd_sleep: boolean LCD 休眠开关
function airui_sleep.update_config(config)
    ...
    if config.lcd_sleep ~= nil then
        lcd_sleep_enable = config.lcd_sleep
        log.info("airui_sleep", "LCD休眠更新为:", lcd_sleep_enable)
    end

    -- 互斥检查：PSM+ 模式与触摸唤醒互斥
    if sleep_type == SLEEP_TYPE_MODULE_PSM_PLUS
        and (wake_type == WAKE_TYPE_TOUCH or wake_type == WAKE_TYPE_TOUCH_AND_TIMER) then
        log.warn("airui_sleep", "PSM+ 模式与触摸唤醒互斥，自动切换为定时唤醒")
...
function airui_sleep.get_sleep_type_name(s_type)
    ...
    local names = {
        ...
        [SLEEP_TYPE_AIRUI_LCD] = "AirUI+LCD休眠",
...
function airui_sleep.get_wake_type_name(w_type)
    ...
    local names = {
        ...
        [WAKE_TYPE_KEY_AND_TIMER] = "按键+定时唤醒",
...
-- 按键事件任务：监听 KEY_EVENT 消息（由本模块的 handle_boot_key/handle_pwr_key 内部发布）
-- 按键作为通用唤醒源：只要处于休眠状态、按键按下（XXX_down）都会触发唤醒
-- 这样设计的原因：
-- 1、PWR_KEY/BOOT 是模组内置硬件按键，中断由模组底层管理，在低功耗模式下也能唤醒 MCU
-- 2、按键唤醒不依赖任何额外配置，应作为"逃生通道"，独立于 wake_type 配置
-- 3、wake_type 配置主要控制定时器和触摸 INT 引脚等额外唤醒源的开关
local function key_event_task()
    while true do
        local result, key_name = sys.waitUntil("KEY_EVENT")
        if result then
            -- 重置空闲计数，按键也算用户操作
            record_touch_event()
            -- 仅在按键"按下"（_down 后缀）且处于休眠状态时触发唤醒
            -- 弹起（_up）忽略，避免重复唤醒
            if is_sleeping and key_name and key_name:find("_down") then
                log.info("airui_sleep", "按键唤醒触发，按键:", key_name)
                sys.publish("AIRUI_SLEEP_WAKEUP", "key:" .. tostring(key_name))
            end
        end
    end
end

-- 注册休眠和唤醒消息订阅（占位订阅，确保消息不被丢弃）
...
local function create_sleep_ui()
    ...
    airui.switch({x=sw-60, y=y-2, w=50, h=28, checked=sleep_enable,
    ...
    local st_names = {"AirUI(禁用触摸)","AirUI(触摸唤醒)","AirUI+LCD","模组常规低功耗","模组超低功耗(PSM+)","仅TP休眠"}
    y = y + lh
    ...
    local wt_names = {"触摸唤醒","定时唤醒","按键唤醒","触摸+定时唤醒","按键+定时唤醒","无外部唤醒"}
    y = y + lh
    ...
    -- LCD 休眠开关
    y = y + dh + 8
    local lcd_label = lb(y, "LCD休眠: " .. (lcd_sleep_enable and "开" or "关"))
    airui.switch({x=sw-60, y=y-2, w=50, h=28, checked=lcd_sleep_enable,
        on_change = function(self)
            local v = self:get_state()
            lcd_sleep_enable = v
            lcd_label:set_text("LCD休眠: " .. (v and "开" or "关"))
            log.info("airui_sleep", "LCD休眠开关更新为:", v)
        end})
    ...
    airui.button({x=pad, y=y, w=sw-pad*2, h=bh, text="确 定 应 用",
        on_click = function(self)
            ...
            lcd_label:set_text("LCD休眠: " .. (lcd_sleep_enable and "开" or "关"))
    ...
    -- 底部注意事项（文本需简短，避免换行溢出）
    y = sh - 88
    airui.label({text="注意: 休眠保持VCC/RST高电平", x=pad, y=y, w=sw-pad*2, h=18, font_size=11})
    airui.label({text="PSM+下VDD-EXT勿用于外设供电", x=pad, y=y+18, w=sw-pad*2, h=18, font_size=11})
    airui.label({text="按键唤醒已内置(BOOT/PWR_KEY)", x=pad, y=y+36, w=sw-pad*2, h=18, font_size=11})
    airui.label({text="PSM+与触摸唤醒互斥", x=pad, y=y+54, w=sw-pad*2, h=18, font_size=11})
...
-- 6、触摸唤醒模式下不会调用 tp.sleep()，GT911 保持唤醒，INT 引脚用于硬件唤醒
-- 7、tp.sleep 后唤醒（非触摸唤醒），GT911 仍处于休眠 I2C 不可访问，触摸需重启后恢复
-- 8、PSM+ 唤醒后设备重启，pm.lastReson() 判断唤醒源
-- 9、功耗参考: 低功耗约40-70uA，PSM+约3-10uA
-- 10、lcd.sleep()/lcd.wakeup() 需配合 pin_pwr 使用，休眠前自动关闭背光，唤醒后自动打开背光
-- 11、按键驱动内置：airui_sleep.init() 自动初始化 GPIO0(BOOT) / gpio.PWR_KEY 中断，通过 KEY_EVENT 消息驱动唤醒
-- 12、PSM+ 模式与触摸唤醒互斥，update_config 会自动将互斥配置切换为定时唤醒
