...
5. **触摸屏中断引脚（INT）**
   ...
   - **情况一：需要触摸唤醒**（以 GT911 为例，触摸 IC 浅睡眠功耗约 3mA，支持触摸唤醒）
     ...
     ✅ AGPIO 支持在模组进入低功耗模式1下触发中断唤醒程序，普通 GPIO 不支持。但由于 AGPIO 驱动能力弱，屏幕 INT 引脚对外每 40ms 的波动无法抵消，容易出现触摸异常，甚至一直被此波动触发 INT 中断让主控去读取触摸 I2C 数据，以及休眠唤醒时 I2C 总线读写异常。
     ✅ 解决方案：使用 AGPIO 接 INT，增加 100kΩ 上拉电阻确保能抵消 AirLCD_1010 屏幕对外每 40ms 的电平波动，使 INT 不对 I2C 读写数据产生干扰。

   - **情况二：触摸完全休眠，使用按键唤醒**（功耗约 200μA）
     ✅ 可以直接使用普通 GPIO 接 INT。

   - **情况三：阶段性休眠**（30 秒黑屏，支持触摸唤醒 → XX 分钟进入深度休眠，仅支持按键唤醒）
     ✅ 使用 AGPIO 接 INT，增加 100kΩ 上拉电阻。
...
-- require("airui_sleep")  -- AirUI休眠/唤醒管理演示（内置按键驱动，按键唤醒开箱即用）
```
