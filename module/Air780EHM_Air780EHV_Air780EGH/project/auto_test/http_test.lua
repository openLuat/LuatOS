...
function http_test.run_tests()
    ...
    local failed_items = {method =nil ,description = nil,actual =nil}
    for i, test in ipairs(tests) do
        ...
        if len == tests_len then
            log.info("测试结束")
            if pass then
                test_mode = "all_test_succed"
                log.info(test_mode)
            else
                test_mode = "partial_failure"
                log.info("测试有项目未通过")
            end
            
            local data = {
                type = test_type, --http_test
                total_tests = tests_len, -- 总测试数
                passed_tests = pass_tests, -- 通过项
                test_mode = test_mode,
            ...
            table.insert(excloud_message.http_test,data)
            -- sys.wait(5000)
            break
        end
