...
--AirKEY_1000上8个按键对应的Air780EPM的GPIO ID
local KEY1_GPIO_ID = 26
local KEY2_GPIO_ID = 21
local KEY3_GPIO_ID = 24
local KEY4_GPIO_ID = 22
local KEY5_GPIO_ID = 23
local KEY6_GPIO_ID = 25
local KEY7_GPIO_ID = 28
local KEY8_GPIO_ID = 27


--按键1的中断处理函数
--int_level：number类型，表示触发中断后，某一时刻引脚的电平，1为高电平，0为低电平，并不一定是触发中断时的电平
--gpio_id：number类型，air_key.setup函数配置按键1时，对应的Air780EPM上的GPIO ID
...
--按键2的中断处理函数
--int_level：number类型，表示触发中断后，某一时刻引脚的电平，1为高电平，0为低电平，并不一定是触发中断时的电平
--gpio_id：number类型，air_key.setup函数配置按键2时，对应的Air780EPM上的GPIO ID
--在中断处理函数中，不要直接执行耗时较长的动作，例如写fskv，写文件，延时等
--可以publish消息给其他协程或者给订阅消息的处理函数去执行耗时动作
...
--按键3的中断处理函数
--int_level：number类型，表示触发中断后，某一时刻引脚的电平，1为高电平，0为低电平，并不一定是触发中断时的电平
--gpio_id：number类型，air_key.setup函数配置按键3时，对应的Air780EPM上的GPIO ID
--在中断处理函数中，不要直接执行耗时较长的动作，例如写fskv，写文件，延时等
--可以publish消息给其他协程或者给订阅消息的处理函数去执行耗时动作
...
--按键4的中断处理函数
--int_level：number类型，表示触发中断后，某一时刻引脚的电平，1为高电平，0为低电平，并不一定是触发中断时的电平
--gpio_id：number类型，air_key.setup函数配置按键4时，对应的Air780EPM上的GPIO ID
--在中断处理函数中，不要直接执行耗时较长的动作，例如写fskv，写文件，延时等
--可以publish消息给其他协程或者给订阅消息的处理函数去执行耗时动作
...
--按键5、6、7、8的中断处理函数
--int_level：number类型，表示触发中断后，某一时刻引脚的电平，1为高电平，0为低电平，并不一定是触发中断时的电平
--gpio_id：number类型，air_key.setup函数配置按键5、6、7、8时，对应的Air780EPM上的GPIO ID
--在中断处理函数中，不要直接执行耗时较长的动作，例如写fskv，写文件，延时等
--可以publish消息给其他协程或者给订阅消息的处理函数去执行耗时动作
...
--本demo中，Air780EPM核心板和AirKEY_1000配件板的接线方式如下
...
--分别和Air780EPM的KEY1_GPIO_ID、KEY2_GPIO_ID、KEY3_GPIO_ID、KEY4_GPIO_ID四个引脚相连
...
--分别和Air780EPM的KEY5_GPIO_ID、KEY6_GPIO_ID、KEY7_GPIO_ID、KEY8_GPIO_ID四个引脚相连
