...
1. **"KEY_EVENT"** - 按键事件消息，包含按键类型和状态
   ...
   - 按键功能定义：
     ...
     - 摄像头预览页面：预览完成后PWR键拍照，Boot键返回主页；照片显示页Boot键切换焦点，PWR键执行操作
...
#### 7.4.1 主页面操作
1. 设备启动后显示主页面，包含三个功能按钮（摄像头预览、LCD演示、自定义字体），作为按键页面切换演示
...
1. 按PWR键拍照
2. Boot键返回主页
3. 拍照后进入照片显示页面，Boot键切换焦点（保存/删除/返回），PWR键执行操作
4. 点击保存后文字变绿显示"已保存"
5. 点击删除后重新进入预览模式
...
- **摄像头预览页面**：预览画面正常显示，拍照响应及时，照片保存/删除功能正常，焦点切换流畅
