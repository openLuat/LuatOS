...
在低功耗模式下，UART1_TX 的电平会发生变化
1、如果需要保持 LPUART 的唤醒功能，那么就不能将 UART1 设置为 GPIO，也就无法消除 UART1_TX 的电平变化
2、如果可以接受关闭 LPUART 的唤醒功能，那么可以做如下两个动作：
    1) 软件上，执行下列代码：
        uart.close(1)
        pins.setup(16, "GPIO19") -- 把原有的uart改成其他功能, 或者gpio
        pins.setup(17, "GPIO18") -- 把原有的uart改成其他功能, 或者gpio
        gpio.close(19)           -- 关闭对应的gpio
        gpio.close(18)           -- 关闭对应的gpio
    2) 硬件上，进行如下操作：
...
local function set_lowpower_func_item()
    ...
    -- 根据自己的项目需求决定：进入低功耗模式前，来关闭AGPIO（GPIO24、GPIO25、GPIO26、GPIO27、GPIO28）控制的电路单元的功耗
