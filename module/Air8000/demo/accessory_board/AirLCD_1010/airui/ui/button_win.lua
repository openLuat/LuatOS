...
@author  江访
@usage
本文件展示按钮组件 V1.1.0 的新特性，采用exwin窗口管理扩展库。
支持样式表、动态样式、切换文本、点击计数等。
]]
...
local function on_destroy()
    ...
    win_id = nil
    log.info("button_win", "窗口销毁")
end

-- 窗口获得焦点回调
local function on_get_focus()
    log.info("button_win", "窗口获得焦点")
end

-- 窗口失去焦点回调
local function on_lose_focus()
    log.info("button_win", "窗口失去焦点")
end

-- 订阅打开按钮窗口消息
sys.subscribe("OPEN_BUTTON_WIN", function()
    if not exwin.is_active(win_id) then
        ...
        log.info("button_win", "窗口打开，ID:", win_id)
    end
end)