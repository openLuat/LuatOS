...
@author  江访
@usage
本文件展示图表组件 V1.1.0 的新特性，采用exwin窗口管理扩展库。
多系列、折线/柱状/堆叠、坐标轴自定义、动态推流、点击点回调，通过消息机制打开和关闭。
]]
...
local function chart_timer_cb()
    if not win_active or not chart then return end
...
local function create_ui()
    ...
    local init_data = {}
    for i = 1, 40 do
        table.insert(init_data, math.floor(50 + 30 * math.sin(i * 0.2) + 0.5))
    end
    chart:set_values(series_ids[1], init_data)

    local init_data2 = {}
    for i = 1, 40 do
        table.insert(init_data2, math.floor(40 + 20 * math.cos(i * 0.3) + 0.5))
    end
    chart:set_values(series_ids[2], init_data2)

    local init_data3 = {}
    for i = 1, 40 do
        table.insert(init_data3, math.floor(10 + 10 * math.sin(i * 0.5) + 0.5))
    end
...
local function on_destroy()
    ...
    win_id = nil
    log.info("chart_win", "图表窗口销毁")
end

-- 窗口获得焦点回调
local function on_get_focus()
    log.info("chart_win", "图表窗口获得焦点")
end

-- 窗口失去焦点回调
local function on_lose_focus()
    log.info("chart_win", "图表窗口失去焦点")
end

-- 订阅打开图表窗口消息
sys.subscribe("OPEN_CHART_WIN", function()
    if not exwin.is_active(win_id) then
        ...
        log.info("chart_win", "图表窗口打开，ID:", win_id)
    end
end)