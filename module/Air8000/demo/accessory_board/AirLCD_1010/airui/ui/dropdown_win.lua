...
@author  江访
@usage
本文件是下拉框组件的演示窗口，采用exwin窗口管理扩展库。
展示下拉框的各种用法，支持选项选择和联动。
]]
...
local function on_destroy()
    ...
    win_id = nil
    log.info("dropdown_win", "窗口销毁")
end

-- 窗口获得焦点回调
local function on_get_focus()
    log.info("dropdown_win", "窗口获得焦点")
end

-- 窗口失去焦点回调
local function on_lose_focus()
    log.info("dropdown_win", "窗口失去焦点")
end

-- 订阅打开下拉框窗口消息
sys.subscribe("OPEN_DROPDOWN_WIN", function()
    if not exwin.is_active(win_id) then
        ...
        log.info("dropdown_win", "窗口打开，ID:", win_id)
    end
end)