...
4、exwin.return_idle()：一键返回首窗口（ID=1），销毁其他所有窗口
]]
...
--[[
一键返回首窗口（win_id=1），销毁其他所有窗口，并保留第一个窗口
@api exwin.return_idle()
@return nil
@usage
-- 在任何窗口调用，都会回到首窗口并销毁中间窗口
exwin.return_idle()
]]
function exwin.return_idle()
    -- 从栈顶向下遍历，销毁除栈底（第一个窗口）外的所有窗口
    -- 栈底窗口的ID应为1（假设第一个打开的窗口是首窗口）
