...
@author  江访
@usage
本文件是俄罗斯方块游戏的演示窗口，采用exwin窗口管理扩展库。
通过消息机制打开和关闭。
]]
...
local function on_destroy()
    ...
    win_id = nil
    log.info("game_win", "游戏窗口销毁")
end

-- 窗口获得焦点回调
local function on_get_focus()
    log.info("game_win", "游戏窗口获得焦点")
end

-- 窗口失去焦点回调
local function on_lose_focus()
    log.info("game_win", "游戏窗口失去焦点")
end

-- 订阅打开游戏窗口消息
sys.subscribe("OPEN_GAME_WIN", function()
    if not exwin.is_active(win_id) then
        ...
        log.info("game_win", "游戏窗口打开，ID:", win_id)
    end
end)