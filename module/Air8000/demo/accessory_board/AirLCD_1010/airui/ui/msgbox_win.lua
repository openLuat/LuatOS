...
@date       2026.03.17
@author     江访
@usage      本文件是消息框组件的演示窗口，采用exwin窗口管理扩展库。
            展示消息框的各种用法，通过消息机制打开和关闭。
]]
...
local function on_destroy()
    ...
    win_id = nil
    log.info("msgbox_win", "窗口销毁")
end

-- 窗口获得焦点回调
local function on_get_focus()
    log.info("msgbox_win", "窗口获得焦点")
end

-- 窗口失去焦点回调
local function on_lose_focus()
    log.info("msgbox_win", "窗口失去焦点")
end

-- 订阅打开消息框窗口消息
sys.subscribe("OPEN_MSGBOX_WIN", function()
    if not exwin.is_active(win_id) then
        ...
        log.info("msgbox_win", "窗口打开，ID:", win_id)
    end
end)