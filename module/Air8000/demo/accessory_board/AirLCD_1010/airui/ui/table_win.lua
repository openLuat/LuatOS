...
@author  江访
@usage
本文件是表格组件的演示窗口，采用exwin窗口管理扩展库。
展示表格的各种用法，支持动态更新和样式调整。
]]
...
local function on_destroy()
    ...
    win_id = nil
    log.info("table_win", "窗口销毁")
end

-- 窗口获得焦点回调
local function on_get_focus()
    log.info("table_win", "窗口获得焦点")
end

-- 窗口失去焦点回调
local function on_lose_focus()
    log.info("table_win", "窗口失去焦点")
end

-- 订阅打开表格窗口消息
sys.subscribe("OPEN_TABLE_WIN", function()
    if not exwin.is_active(win_id) then
        ...
        log.info("table_win", "窗口打开，ID:", win_id)
    end
end)