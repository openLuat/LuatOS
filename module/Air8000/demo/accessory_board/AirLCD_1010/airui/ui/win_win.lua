...
@date       2026.03.18
@author     江访
@usage      本文件是窗口组件的演示窗口，采用exwin窗口管理，
            展示窗口的各种用法，支持动态样式切换。
]]
...
local function on_destroy()
    ...
    multi_switch = nil
    win_id = nil
    log.info("win_win", "窗口销毁")
end

-- 窗口获得焦点回调
local function on_get_focus()
    log.info("win_win", "窗口获得焦点")
end

-- 窗口失去焦点回调
local function on_lose_focus()
    log.info("win_win", "窗口失去焦点")
end

-- 订阅打开窗口窗口消息
sys.subscribe("OPEN_WIN_WIN", function()
    if not exwin.is_active(win_id) then
        ...
        log.info("win_win", "窗口打开，ID:", win_id)
    end
end)