...
-- local cspin_hw = gpio.setup(CS_PIN , 1)---硬件SPI
-- local cspin_soft = gpio.setup(Soft_CS_PIN , 1)
      

-- 1. 以对象方式设置并启用 SPI
local function spiDev_init_func()
    sys.wait(1000)
    log.info("AirSPINORFLASH_1000", "SPI_ID", SPI_ID, "CS_PIN", CS_PIN)    
    
    --以对象的方式初始化spi，高位在前，主模式，全双工模式
    local spiDevice = spi.deviceSetup(SPI_ID,CS_PIN, CPHA, CPOL, data_Width,bandrate,spi.MSB,1,0)  
