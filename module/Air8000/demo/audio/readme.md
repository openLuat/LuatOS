...
7、record_pcm_to_4017.lua：通过Air4017语音芯片录音与播放功能模块，演示PCM格式音频的流式录音与播放；
...
### 8、录音到文件功能 - 通过Air4017录音与播放（record_pcm_to_4017.lua）

- 通过UART1（波特率固定2M）连接合宙Air4017串口语音芯片，完成PCM流式录音与播放，Air4017无需I2C/PA/CODEC硬件初始化
...
├── record_pcm_to_4017.lua # 通过Air4017语音芯片的PCM流式录音与播放功能模块
...
1. 搭建好硬件环境（Air780EHM/Air780EGH核心板 + Air4017语音芯片，UART1对接）
2. 打开main.lua，确保保留`require "record_pcm_to_4017"`这一行
3. 将代码下载到开发板并运行
4. **演示效果**：通过Air4017进行PCM流式录音与播放，按Power键录音、按Boot键播放
...
I/user.air4017 初始化完成 1 2000000
I/user.exaudio air4017不支持调节麦克风音量
...
I/user.air4017 发送程序复位 02 05 00, 等待重新初始化后自动恢复上行
I/user.exaudio air4017录音已开始(MIC上行)
I/user.录音已开始，按任意键可提前结束
...
I/user.exaudio air4017流式播放已启动，等待play_stream_write喂数据
I/user.流式播放已开始
I/user.开始流式读取录音数据
...
**注意事项：**
- Air4017为串口语音芯片，仅需在`audio_setup_param`中配置`model="air4017"`和`uart_id`（默认UART1，波特率2M固定），无需I2C/PA/CODEC相关参数
- 录音MIC上行固定16kHz/16bit/单声道、512字节/帧；下行播放按320字节/10ms=32KB/s节奏喂数据，避免Air4017下行缓冲溢出丢帧
