...
@summary Air8000 通过 UART1 连接 Air4017 语音芯片的录音与播放演示（PCM 流式）
@version 1.0
@date    2026.09.02
@author  拓毅恒
@usage
...
功能说明：
  本 demo 演示 Air8000 通过 UART1（波特率固定 2M）连接合宙 Air4017 语音芯片，
...
播放：
  ...
  - 下行按 320B/10ms=32KB/s 节奏喂数据，防止 Air4017 下行缓冲溢出丢帧
...
硬件连接：
  - Air8000 的 UART1 对接 Air4017 串口，波特率 2M
  - 使用 SD 卡需按实际硬件配置 sd_spi_id / sd_cs_pin 并打开供电脚

工作流程：
  1. 初始化：挂载 SD 卡（失败则回退内存路径），exaudio.setup({model="air4017", uart_id=1}) 初始化 Air4017
...
local audio_setup_param = {
    model = "air4017",            -- Air4017 语音芯片: 通过 UART 串口驱动, 无需 I2C/PA/CODEC 硬件初始化
    uart_id = 1,                 -- 连接 Air4017 的 UART 端口(固定 2M 波特率), 默认 UART1
}
...
local function stream_audio_data()
    ...
    while is_playing do
        ...
        if audio_setup_param.model == "air4017" then
            -- air4017 下行按 320B/10ms=32KB/s 消费; 3200B 缓冲正好 100ms,
...
local audio_record_param = {
    ...
    path = function(buff, size)
        ...
        -- 避免每帧 open/write/close 拖慢导致 exaudio 写任务追不上 Air4017 上行速率、停止后还倒很久
