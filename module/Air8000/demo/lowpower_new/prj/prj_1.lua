...
1、插sim卡，上电开机之后，大约7秒后，成功进入低功耗状态；
   初始化阶段（常规模式），平均电流41mA；（此数据仅供参考，测试网络环境不同，持续时长以及平均电流都会不同，但是差异应该不是特别大才对）
   低功耗阶段（因为有网络寻呼，所以是低功耗模式+常规模式自动切换），平均电流2.2mA左右（1.7mA到2.3mA左右都属于正常值，和天线性能以及网络环境有关系，以自己的实际硬件+实际网络环境测试数据为准，但是差异应该不是特别大才对）

   2026.02.10：目前实际测试，在低功耗阶段，电流数据在2.2mA左右，待目前已知问题解决后，再进行一次测试
...
2、插sim卡，上电开机之后，立即进入飞行模式（打开drv_lowpower.lua中的mobile.flymode(0, true)代码），大约2.1秒后，成功进入低功耗状态；
   ...
   低功耗阶段（低功耗模式），平均电流973uA左右（850uA到950uA左右都属于正常值）

   2026.02.10：目前实际测试，在低功耗阶段，电流数据在973.2383uA左右，待目前已知问题解决后，再进行一次测试

3、不插sim卡，上电开机之后，大约2.7秒后，成功进入低功耗状态；
   初始化阶段（常规模式），平均电流14mA；（此数据仅供参考，测试网络环境不同，持续时长以及平均电流都会不同，但是差异应该不是特别大才对）
   低功耗阶段（低功耗模式），平均电流969uA左右（850uA到950uA左右都属于正常值）

   2026.02.10：目前实际测试，在低功耗阶段，电流数据在969.0119uA左右，待目前已知问题解决后，再进行一次测试
]]
