...
1、插sim卡，上电开机之后，大约6秒后，成功进入低功耗状态；
   ...
   低功耗阶段（因为有网络寻呼和tcp交互，所以是低功耗模式+常规模式自动切换），平均电流3.6mA左右（3.1mA到3.7mA左右都属于正常值，和天线性能以及网络环境有关系，以自己的实际硬件+实际网络环境测试数据为准，但是差异应该不是特别大才对）

   2026.02.10：目前实际测试，在低功耗阶段，电流数据在3.6mA左右，待目前已知问题解决后，再进行一次测试
