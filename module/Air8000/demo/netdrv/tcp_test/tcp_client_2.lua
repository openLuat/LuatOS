...
local tcp_client_receiver = require "tcp_client_r_2"
-- 加载tcp client socket数据发送功能模块
local tcp_client_sender = require "tcp_client_s_2"

-- 电脑访问：https://netlab.luatos.com/
-- 点击 打开TCP 按钮，会创建一个TCP server
-- 将server的地址和端口赋值给下面这两个变量
local SERVER_ADDR = "netlab.luatos.com"
...
local function set_tcp_client_params(adapter, server_addr, server_port)
    ...
    sys.taskInitEx(WIFI_SOCKET, TASK_NAME, tcp_client_main_cbfunc)
end
