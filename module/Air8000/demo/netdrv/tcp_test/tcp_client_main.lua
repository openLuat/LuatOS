...
local SERVER_ADDR = "192.168.4.100"
local SERVER_PORT = 2333

local NET_ADAPTER = socket.LWIP_ETH
...
while not socket.adapter(NET_ADAPTER) do
    log.warn("GP_SOCKET", "wait IP_READY", NET_ADAPTER)
...
log.info("GP_SOCKET", "recv IP_READY", NET_ADAPTER)
        ...
        -- 如果配置失败
        if not result then
            log.error("GP_SOCKET", "socket.config error")
            goto EXCEPTION_PROC
        end
        ...
        -- 如果连接server失败
        if not result then
            log.error("GP_SOCKET", "libnet.connect error")
            goto EXCEPTION_PROC
        end

        log.info("GP_SOCKET", "libnet.connect success")
        ...
        while true do
            ...
            if not tcp_client_receiver.proc(socket_client) then
                log.error("GP_SOCKET", "tcp_client_receiver.proc error")
                break
            end
            ...
            if not tcp_client_sender.proc(TASK_NAME, socket_client) then
                log.error("GP_SOCKET", "tcp_client_sender.proc error")
                break
            end
            ...
            log.info("GP_SOCKET", "libnet.wait", result, para1, para2)
			
			-- 如果连接异常，则退出循环
			if not result then
				log.warn("GP_SOCKET", "connection exception")
				break
            end
        end
...
local function set_tcp_client_params(adapter, server_addr, server_port)
    NET_ADAPTER = adapter or socket.LWIP_ETH
    SERVER_ADDR = server_addr or "192.168.4.100"
    SERVER_PORT = server_port or 2333
    log.info("tcp_client_main", "params", NET_ADAPTER, SERVER_ADDR, SERVER_PORT)
    --创建并且启动一个task
    --运行这个task的主函数GP_SOCKET
    sys.taskInitEx(GP_SOCKET, TASK_NAME, tcp_client_main_cbfunc)
end


return {
    set_tcp_client_params = set_tcp_client_params
}
