--[[
@module  bl0939_app
@summary BL0939 双路免校准电能计量数据读取业务模块（适配 BL0939V1.4 板 + Air8000）
@version 1.6
@date    2026.08.21
@author  李明
@history
  1.6（2026.08.21）：增加 SPI 模式支持，MODE 变量可在 "uart"/"spi" 间切换（Air8000 用 SPI1：SCLK=PIN38/MOSI=PIN40/MISO=PIN39）
  ...
  1.2（2026.08.21）：由 Air780EPM 工程移植至 Air8000，UART1 = MAIN_UART（PIN16/17），其余不变
]]
...
--   Air8000 3.3V              -> P2-1 V   （光耦次级供电）
--   Air8000 GND                -> P2-4 G
--   Air8000 MAIN_TXD(PIN16)    -> P2-2 RXD
--   Air8000 MAIN_RXD(PIN17)    <- P2-3 TXD
--
-- SPI 模式接线（⚠️ 无光耦隔离，信号为热侧直连，只能冷态测试，禁止接市电！）：
--   1) BL0939 板由外部 3.3V 供电：P3-1(+3V3)、P3-2(GND)，SEL(U1-16) 拉高到 3.3V
--   2) Air8000 使用 SPI1（默认通用 SPI；SPI0 被 WiFi/BLE 的 AirLink 占用，仅 D/DB/T 型号可用）：
--      SCLK=PIN38、MOSI=PIN40、MISO=PIN39；CS=PIN41(GPIO12) 仅作 spi.deviceSetup 占位，BL0939 无 CS 不接
...
-- UART 模式专用：器件地址（本板 SSOP20L；Air8000 工程按实际板卡配置，此处 UART_ADDR=5）
local UART_ADDR = 5

-- SPI 模式专用：Air8000 使用 SPI1
local SPI_ID = 1
local SPI_CS = 12   -- PIN41/GPIO12 占位，不接 BL0939
