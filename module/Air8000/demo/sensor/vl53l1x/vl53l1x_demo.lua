...
local function demo_standard_ranging()
    ...
    -- 软件 I2C 模式：SCL=GPIO1, SDA=GPIO2（Air8000 接线）
    local result = exs_vl53l1x.setup({scl = 1, sda = 2})
    if not result then
...
local function demo_range_mode()
    ...
    local ok = exs_vl53l1x.setup({scl = 1, sda = 2, range_mode = "short"})
    ...
    ok = exs_vl53l1x.setup({scl = 1, sda = 2, range_mode = "long"})
    if not ok then return end
    sys.wait(200)
