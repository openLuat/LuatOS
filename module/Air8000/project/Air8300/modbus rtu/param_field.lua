...
2、与从站 1 进行通信
    ...
    2. 对从站 1 进行 4 秒一次的写入保持寄存器 0-1 操作
...
local rs485_dir_gpio = 37 -- Air8000 开发板 RS485 方向引脚
...
local write_config = {
    ...
    -- 设置为 false 时，写单个线圈时功能码为 0x05，写单个保持寄存器时功能码为 0x06，写多个线圈时功能码为 0x0F，写多个保持寄存器时强制功能码为 0x10
    timeout = 1000 -- 超时时间 1000 ms
}
...
-- ==================== LED 控制（通信成功时闪烁）====================
-- LED GPIO定义: GPIO26, GPIO27, GPIO28
local leds = {26, 27, 28}
local comm_success = false  -- 通信成功标志

-- 初始化所有LED引脚为输出，默认熄灭
for i, pin in ipairs(leds) do
...
local function led()
    ...
    while true do
        if comm_success then
            ...
            -- 点亮当前LED
            gpio.set(leds[current], 1)
            
            -- 切换到下一个
            current = current + 1
            if current > #leds then
                current = 1
            end
...
local function read_slave1_holding_registers()
    ...
    elseif read_result.status == exmodbus.STATUS_TIMEOUT then
        ...
        comm_success = false  -- 通信超时，设置失败标志
    end
end

-- 写入从站 1 保持寄存器数据的函数
local function write_slave2_holding_registers()
    log.info("exmodbus_test", "开始写入从站 1 保持寄存器 0-1 的值，寄存器 0 数值为", slave2_data.data1, "，寄存器 1 数值为", slave2_data.data2)
    ...
    if write_result.status == exmodbus.STATUS_SUCCESS then
        log.info("exmodbus_test", "成功写入从站 1 保持寄存器 0-1 的值")
        comm_success = true  -- 写入成功，设置通信成功标志
    elseif write_result.status == exmodbus.STATUS_DATA_INVALID then
        log.info("exmodbus_test", "收到从站 1 的响应数据但数据损坏/校验失败")
        comm_success = false  -- 通信失败
    elseif write_result.status == exmodbus.STATUS_EXCEPTION then
        log.info("exmodbus_test", "收到从站 1 的 modbus 标准异常响应，异常码为", write_result.execption_code)
        comm_success = false  -- 通信失败
