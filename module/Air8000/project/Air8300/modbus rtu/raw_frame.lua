...
local function write_slave1_holding_registers()
    ...
    log.info("exmodbus_test", "开始写入从站 1 保持寄存器 0-1 的值")
    ...
    if write_result.status == exmodbus.STATUS_SUCCESS then
        ...
        -- 注意：这里放宽检查，只要 >= 8 字节即可，因为可能收到额外数据
        if #resp < 8 then
            log.info("exmodbus_test", "响应长度错误，期望至少 8 字节，实际:", #resp)
            comm_success = false  -- 通信失败
            return
        end
        ...
        log.info("exmodbus_test", "成功写入从站 1 保持寄存器 0-1")
