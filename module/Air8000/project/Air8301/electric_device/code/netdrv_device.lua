--[[
@module  netdrv_device
@summary 网络驱动设备功能模块
@version 1.0
@date    2025.07.24
@author  朱天华
@usage
本文件为网络驱动设备功能模块，核心业务逻辑为：根据项目需求，选择并且配置合适的网卡(网络适配器)
1、netdrv_4g：socket.LWIP_GP，4G网卡；
2、netdrv_eth_spi：socket.LWIP_USER1，通过SPI外挂CH390H芯片的以太网卡；
3、netdrv_multiple：可以配置多种网卡的优先级，按照优先级配置，使用其中一种网卡连接外网；
4、netdrv_pc：pc模拟器上的网卡

根据自己的项目需求，只需要require以上四种中的一种即可；


本文件没有对外接口，直接在main.lua中require "netdrv_device"就可以加载运行；
]]


-- 根据项目需求，本项目（Air8301）同时使用 4G 网卡与 WiFi 网卡（双网卡）
-- 网络优先级：WiFi > 4G（由 netdrv_wifi 通过 exnetif.set_priority_order 统一管理）
-- WiFi 可用时使用 WiFi，不可用时自动降级使用 4G
if rtos.bsp() == "PC" then
    -- 加载"pc模拟器网卡"驱动模块
    require "netdrv_pc"
elseif rtos.bsp() == "Air8101" then
    -- Air8101：加载"wifi"驱动网卡
    require "netdrv_wifi"
else
    -- 其他模组（Air8301 等）：同时加载 4G 网卡与 WiFi 网卡驱动
    require "netdrv_4g"
    require "netdrv_wifi"
end


-- 加载"通过SPI外挂CH390H芯片的以太网卡"驱动模块
-- require "netdrv_eth_spi"

-- 加载"可以配置优先级的多种网卡"驱动模块
-- require "netdrv_multiple"

-- 加载"pc模拟器网卡"驱动模块
-- require "netdrv_pc"
