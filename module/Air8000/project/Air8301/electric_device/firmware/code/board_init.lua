...
PC 模拟器兼容：
- 模拟器无 `pins` 核心库与真实 GPIO/PWM 硬件，本模块在 PC 上整体跳过（rtos.bsp() == "PC"），
  仅打印日志后返回，避免 VM 因 pins 为 nil 而异常退出。
]]
...
local function board_init_task()
    -- ⚠️ PC 模拟器兼容：模拟器没有 `pins` 核心库（引脚复用为固件+工具链能力），
    --    直接调用会抛 "attempt to index a nil value (global 'pins')"，
    --    且该异常会导致 Lua VM 退出并重启（整个脚本跑不起来），故整体跳过板级上电时序。
    if rtos.bsp() == "PC" then
        log.info("board_init", "PC 模拟器环境：跳过板级上电时序（pins/gpio/pwm 硬件初始化）")
        return
    end
