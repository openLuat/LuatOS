...
local function do_locate()
    -- pcall 包裹：airlbs 内部异常（如所依赖的 mobile/hmeta 接口缺失）不应中断定位任务
    local call_ok, ok, data = pcall(airlbs.request, {
        project_id  = PROJECT_ID,
        project_key = PROJECT_KEY,
        timeout     = LOC_TIMEOUT,
    })
    if not call_ok then
        log.warn("lbs_app", "定位调用异常:", tostring(ok))
        return false
    end
...
local function lbs_task()
    ...
    -- ⚠️ PC 模拟器兼容：模拟器无蜂窝网络/基站，且 airlbs 内部依赖 mobile.muid()（模拟器无该接口），
    --    调用会抛 "attempt to call a nil value (field 'muid')" 中断定位任务，故整体跳过定位。
    if rtos.bsp() == "PC" then
        log.info("lbs_app", "PC 模拟器环境：跳过基站定位（airlbs 不可用）")
        return
    end
