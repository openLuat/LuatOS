...
-- 因为mqtt client使用的是这个ID作为client id，如果不同PC上的id一样，模拟器在不同PC上同时运行时，mqtt client就会频繁出现被踢下线的问题
-- 目前模拟器上还没有合适的接口获取唯一ID，所以此处先简单的构造一个ID，需要手动保证ID唯一，在此处我简单使用了zhutianhua1作为ID
_G.mobile = {}
