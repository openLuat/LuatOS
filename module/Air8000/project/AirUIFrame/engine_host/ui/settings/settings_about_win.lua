...
local function create_info_row(parent, y, label_text, value_text)
    ...
    local row_height = is_kernel and 120 or 70
    local value_width = card_w - 150 - 30
    local value_height = is_kernel and 100 or 50
...
local function create_ui()
    ...
    unique_id_label = create_info_row(card_info, 70, "设备 ID", "--")
    unique_id_hex_label = create_info_row(card_info, 130, "设备 ID (HEX)", "--")
    version_label = create_info_row(card_info, 190, "软件版本", "--")
    kernel_label = create_info_row(card_info, 250, "内核版本", "--")
end
