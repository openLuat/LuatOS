...
-- 加载 TCP 从站应用模块（从站会先等待自己的网卡就绪，然后创建从站，成功后发布 TCP_SLAVE_READY 消息）
require "tcp_slave_manage"

-- 等待从站就绪，然后等待主站网卡就绪，最后加载主站代码
sys.taskInit(function()
    log.info("main", "等待从站就绪...")
    
    -- 等待从站创建完成的消息
    local result = sys.waitUntil("TCP_SLAVE_READY", 30000)
    if not result then
        log.error("main", "等待从站就绪超时")
        return
    end
    log.info("main", "从站已就绪，开始检查主站网卡")
    
    -- 等待主站网卡就绪
    log.info("main", "等待主站网卡就绪")
    while true do
        local is_ready, index = socket.adapter(socket.LWIP_ETH)
        if is_ready then
            log.info("main", "主站网卡已就绪，index:", index)
            break
        end
        log.info("main", "主站网卡未就绪，继续等待")
        sys.wait(1000)
    end
