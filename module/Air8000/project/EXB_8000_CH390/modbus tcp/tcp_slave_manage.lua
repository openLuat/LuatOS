...
local tcp_slave_config = {
    ...
    adapter = socket.LWIP_USER1, -- 网卡 ID：LwIP 协议栈的以太网卡
    port = 6000,               -- 本地端口号：6000（主站：服务器端口；从站：本地端口）
}

-- 等待从站网卡就绪后再创建从站
sys.taskInit(function()
    log.info("exmodbus_test", "等待从站网卡就绪...")
    
    -- 等待从站网卡就绪（LWIP_USER1）
    while true do
        local is_ready, index = socket.adapter(socket.LWIP_USER1)
        if is_ready then
            log.info("exmodbus_test", "从站网卡已就绪，index:", index)
            break
        end
        log.info("exmodbus_test", "从站网卡未就绪，继续等待...")
        sys.wait(1000)
    end
    ...
    if not tcp_slave then
        log.error("exmodbus_test", "tcp_slave 创建失败")
        return
    end
    ...
    log.info("exmodbus_test", "从站回调函数已注册，开始监听主站请求...")
    
    -- 发布从站就绪消息，通知 main 可以加载主站代码
    sys.publish("TCP_SLAVE_READY")
    log.info("exmodbus_test", "已发布 TCP_SLAVE_READY 消息")
end)
