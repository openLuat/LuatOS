-- home_win.lua - 首页（使用 exwin.is_active 判断活跃）
local home_win = {}
...
local function handle_sim_ind(status, value)
    ...
    -- 无论是否活跃，都尝试更新，但 update_signal 内部会判断
    update_signal()
end
...
local function create_ui()
    ...
    temp_label = airui.label({ parent = card_temp, x = 10, y = 45, w = 80, h = 35, text = "未接入传感器", font_size = 16, color = 0xFF0000, align =
    airui.TEXT_ALIGN_RIGHT })
    airui.label({ parent = card_temp, x = 100, y = 60, w = 30, h = 16, text = "℃", font_size = 16, color = 0x000000 })
    airui.label({ parent = card_temp, x = 10, y = 88, w = 130, h = 20, text = "当前温度", font_size = 16, color = 0x838383, align =
    airui.TEXT_ALIGN_CENTER })

    -- 湿度卡片
    airui.image({ parent = card_hum, x = 54, y = 8, w = 32, h = 32, src = "/luadb/shidu.png" })
    hum_label = airui.label({ parent = card_hum, x = 10, y = 45, w = 80, h = 35, text = "未接入传感器", font_size = 16, color = 0xFF0000, align =
    airui.TEXT_ALIGN_RIGHT })
    airui.label({ parent = card_hum, x = 100, y = 60, w = 30, h = 18, text = "%", font_size = 16, color = 0x000000 })
    airui.label({ parent = card_hum, x = 10, y = 88, w = 130, h = 20, text = "当前湿度", font_size = 16, color = 0x838383, align =
    airui.TEXT_ALIGN_CENTER })

    -- 空气质量卡片
    airui.image({ parent = card_air, x = 54, y = 8, w = 32, h = 32, src = "/luadb/kongqizhiliang.png" })
    air_label = airui.label({ parent = card_air, x = 10, y = 45, w = 80, h = 36, text = "未接入传感器", font_size = 16, color = 0xFF0000, align =
    airui.TEXT_ALIGN_RIGHT })
    airui.label({ parent = card_air, x = 95, y = 60, w = 40, h = 16, text = "ppb", font_size = 16, color = 0x000000 })
    airui.label({ parent = card_air, x = 10, y = 88, w = 130, h = 20, text = "空气质量", font_size = 16, color = 0x838383, align =
    airui.TEXT_ALIGN_CENTER })

    -- 标题
    airui.label({ parent = content, x = 0, y = 130, w = 240, h = 20, text = "扫码查看设备云端数据", font_size = 16, color = 0x3d3d3d, align =
    airui.TEXT_ALIGN_CENTER })
    airui.label({ parent = content, x = 240, y = 130, w = 240, h = 20, text = "扫码查看设备使用说明", font_size = 16, color = 0x3d3d3d, align =
    airui.TEXT_ALIGN_CENTER })

    -- 二维码
    qrcode1 = airui.qrcode({ parent = content, x = 80, y = 150, size = 90, data = aircloud_qr or "", dark_color = 0x000000, light_color = 0xFFFFFF, quiet_zone = true })
    airui.qrcode({ parent = content, x = 320, y = 150, size = 90, data =
    "https://docs.openluat.com/air8000/product/air8000a_turnkey_devboard/", dark_color = 0x000000, light_color = 0xFFFFFF, quiet_zone = true })
...
local function sensor_read_task()
    while true do
        ...
        if exwin.is_active(win_id) then
            if temp_label then
                if t then
                    temp_label:set_text(string.format("%.1f", t)); temp_label:set_color(0x000000); temp_label
                        :set_font_size(36)
...
-- 生命周期回调
function home_win.on_create(id)
    win_id = id
    create_ui()
...
function home_win.on_destroy(id)
    if time_timer then
...
function home_win.on_get_focus(id)
    -- 页面获得焦点时主动刷新一次
    update_time()
    update_signal()
end

function home_win.on_lose_focus(id)
    -- 无需操作
end

-- 订阅打开首页的消息
local function open_home_win_handler()
    exwin.open({
        on_create = home_win.on_create,
        on_destroy = home_win.on_destroy,
        on_lose_focus = home_win.on_lose_focus,
        on_get_focus = home_win.on_get_focus,
    })
end
sys.subscribe("OPEN_HOME_WIN", open_home_win_handler)
