...
function wifi_win.on_create(id)
    win_id = id
    create_ui()
    -- 可在此启动定时器、订阅等
end

function wifi_win.on_destroy(id)
    if main_container then main_container:destroy(); main_container = nil end
    -- 停止定时器、取消订阅等
end

function wifi_win.on_get_focus(id)
    -- 获得焦点时可刷新UI
    -- 例如更新列表
end

function wifi_win.on_lose_focus(id)
    -- 失去焦点时可暂停某些操作
end
...
local function open_wifi_win_handler()
    exwin.open({
        on_create = wifi_win.on_create,
        on_destroy = wifi_win.on_destroy,
        on_lose_focus = wifi_win.on_lose_focus,
        on_get_focus = wifi_win.on_get_focus,
    })
end
