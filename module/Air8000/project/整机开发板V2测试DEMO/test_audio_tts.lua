...
2. 演示如何播放TTS，注意此demo 不可以和audio_record,audio_play  demo同时使用
3. 使用了如下的IO口
...
4. 本文处理逻辑为，烧录qianzw.txt文件，通过tts 转化为音频，并通过扬声器逐个播报出来
]]
...
local function audio_tts_event(id, event)
...
local function audio_tts_task()
    local result    
    sys.taskInit(audio_setup)   -- 外设初始化
    audio.on(0,audio_tts_event)   --注册audio播放事件回调
    ...
    while true do
        ...
        line = nil
        if line == nil then
            log.info("读取文件失败，请查看是否烧录qianzw.txt")
        end
        line = line = "一二三四五六七八九十一二三四五六七八九十一二三四五六七八九十一二三四五六七八九十一二三四五六七八九十"
        log.info("播放内容", line)
...
sysplus.taskInitEx(audio_tts_task, taskName)  