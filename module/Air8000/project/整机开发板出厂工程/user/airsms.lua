...
function start_send_sms()                                       --创建“发短信”功能函数
     sms.send("13316920513", "102")                                   --发送短信到10001，内容102
end
...
function airsms.run()       
    ...
    while true do
        ...
        lcd.drawStr(0,95,"需要代码内修改发送短信的号码" )
        lcd.drawStr(0,100,showdata)
        -- 此处可填写短信demo ui
        --]
...
function read_msg_table()                                                      --创建读取文件中短信信息的功能函数
    ...
    return num_new,txt_new                                                     --该函数返回最近一次来信号码和来信信息。
end 
sys.taskInit(read_msg_table)                                                   --执行read_msg_table功能函数，进入waitUntil状态等待信息
sys.taskInit(read_msg)                                                         --执行read_msg功能函数，进入waitUntil状态等待信息
