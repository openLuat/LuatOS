...
sys.subscribe("WLAN_STA_INC", function(evt, data)
    if evt == "CONNECTED" then
        ...
        sysplus.taskInitEx(start_muti_network,"start_muti_network")
    elseif evt == "DISCONNECTED" then
...
function multi_network.run()       
    log.info("multi_network.run")
...
function multi_network.start_sta() 
    start_sta()
end

function multi_network.tp_handal(x,y,event)       -- 判断是否需要停止播放
