...
本文件为airlbs“多wifi”两种应用场景的定位功能演示，核心业务逻辑为：
1. 等待网络就绪 
2. 发起NTP授时 
3. 循环定位请求 
...
local timeout = 10 -- 扫描wifi 做 wifi定位 的超时时间，最小5S,最大60S
...
--多wifi定位
local function airlbs_wifi_task_func()
    local mac1 = netdrv.mac(socket.LWIP_STA)
    log.info("airlbs_multi_cells_wifi_func", "WIFI MAC address:", mac1)
    ...
    while 1 do
        ...
        if result then
            ...
            log.info("多wifi定位返回的经纬度数据为", data_str)-- 解析经纬度
        ...
        sys.wait(60000) 
        -- 循环60S一次wifi定位，此处设置定位请求间隔为60S非必须，请求频率可根据自己所购买的套餐进行计算，超过频次的定位请求均会返回定位失败。返回错误码，不包含位置信息。
    end
...
-- 多wifi定位
sys.taskInit(airlbs_wifi_task_func)
