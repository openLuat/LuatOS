...
录音到文件演示程序，按键功能：
1. GPIO8：开始/停止录音，停止播放
   - 空闲时按GPIO8开始5秒录音
   - 录音中按GPIO8提前结束录音
   - 播放中按GPIO8停止播放
2. GPIO9：开始/停止播放，停止录音
   - 空闲时按GPIO9播放录音文件
   - 播放中按GPIO9停止播放
   - 录音中按GPIO9提前结束录音
...
播放逻辑：
  使用流式播放方式播放PCM格式录音文件
  演示使用16kHz采样率、16位采样深度
  注意：播放采样位深仅支持到24位，如果录制32位录音则无法播放，需要用电脑进行播放！！！

工作流程：
1. 初始化：设置音频硬件参数
2. 录音：流式录音，实时写入文件
3. 播放：流式播放，按下GPIO9按键读取文件并播放
4. 状态管理：互斥控制录音/播放状态

注意事项：
1. Air8101开发板暂不支持录音到文件，如需录制到文件请使用Air8101核心板+AirMICROSD_1010配件板
]]
...
local audio_setup_param = {
    ...
    pa_ctrl = 28,              -- 音频放大器控制引脚
    pa_on_level = 0,           -- PA打开电平
    pa_delay = 10,             -- PA延时
    dac_ch = 0,                -- DAC通道号
    
    i2s_sample = 16000,        -- 采样率
    bits_per_sample = 16,      -- 录音位深
    i2s_framebit = 16          -- 通道位宽
}
...
local function mount_sd_card()
    ...
    -- 供电控制 (Air8101专用)
    -- gpio13为8101 TF卡的供电控制引脚，在挂载前需要设置为高电平，不能省略
    gpio.setup(13, 1)
...
local function main_audio_task()
    ...
    if not mount_sd_card() then
        log.error("TF卡挂载失败，录音文件将保存到默认路径")
        -- 如果TF卡挂载失败，使用默认路径
    ...
    if exaudio.setup(audio_setup_param) then
        ...
        log.info("按键功能说明：")
        log.info("1. GPIO8: 开始/停止录音，停止播放")
        log.info("2. GPIO9: 开始/停止播放，停止录音")
        log.info("3. 录音时长: ", RECORD_DURATION, "秒，可提前结束")
        log.info("4. 录音文件保存到:", recordPath)
    else
        log.error("音频硬件初始化失败")
    end
end
...
-- 设置GPIO8（开始/停止录音）
gpio.setup(8, powerkey_handler, gpio.PULLUP, gpio.FALLING)
gpio.debounce(8, 200, 1)

-- 设置GPIO9（播放录音文件）
gpio.setup(9, boot_key_handler, gpio.PULLUP, gpio.FALLING)
