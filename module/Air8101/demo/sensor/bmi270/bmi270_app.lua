...
通过 MODE 变量选择通信模式（改 MODE 的值即可，无需注释/取消注释代码）：
  ...
  MODE = 2  硬件 I2C（指定 i2c_id，占用 CPU 少，Air8101 用 I2C0）
  MODE = 3  SPI（高速模式，需指定 spi_id + cs，Air8101 用 SPI1，适合高速采样）
...
-- 注意：Air8101 的 GPIO10/GPIO11 被 DBG_UART0（下载调试）占用，不可用于外部中断；
--       SPI1 模式时 CS 占用 GPIO3，INT1 请保持使用 GPIO6 避免冲突
local INT1_GPIO = 6
...
elseif MODE == 3 then
    ...
    init_mode = "SPI"
    init_config = {spi_id = 1, cs = 3, speed = 1000000, acc_range = "4g", gyro_range = "500", acc_odr = 200, gyro_odr = 200}
else
