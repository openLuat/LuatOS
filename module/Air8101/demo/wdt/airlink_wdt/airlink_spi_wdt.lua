...
-- AirLink SPI 配置
local AIRLINK_SPI_ID = 0    -- SPI ID
local AIRLINK_SPI_CS_PIN = 15    -- CS引脚gpio号
local AIRLINK_SPI_RDY_PIN = 48    -- RDY引脚gpio号
...
local function init_airlink_wdt()
    ...
    -- 初始化看门狗，TO_RESET 使用 GPIO6
    local success = exairlinkwdt.open({ reset_pin = 6 , reset_idle_level = 1 })
    if success then
