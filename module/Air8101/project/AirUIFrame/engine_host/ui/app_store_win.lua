...
local function show_progress_dialog(app_name)
    ...
    local mask = airui.container({
        ...
        color = 0x000000, color_opacity = 180,
    })
    ...
    local dialog = airui.container({
        ...
        color = COLOR_CARD, radius = 16, border_width = 1, border_color = COLOR_DIVIDER
    })
    airui.label({
        parent = dialog, x = 20, y = 20, w = dialog_w - 40, h = 30,
        text = "正在安装 " .. (app_name or ""),
        font_size = title_font_size, color = COLOR_TEXT, align = airui.TEXT_ALIGN_CENTER
    })
    local bar = airui.bar({
        parent = dialog, x = 20, y = 70, w = dialog_w - 40, h = 20,
        min = 0, max = 100, value = 0,
        bg_color = COLOR_DIVIDER, indicator_color = COLOR_PRIMARY, radius = 10
    })
    local text_label = airui.label({
        parent = dialog, x = 20, y = 105, w = dialog_w - 40, h = 24,
        text = "准备下载...",
        font_size = desc_font_size, color = COLOR_TEXT_SECONDARY, align = airui.TEXT_ALIGN_CENTER
    })
...
local function show_success_toast(action, app_name)
    local msg = ""
    if action == "install" then msg = (app_name or "应用") .. " 安装完成"
    elseif action == "uninstall" then msg = (app_name or "应用") .. " 卸载完成"
    elseif action == "update" then msg = (app_name or "应用") .. " 更新完成"
    else msg = "操作完成" end
    local toast = airui.msgbox({
        title = "提示", text = msg, buttons = { "确定" }, timeout = 1000,
...
local function create_ui()
    ...
    airui.button({
        parent = top_bar, x = 8, y = top_item_y, w = top_item_h, h = top_item_h,
        text = "←",
        ...
        on_click = function()
            if win_id then exapp.init(); exwin.return_idle() end
        end
    })
    ...
    local search_bg = airui.container({
        parent = top_bar, x = top_item_h + 12, y = top_item_y, w = search_bg_w_eff, h = search_bg_h_eff,
        color = 0xEEEEEE, radius = top_item_r, border_width = 1, border_color = COLOR_DIVIDER
    })
    ...
    search_input = airui.textarea({
        parent = search_bg, x = 8, y = 2, w = search_bg_w_eff - 16, h = search_bg_h_eff - 4,
        placeholder = "搜索应用...", font_size = button_font_size, color = COLOR_TEXT, keyboard = search_keyboard
    })
    local search_btn_w = math.min(60, screen_w - top_item_h - 12 - search_bg_w_eff - 8)
    airui.button({
        parent = top_bar, x = top_item_h + 12 + search_bg_w_eff + 4, y = top_item_y, w = search_btn_w, h = top_item_h,
        text = "搜索", font_size = button_font_size,
    ...
    sort_select = airui.dropdown({
        parent = sort_bar, x = sort_label_w + 12, y = sort_btn_y, w = sort_dd_w, h = sort_btn_h,
    ...
    airui.button({
        parent = sort_bar, x = math.max(sort_refresh_x, screen_w - 78), y = sort_btn_y,
        w = math.min(70, screen_w - math.max(sort_refresh_x, screen_w - 78) - 4), h = sort_btn_h,
        text = "刷新", font_size = button_font_size,
    ...
    for i, cat in ipairs(categories) do
        local btn = airui.button({
            parent = category_sidebar, x = 10, y = cat_y, w = cat_width, h = cat_height,
            text = cat, font_size = button_font_size,
            style = {
                ...
                radius = cat_radius, border_width = 1, border_color = COLOR_DIVIDER, pad = 0
            },
            on_click = function()
    ...
    prev_btn = airui.button({
        parent = page_bar, x = prev_btn_x, y = page_nav_btn_y, w = prev_btn_w, h = page_nav_btn_h,
        text = "上一页",
    ...
    next_btn = airui.button({
        parent = page_bar, x = screen_w - side_w - 86, y = page_nav_btn_y, w = next_btn_w, h = page_nav_btn_h,
        text = "下一页",
        font_size = math.max(math.floor(14 * _G.density_scale), button_font_size - math.floor(4 * _G.density_scale)),
        style = { bg_color = COLOR_PRIMARY, pressed_bg_color = COLOR_PRIMARY_DARK, text_color = 0xFFFFFF, radius = page_nav_radius, border_width = 0 },
        on_click = function()
    ...
    page_label = airui.label({
        parent = page_bar, x = label_x, y = math.floor((page_btn_h - 30) / 2),
        w = screen_w - side_w - next_btn_w * 2 - 40, h = 30,
        text = "1/?", font_size = button_font_size, color = COLOR_TEXT_SECONDARY, align = airui.TEXT_ALIGN_CENTER
    })
end
...
local function render_apps(apps, more)
    ...
    apps_grid = airui.container({
        parent = apps_container, x = grid_margin, y = grid_margin, w = apps_grid_w, h = new_grid_h, color = COLOR_BG
    })
    ...
    for idx, app in ipairs(apps) do
        ...
        local card = airui.container({
            parent = apps_grid, x = x, y = y, w = card_w, h = card_h,
            color = COLOR_CARD, radius = 16, border_width = 1, border_color = COLOR_DIVIDER
        })
        ...
        local name_y = 15
        airui.label({
            parent = card, x = icon_size + 20, y = name_y,
            w = card_w - icon_size - 28, h = title_font_size + 4,
            text = app.title or app.name or "未知",
            font_size = title_font_size, color = COLOR_TEXT
        })
        ...
        airui.label({
            parent = card, x = icon_size + 20, y = info_y,
            w = card_w - icon_size - 28, h = info_font_size + 4,
            text = info_text, font_size = info_font_size, color = COLOR_TEXT_SECONDARY
        })
        ...
        if max_desc_lines > 0 then
            ...
            airui.label({
                parent = card, x = 12, y = desc_y, w = card_w - 24, h = desc_height,
                text = app.desc or "", font_size = desc_font_size, color = COLOR_TEXT_SECONDARY
            })
        end
        ...
        if app.installed then
            if app.has_update then
                ...
                airui.button({
                    parent = card, x = 12, y = btn_y, w = btn_w, h = btn_height,
                    text = "更新", font_size = button_font_size,
                    ...
                    on_click = function()
                        local msgbox = airui.msgbox({
                            title = "确认更新", text = "是否更新应用 " .. (app.title or app.name) .. "？",
                            buttons = { "确定", "取消" },
                            on_action = function(self, label)
                                if label == "确定" then
                ...
                airui.button({
                    parent = card, x = 12 + btn_w + 12, y = btn_y, w = btn_w, h = btn_height,
                    text = "卸载", font_size = button_font_size,
                    style = { bg_color = 0xF44336, pressed_bg_color = 0xD32F2F, text_color = 0xFFFFFF, radius = 16, border_width = 0 },
                    on_click = function()
                        local msgbox = airui.msgbox({
                            title = "确认卸载", text = "是否卸载应用 " .. (app.title or app.name) .. "？",
                            buttons = { "确定", "取消" },
                            on_action = function(self, label)
                                if label == "确定" then sys.publish("APP_STORE_UNINSTALL", tostring(app.aid), current_category, current_sort) end
                                self:hide()
                            end
                        })
                        msgbox:show()
...
local function on_list_updated(apps, page_info)
    ...
    if type(page_info) == "table" then
        if type(page_info.page) == 'number' then current_page = page_info.page end
        if type(page_info.total_pages) == 'number' then total_pages_now = page_info.total_pages
        elseif type(page_info.pages) == 'number' then total_pages_now = page_info.pages end
        if page_info.has_more ~= nil then has_more_now = (page_info.has_more == true)
        else has_more_now = (current_page < total_pages_now) end
    end

    has_more = has_more_now
    if type(total_pages_now) ~= "number" then total_pages_now = 1 end
    ...
    for _, app in ipairs(apps) do
        ...
        if cached ~= nil then
            ...
            if cached and installed_apps[aid] then
                ...
                if info and info.path then app.path = info.path; app.icon_path = info.icon_path end
            end
        else
...
local function on_progress(aid, percent, status_text)
    if progress_bar and progress_label then
        ...
        if percent >= 100 then progress_label:set_text("解压完成，请稍候...") end
    end
end
    ...
    if success then
        ...
        if apps then
            ...
            for i, app in ipairs(apps) do
                if tostring(app.aid) == aid then idx = i; break end
            end
            if idx then
                if action == "install" then
                    apps[idx].installed = true; apps[idx].has_update = false
                elseif action == "update" then
                    apps[idx].has_update = false; apps[idx].installed = true
        ...
        if action == "install" and success then local_installed_ids[key] = true
        elseif action == "uninstall" and success then local_installed_ids[key] = false end
    end
end

local function on_error(msg)
    close_progress_dialog()
    local msgbox = airui.msgbox({ title = "错误", text = msg, buttons = { "确定" }, on_action = function(self, label) self:hide() end })
    msgbox:show()
end
...
local function on_get_focus()
    local apps, more = exapp.get_current_list()
    if apps then render_apps(apps, more) end
end
