...
local function create_ui()
    ...
    airui.label({ parent = header, x = 10, y = 12, w = 460, h = 40, align = airui.TEXT_ALIGN_CENTER, text="WiFi", font_size=32, color=0xffffff })

    content = airui.container({ parent = main_container, x=0, y=60, w=480, h=740, color=0xF3F4F6 })

    airui.button({
        parent = content, x=180, y=20, w=120, h=50,
        text = "扫描",
        font_size = 20,
    ...
    scan_list = airui.table({
        parent = content, x=20, y=90, w=440, h=200,
    ...
    connect_btn = airui.button({
        parent = content, x=180, y=310, w=120, h=50,
        text = "连接",
        font_size = 20,
        on_click = function() log.info("wifi", "连接") end
    })

    airui.textarea({ parent = content, x=20, y=310, w=140, h=40, placeholder = "密码", font_size=18 })
end
