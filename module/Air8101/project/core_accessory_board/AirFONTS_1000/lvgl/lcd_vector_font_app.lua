...
local function lcd_vector_font_app_task_func()
    ...
    local font16 = lvgl.font_load(air_vetor_fonts.spi_gtfont, 16)

    -- 创建一个按钮
    local btn = lvgl.btn_create(scr)  -- 在屏幕上创建一个按钮
    lvgl.obj_set_size(btn, 800, 400)  -- 设置按钮大小为240x50
    lvgl.obj_align(btn, nil, lvgl.ALIGN_CENTER, 0, 0)  -- 将按钮左右居中，向下偏移200像素

    -- 在按钮上添加一个标签
    local btn_label = lvgl.label_create(btn)  -- 在按钮上创建一个标签
    lvgl.obj_set_style_local_text_font(btn_label, lvgl.LABEL_PART_MAIN, lvgl.STATE_DEFAULT, font16)  -- 设置为16号矢量字体
    ...
    --         -- 如果大于192，复位到默认值16
    --         if counter_label_font_size>192 then counter_label_font_size = 16 end
    ...
    --         lvgl.obj_set_style_local_text_font(btn_label, lvgl.LABEL_PART_MAIN, lvgl.STATE_DEFAULT, font_counter_label) 
    --         -- 设置标签上显示内容的颜色，依次在黑，红，绿，蓝之间循环
    --         lvgl.obj_set_style_local_text_color(btn_label, lvgl.LABEL_PART_MAIN, lvgl.STATE_DEFAULT, color_table[counter_label_font_size%4+1])

    --         -- 更新标签上显示的内容
    --         lvgl.label_set_text(btn_label, counter_label_font_size .. "号")
    --     end
    -- end
