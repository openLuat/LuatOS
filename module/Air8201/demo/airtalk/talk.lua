...
-- 硬件版本选择：Air8201G 或 Air8201H
-- Air8201G: BTB扩展板按键使用 WAKEUP0 + PWRKEY
-- Air8201H: 板载按键使用 BOOT(GPIO0) + PWRKEY
local HW = "H"  -- 请根据实际硬件版本修改: "G" 或 "H"
...
-- Boot/Wakeup键回调函数
-- Air8201H: GPIO0(BOOT键)  Air8201G: gpio.WAKEUP0
-- 用于一对一對講控制
local function boot_key_callback()
    log.info("boot_key_callback")
    sys.sendMsg(USER_TASK_NAME, MSG_KEY_PRESS, false)  -- false表示Boot/Wakeup键
end
...
-- 初始化按键
-- Air8201G: BTB扩展板 WAKEUP0 + PWRKEY
-- Air8201H: 板载 BOOT(GPIO0) + PWRKEY
local function init_buttons()
    if HW == "G" then
        gpio.setup(gpio.WAKEUP0, boot_key_callback, gpio.PULLUP, gpio.FALLING)
