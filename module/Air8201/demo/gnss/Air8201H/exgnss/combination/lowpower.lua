...
@author  李源龙
@usage
使用Air8201H板子，外接GPS天线，起一个60s定位一次的定时器，唤醒模块120s一定位，
...
local function gnss_fnc()
    ...
    --GPIO25(主供电，交给exgnss动态开关)
    --所以此处先手动把GPIO26常置为高，只把GPIO25交给exgnss的gnss_volgpio参数动态控制
    gpio.setup(26, 1)
    local gnssotps={
        gnssmode=1, --1为卫星全定位，2为单北斗
        agps_enable=true,    --是否使用AGPS，开启AGPS后定位速度更快，会访问服务器下载星历，星历时效性为北斗1小时，GPS4小时，默认下载星历的时间为1小时，即一小时内只会下载一次
        gnss_volgpio=25,    --8201H使用GPIO25控制GNSS主供电，GPIO26(备电)已在上方常置为高
        -- debug=true,    --是否输出调试信息
        -- uart=2,    --使用的串口,8201H默认串口2
        -- uartbaud=115200,    --串口波特率，8201H默认115200
    ...
    sys.timerLoopStart(lower_open,120000)       --每120s开启一次GNSS
    -- gpio.close(23)--此脚为gnss备电脚，功能是热启动和保存星历文件，关掉会没有热启动，常开功耗会增高0.5-1MA左右
