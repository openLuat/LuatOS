...
2、netdrv_device.lua：网卡驱动设备，可以配置使用netdrv文件夹内的两种网卡(单4g网卡，pc模拟器上的网卡)中的任何一种网卡；

3、netdrv文件夹：两种网卡，单4g网卡、pc模拟器上的网卡，供netdrv_device.lua加载配置；
...
1、Air8201整机板一块+可上网的 sim 卡一张：

* sim 卡插入整机板的 sim 卡槽

2、TYPE-C USB 数据线一根，Air8201整机板和电脑的硬件接线方式为：

* Air8201整机板通过 TYPE-C USB 口供电；
...
5、烧录成功后，代码会自动运行，查看打印日志，如果正常运行，会打印网络就绪、时间同步成功、本地时间以及UTC时间等信息，如下log显示：
