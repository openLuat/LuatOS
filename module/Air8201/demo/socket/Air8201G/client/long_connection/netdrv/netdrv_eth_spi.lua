...
Air8201G BTB扩展板和AirETH_1000配件板的硬件接线方式为:
Air8201G BTB扩展板通过TYPE-C USB口供电；
如果测试发现软件重启，并且日志中出现  poweron reason 0，表示供电不足，此时再通过直流稳压电源对Air8201G BTB扩展板的5V管脚进行5V供电；
...
-- 本demo使用Air8201G BTB扩展板+AirETH_1000配件板测试，BTB扩展板上的硬件配置为：
-- Air8201G BTB扩展板的VDD 3V3管脚对AirETH_1000配件板进行供电；3V3管脚是作为LDO 3.3V输出，供测试用的，仅在使用DCDC供电时有输出，默认打开，无需控制
-- 使用spi0，片选引脚使用GPIO8
