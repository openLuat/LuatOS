...
local config = {
    ...
    mtn_log_write_way = exmtn.CACHE_WRITE, -- 运维日志写入方式
}
...
local function build_header(need_reply, is_udp_transport, data_length)
    ...
    log.info("[excloud]构建消息头", "seq:", sequence_num, "len:", data_length, "flags:", flags, "dev:", device_id_binary and string.toHex(device_id_binary) or "nil")
...
function excloud.build_tlv(field_meaning, data_type, value)
    ...
    if value_encoded == nil or value_encoded == "" then
        log.warn("[excloud]构建tlv打包数据失败: encode_value返回空", "field:", field_meaning, "type:", data_type)
    ...
    local tlv_data = to_big_endian(field_type, 2) ..
                     to_big_endian(#value_encoded, 2) ..
                     value_encoded
...
local function send_auth_request()
    ...
    elseif config.device_type == 4 then -- 以太网设备
        auth_data = config.auth_key .. "-" .. config.device_id:gsub("[^%w]", ""):upper() .. "-" .. mcu.unique_id():toHex()
...
local function send_mtn_status(index, status)
    excloud.send({{
        ...
        data_type = DATA_TYPES.BINARY,
        value = json.encode({ index = index, status = status, timestamp = os.time() })
    }}, false)
end
...
function excloud.getip(getip_type)
    ...
    log.info("[excloud]getip响应", "HTTP:", code, "Body:", #response_body > 128 and string.sub(response_body, 1, 128) .. "..." or response_body)
    ...
    if getip_type >= 3 and getip_type <= 5 then
        if response_json.conninfo then
            ...
            if getip_type == 5 then
                log.info("[excloud]MQTT连接信息", "host:", response_json.conninfo.ssl, "port:", response_json.conninfo.port, "user:", response_json.conninfo.username)
                log.info("[excloud]MQTT设备认证", "client:", mobile.imei(), "pwd:", mobile.muid())
            else
                log.info("[excloud]TCP/UDP连接信息", "host:", response_json.conninfo.ipv4, "port:", response_json.conninfo.port, "key:", response_json.conninfo.key)
            end
        else
...
local function send_file_upload_notify(notify_field, file_type, file_name, file_size, upload_ok)
    local tlvs = {
        { field_meaning = notify_field, data_type = DATA_TYPES.INTEGER, value = 0 },
        { field_meaning = FIELD_MEANINGS.FILE_UPLOAD_TYPE, data_type = DATA_TYPES.INTEGER, value = file_type },
        { field_meaning = FIELD_MEANINGS.FILE_NAME, data_type = DATA_TYPES.ASCII, value = file_name },
    }
    if file_size then
        tlvs[#tlvs + 1] = { field_meaning = FIELD_MEANINGS.FILE_SIZE, data_type = DATA_TYPES.INTEGER, value = file_size }
    end
    if upload_ok ~= nil then
        tlvs[#tlvs + 1] = { field_meaning = FIELD_MEANINGS.UPLOAD_RESULT_STATUS, data_type = DATA_TYPES.INTEGER, value = upload_ok and 0 or 1 }
    end
...
local function do_upload_file(file_type, file_path, file_name, upload_info)
    ...
    while retry_count <= max_retries do
        ...
        if use_zbuff then
            ...
            if not body_zbuff then
                ...
                if callback_func then callback_func("file_upload", { success = false, error = "内存不足" }) end
                return
            end
    ...
    -- 发送上传完成通知（运维日志不需要）
    if file_type ~= 3 then
        local notify_ok, notify_err = send_file_upload_notify(FIELD_MEANINGS.FILE_UPLOAD_FINISH, file_type, file_name, nil, upload_success)
    ...
    if reconnect_count >= config.max_reconnect then
        ...
        sys.taskInit(function()
            ...
            if config.use_getip then
                ...
                if ok then
                    log.info("[excloud]重连获取服务器成功", "host:", config.host, "port:", config.port, "transport:", config.transport)
...
local function _socket_callback(label, netc, event, param)
    ...
    elseif event == socket.ON_LINE then
        if param ~= 0 then
            ...
            is_connected = false; is_authenticated = false
            if callback_func then callback_func("connect_result", { success = false, error = "Connection failed, param=" .. param }) end
            cleanup_connection()
            if config.auto_reconnect and is_open then schedule_reconnect() end
            return
        end
        log.info("[excloud]" .. label .. " socket", label .. "连接成功")
        is_connected = true; reconnect_count = 0
        if callback_func then callback_func("connect_result", { success = true }) end
        send_auth_request()
    ...
    elseif event == socket.EVENT then
        ...
        if type(ok) == "boolean" and not ok then
            ...
            is_connected = false; is_authenticated = false
            if callback_func then callback_func("disconnect", {}) end
            cleanup_connection()
            if config.auto_reconnect and is_open then schedule_reconnect() end
        else
            if rxbuff:used() > 0 then
                recv_buf = recv_buf .. rxbuff:query()
                rxbuff:del()
                while #recv_buf >= 16 do
                    local msg_len = 16 + from_big_endian(recv_buf, 11, 2)
                    if #recv_buf < msg_len then break end
                    local msg = recv_buf:sub(1, msg_len)
                    recv_buf = recv_buf:sub(msg_len + 1)
                    sys.publish("EXCLOUD_RAW_MSG", msg)
                    parse_data(msg)
                end
            end
        end
...
local function _connect_socket(is_udp, label, ssl_config, callback)
    ...
    if not rxbuff then return false, "Failed to create rx buffer" end
    log.info("[excloud]创建" .. label .. "连接")
    connection = socket.create(socket.dft(), callback)
    if not connection then return false, "Failed to create socket" end
    local config_ok = socket.config(connection, config.local_port, is_udp,
        ...
        ssl_config and ssl_config.keep_cnt or config.keep_cnt,
    ...
    if not config_ok then socket.release(connection); connection = nil; return false, "Socket config failed" end
    ...
    if not ok then
        stop_connect_timeout()
        socket.close(connection); socket.release(connection); connection = nil
        if config.auto_reconnect then schedule_reconnect() end
        return false, result
    end
    return true
end
    ...
    if config.use_getip then
        ...
        if not config.current_conninfo or (config.transport ~= "mqtt" and not config.current_conninfo.ipv4) or
            ...
            for _, f in ipairs({"imginfo", "audinfo", "mtninfo", "qrinfo"}) do
                if result[f] then config["current_" .. f] = result[f] end
            end
        end
    else
...
function excloud.send(data, need_reply, is_auth_msg)
    ...
    for _, item in ipairs(data) do
        log.info("[excloud]构建发送数据", "field:", item.field_meaning, "type:", item.data_type, "value:", item.value)
