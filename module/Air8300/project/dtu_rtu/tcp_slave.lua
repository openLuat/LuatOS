...
sys.taskInit(function()
    ...
    -- 注册回调函数（包一层，TCP请求发布独立事件）
    if tcp_slave and modbus_callback then
        tcp_slave:on(function(req)
            ...
            if resp and type(resp) == "table" then
                ...
                local a = req.start_addr
                local val = ""
                if a == 0x0000 then val = string.format(" | 湿度:%.1f%% 温度:%.1f℃", d.humidity, d.temperature)
                elseif a == 0x0002 then val = string.format(" | 温度:%.1f℃", d.temperature)
                elseif a == 0x0004 then val = string.format(" | CPU:%.1f℃", d.cpu_temperature)
                elseif a == 0x0006 then val = string.format(" | VBAT:%.3fV", d.vbat_voltage)
                elseif a == 0x001C then val = string.format(" | 信号:%d dBm", d.signal_strength)
                elseif a == 0x0037 then val = string.format(" | 时间戳:%d", d.timestamp)
                end
                sys.publish("MODBUS_TCP_REQ", string.format("[TCP] TX:%s  RX:%02X %02X %02X %s%s",
                    req_hex, req.slave_id, req.func_code, req.reg_count * 2, hex, val))
            end
            return resp
        end)
