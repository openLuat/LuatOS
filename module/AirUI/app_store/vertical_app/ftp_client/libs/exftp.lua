...
function exftp:upload(local_path, remote_path, opts)
    opts = opts or {}
    log.info("opts.timeout", opts.timeout/1000)
    ...
    local fbuf = zbuff.create(buffer_size)

    if not fbuf then
        fbuf = zbuff.create(1024 * 64)  -- 默认64KB，增大缓冲区
    end

    if not fbuf then
        fbuf = zbuff.create(1024 * 24)  -- 降级到24KB
    end

    if not fbuf then
        fbuf = zbuff.create(1024 * 8)   -- 降级到8KB
    end
    ...
    while not is_closed and not upload_completed do
        ...
        -- 检查超时
        log.info("用时",os.time() - start_time ,"超时", timeout / 1000)
...
-- @param remote_path 远程文件路径
-- @param opts 选项，{timeout=超时时间(毫秒)}
-- @return data 文件内容，nil表示失败
function exftp:retr(remote_path, opts)
    opts = opts or {}
    local timeout = opts.timeout or 5 * 60 * 1000  -- 默认5分钟
    ...
    while not is_closed and not self.data_closed do
        -- 先尝试读取已有数据
        if self.data_rx_buff and self.data_rx_buff:used() > 0 then
            local chunk = self.data_rx_buff:query()
            data = data .. chunk
