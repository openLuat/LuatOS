...
4. 004.000.039 新增：电池低压截止保护——未充电且电压≤3.2V 连续确认后执行 YHM2712A 船运模式
   （断开电池关机，USB 插入自动开机续充）；电量 0% 折算线同步下调至 3200mV
5. 004.000.040 新增：充电中电压可信门控——拒绝 VBAT 系统轨被抬升产生的 ~4.12V 假值
   （battery 模块以最近可信基准限制充电中单步跳变，防污染值进入缓存与上报）
6. 004.000.041 修复：YHM2712A 充电电流档位不生效——扩展库 v1.4 起有效档位为 CCMIN/CCMID/CCMAX，
   v1.3 旧档位名 "DEFAULT" 已失效，原样透传会触发库内参数校验失败，导致充电初始化整体不生效
   默认最小电流 CCMIN，服务端下发 MIN/MID/MAX 仍可覆盖，非法值（含旧值 DEFAULT）回退 CCMIN 并告警
]]
PROJECT = "Air8202"
VERSION = "004.000.041"
