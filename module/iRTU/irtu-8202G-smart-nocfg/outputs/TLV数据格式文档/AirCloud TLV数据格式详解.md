...
**适用项目**：iRTU-8202G-smart-nocfg 车辆/宠物定位器固件（LuatOS，Air780EGP）
...
**固件版本**：004.000.037（本文档随 004.000.037 同步更新：移除 1281 JSON 通道，新增 1296~1299 指令应答字段与 799/782/1290/1027 兜底状态帧）
...
| 区间 | 类别 | 本固件是否使用 |
|------|------|---------------|
| 16–255 | 控制信令（鉴权/回复/命令/文件/短信） | ✔ 16/17/18 |
| 256–511 | 传感采集类（温湿度等） | ✘ |
...
- **类型**：ASCII，value 即版本字符串 `"004.000.037"`；
...
- 1290=0（GNSS 关闭），电压 3980mV，未充电，CSQ 25，定位失败（gps_status=3），频段 `BAND_LTE_B3`，单点三轴 `"-0.012,0.003,0.995"`，型号 `Air780EGP`，开机原因 `power_key`，唤醒间隔 300s，ICCID `8986011912345678901`，版本 `004.000.037`
...
| 12 | 1027 固件版本 | `34 03` | `00 0C` | `"004.000.037"` | 16B |
| | | | | **合计** | **141B** |
...
*文档随固件同步更新：004.000.035 新增 1295 实时 1s 三轴流；004.000.036 将 1290 改为上报模式状态（0=GNSS 关 / 1=GNSS 开 / 2=实时）；004.000.037 移除 1281 JSON 通道，新增 1296~1299 指令应答字段与 799/782/1290/1027 兜底状态帧。插图由 `gen_figs_tlv.py` 生成（`images/fig1~fig6`）。*
