...
@usage
本模块的核心功能为：
1. 通过内部 VBAT 检测（adc.CH_VBAT）读取电池电压
2. 通过VBUS(WAKEUP1)引脚检测充电状态
3. 提供电池电压、电量和充电状态查询接口

硬件连接：
- VBAT: Air780EGP 内部 ADC 直连 VBAT 管脚（adc.CH_VBAT，2.2-4.8V），无需外部分压
...
-- 读取电池电压（多次采样取平均）
-- Air780EGP：VBAT 管脚内部已连 ADC，直接用 adc.CH_VBAT 读取电池电压(mV)，无需外部分压。
-- USB 供电（未接电池）时 VBAT 可正常读到约 4V，避免外部 ADC0 无电压导致误判低电量。
-- @return number|nil 电池电压(mV)，失败/异常返回nil（不更新电量，避免误触发低电量）
local function read_battery_voltage()
    ...
    for i = 1, sample_count do
        adc.open(adc.CH_VBAT)          -- 内部 VBAT 检测通道
        local vadc = adc.get(adc.CH_VBAT)  -- 直接返回 mV
        adc.close(adc.CH_VBAT)

        -- 有效范围 2200-4800mV（Air780EGP VBAT 检测范围），异常样本丢弃
        if vadc and vadc >= 2200 and vadc <= 4800 then
            table.insert(bTable, vadc)
        end
