    ...
    sys.taskInit(function()
        ...
        if tonumber(code) == 200 and body then
            ...
            if dat and dat.code == 0 then
                -- 3. 解析 parameter_gnss 并更新 config 模块
                local config = require("config")
                config.load_from_server(dat.parameter_gnss, dat.parameter.project_key)

                -- 4. 持久化到文件系统（参考 iRTU cfg:import）
                local save_sheet = {
                    uconf = true,                              -- 标志位：有本地配置
                    param_ver = dat.parameter.param_ver or 0,  -- 参数版本
                    project_key = dat.parameter.project_key,   -- MQTT 密钥
                    gnss = dat.parameter_gnss,                 -- 定位版配置
                }
                cfg:import(save_sheet)
                param_ver = dat.parameter.param_ver or 0
                log.info("cfg_fetch", "配置已持久化，param_ver:", param_ver)

                -- 5. 有新版配置，重启生效（参考 iRTU）
                if tonumber(dat.parameter.param_ver) ~= nil then
