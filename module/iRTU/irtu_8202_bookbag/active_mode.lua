...
- GNSS 开启时间窗（001.000.002 新增，gnss_schedule 下行命令配置）：启用后为最高优先级硬约束——
  窗内 GNSS 强制开启（无需震动），窗外强制关闭（震动/开机窗口均不触发 GNSS）；
  start>end 跨零点；设备时间未同步（NTP）时时间窗不生效按原逻辑；
  fast_report 结束后的 180 秒收尾保持窗口优先于时间窗（收尾完再关）
...
-- ====== GNSS 开启时间窗（001.000.002 新增，gnss_schedule 下行命令配置） ======
-- 书包场景：上课时段关 GNSS 省电，上下学路上窗内 GNSS 常开。
-- 配置存于 kvstore（重启自动恢复），remote.gnss_schedule 写入后发布 GNSS_SCHEDULE_UPDATED，
-- 本模块刷新缓存并重新评估。窗内强制开 / 窗外强制关（硬约束，优先于震动与开机窗口条件）。
local gnss_schedule_cfg = nil  -- 时间窗配置缓存 {enable,start,end}；nil=未配置

-- 解析 "HH:mm" 为当日分钟数（0~1439），格式非法返回 nil
...
-- 设备时间是否已同步（NTP）：os.time 早于 2026-01-01 视为未同步，
-- 此时时间窗不生效（按原 gsensor 逻辑运行，宁可多开不多关），对时后自动生效
local function is_time_synced()
    return os.time() >= 1767196800  -- 2026-01-01 00:00:00 UTC+8
end

-- 从 kvstore 加载时间窗配置（模块加载时 + 收到 GNSS_SCHEDULE_UPDATED 事件时调用）
local function reload_gnss_schedule()
    local ok, cfg = pcall(kvstore.get_gnss_schedule)
    gnss_schedule_cfg = (ok and cfg) or nil
    if gnss_schedule_cfg and tonumber(gnss_schedule_cfg.enable) == 1 then
        log.info("active_mode", "GNSS时间窗:", tostring(gnss_schedule_cfg.start), "-", tostring(gnss_schedule_cfg["end"]))
    else
        gnss_schedule_cfg = nil
        log.info("active_mode", "GNSS时间窗: 未配置")
    end
end
reload_gnss_schedule()  -- 模块加载时读一次（main.lua 已先 fskv.init()）

-- 判断当前是否处于 GNSS 时间窗内；返回 true=窗内 / false=窗外 / nil=时间窗不生效（未配置或时间未同步）
local function in_gnss_window()
    local cfg = gnss_schedule_cfg
    if not cfg then return nil end
    if not is_time_synced() then return nil end
    local start_min = parse_hhmm(cfg.start)
    local end_min = parse_hhmm(cfg["end"])
    if not start_min or not end_min or start_min == end_min then return nil end
    local now_min = tonumber(os.date("%H")) * 60 + tonumber(os.date("%M"))
    if start_min < end_min then
        return now_min >= start_min and now_min < end_min
    end
    -- 跨零点：start > end（如 22:00-06:00 = 22:00 至次日 06:00）
...
local function is_gnss_required()
    -- 条件-1（最高优先级）：GNSS 时间窗硬约束——窗内强制开，窗外强制关
    -- （覆盖震动/开机窗口条件；未配置或时间未同步时 in_gnss_window 返回 nil 不干预）
    -- 例外：窗外时若处于 fast_report 结束后的收尾保持窗口内仍保持开启（收尾 180 秒后再关）
    local in_win = in_gnss_window()
    if in_win ~= nil then
        if in_win then
            return true
        end
...
local function main_loop()
    ...
    -- 订阅 GNSS 时间窗配置更新（remote.gnss_schedule 下发后发布）：刷新缓存并唤醒主循环
    -- 立即重新评估——出窗时秒级关闭 GNSS，入窗/新配置也即时生效，无需等下一上报周期
    sys.subscribe("GNSS_SCHEDULE_UPDATED", function()
        reload_gnss_schedule()
        sys.publish(MOTION_EVENT)
    end)
    ...
    while true do
        ...
        -- 1) 评估 GNSS 开关需求（时间窗 / 开机窗口 / 正在震动 / 180 秒内震过 / 实时上报结束保持窗口）
        --    实时上报模式期间暂停评估（期间不切换 GNSS 开关，结束后统一处理）
        if not fast_report_active then
            local gnss_needed = is_gnss_required()

            -- 2) GNSS 状态机：仅在状态切换时执行开关动作（原因写入日志/运维日志便于排查）
            if gnss_needed and not gnss_active then
                switch_gnss_on(in_gnss_window() == true and "时间窗" or "开机窗口/震动")
            elseif not gnss_needed and gnss_active then
                ...
                local close_reason = (in_gnss_window() == false) and "时间窗外" or "无震动超时"
                log.info("active_mode", "GNSS 关闭（" .. close_reason .. "），功耗 mode1，每 300 秒上报")
                excloud.mtn_log("info", "gnss", "GNSS关闭", "触发", close_reason, "上报间隔", REPORT_GNSS_OFF .. "s")
            end
        end
