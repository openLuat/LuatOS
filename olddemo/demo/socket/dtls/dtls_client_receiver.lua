--[[
@module  dtls_client_receiver
@summary dtls client socket数据接收应用功能模块
@version 1.0
@date    2026.06.04
@usage
本文件为dtls client socket数据接收应用功能模块，核心业务逻辑为：
从内核读取接收到的数据，然后将数据发送给其他应用功能模块做进一步处理；

本文件的对外接口有2个：
1、dtls_client_receiver.proc(socket_client)：数据接收应用逻辑处理入口，在dtls_client_main.lua中调用；
2、sys.publish("RECV_DATA_FROM_SERVER", "recv from dtls server: ", data)：
   将接收到的数据通过消息"RECV_DATA_FROM_SERVER"发布出去；
   需要处理数据的应用功能模块订阅处理此消息即可，本demo项目中uart_app.lua中订阅处理了本消息；
]]

local dtls_client_receiver = {}
...
@api dtls_client_receiver.proc(socket_client)
...
@usage
-- 
dtls_client_receiver.proc(socket_client)
]]
function dtls_client_receiver.proc(socket_client)
    ...
    while true do
        ...
        if not result then
            log.error("dtls_client_receiver.proc", "socket.rx error")
            return false
        end

        -- 如果读取到了数据, used()就必然大于0, 进行处理
        if recv_buff:used() > 0 then
            log.info("dtls_client_receiver.proc", "recv data len", recv_buff:used())

            -- 读取socket数据接收缓冲区中的数据，赋值给data
            local data = recv_buff:query()

            -- 将数据data通过"RECV_DATA_FROM_SERVER"消息publish出去，给其他应用模块处理
            sys.publish("RECV_DATA_FROM_SERVER", "recv from dtls server: ", data)
