--[[
@module  dtls_client_sender
@summary dtls client socket数据发送应用功能模块
@version 1.0
@date    2026.06.04
@usage
本文件为dtls client socket数据发送应用功能模块，核心业务逻辑为：
1、sys.subscribe("SEND_DATA_REQ", send_data_req_proc_func)订阅"SEND_DATA_REQ"消息，将其他应用模块需要发送的数据存储到队列send_queue中；
2、dtls_client_main主任务调用dtls_client_sender.proc接口，遍历队列send_queue，逐条发送数据到server；
3、dtls client socket和server之间的连接如果出现异常，dtls_client_main主任务调用dtls_client_sender.exception_proc接口，丢弃掉队列send_queue中未发送的数据；
...
2、dtls_client_sender.proc：数据发送应用逻辑处理入口，在dtls_client_main.lua中调用；
3、dtls_client_sender.exception_proc：数据发送应用逻辑异常处理入口，在dtls_client_main.lua中调用；
]]

local dtls_client_sender = {}
...
dtls_client_sender.TASK_NAME = "dtls_client_main"
...
local function send_data_req_proc_func(tag, data, cb)
    ...
    -- 通知dtls_client_main主任务有数据需要发送
    -- dtls_client_main主任务如果处在libnet.wait调用的阻塞等待状态，就会退出阻塞状态
    sys.sendMsg(dtls_client_sender.TASK_NAME, socket.EVENT, 0)
end

--[[
检查socket client是否需要发送数据，如果需要发送数据，读取并且发送完发送队列中的所有数据

@api dtls_client_sender.proc(task_name, socket_client)
...
@usage
dtls_client_sender.proc("dtls_client_main", socket_client)
]]
function dtls_client_sender.proc(task_name, socket_client)
    local send_item
    ...
    while #send_queue>0 do
        ...
        -- 发送失败
        if not result then
            log.error("dtls_client_sender.proc", "libnet.tx error")
        ...
        if buff_full then
            log.error("dtls_client_sender.proc", "buffer is full, wait for the next time")
            table.insert(send_queue, 1, send_item)
            return true
        end

        log.info("dtls_client_sender.proc", "send success")
...
@api dtls_client_sender.exception_proc()

@usage
dtls_client_sender.exception_proc()
]]
function dtls_client_sender.exception_proc()
    -- 遍历数据发送队列send_queue
    while #send_queue>0 do
