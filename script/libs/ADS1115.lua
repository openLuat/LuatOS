...
sys.taskInit(function ()
    ...
    ads1115.Setup(i2cid) -- 一次初始化
	
	while true do
		log.info("ads1115:",ads1115.task_read(5))
		sys.wait(1000)
	end
...
@usage
local ads1115_data = ads1115.task_recv(0,2)
log.info("ads1115", ads1115_data)
]]
function ads1115.task_recv( MU1,FSR )   
...
function ads1115.task_read(MU1)
    local ads = ads1115.task_recv(MU1,0) --先用大量程测量正负6.144去测量 逼近
	if ads == nil then 
	...
	if 	ads_ADC > 10922 then	
        if ads_ADC>20000 then
            ...
            ads = ads1115.task_recv(MU1,1)
            mv = ads * 0.125
        end
		
	elseif 	ads_ADC > 5461 then	
		ads = ads1115.task_recv(MU1,2)
		mv = ads * 0.0625
	elseif 	ads_ADC > 2730 then
		ads = ads1115.task_recv(MU1,3)
		mv = ads * 0.03125
	elseif ads_ADC > 1365 then 	
		ads = ads1115.task_recv(MU1,4)
		mv = ads * 0.015625
	else
		ads = ads1115.task_recv(MU1,5)
		mv = ads * 0.0078125
	end
