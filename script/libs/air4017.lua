--[[
    ...
    @usage
        ...
        -- 1) 初始化: UART1, 波特率 2M
        air4017.init(1, 2000000)

        -- 2) 注册回调
        air4017.on_audio_data(function(data)
            -- data: 每帧 512B PCM(16ms / 16kHz / 16bit / 单声道), 来自 Air4017 MIC 上行
        end)
        air4017.on_identify(function(text)
            -- text: 离线识别结果 UTF-8 文本(如 "打开灯光")
        end)

        -- 3) 下行播放(接 AI 大模型/通话下行音频)
        air4017.play_stream_start(20)     -- 启动流式播放, 音量 20(0~31)
        air4017.play_stream_write(pcm)    -- 喂入 PCM(任意长度, 内部按 320B/帧切分)
        air4017.play_stream_stop()        -- 停止播放
...
function air4017.on_identify(cb)
    cb_identify = cb
end

-- 注册上行 MIC 数据回调
-- @param cb function(data), data 为每帧 512B PCM(16ms / 16kHz / 16bit / 单声道)
function air4017.on_audio_data(cb)
    cb_audio_data = cb
end

-- 开关接收数据解析(非通话时关闭)
-- @param flag true=解析 false=丢弃
function air4017.set_rx_enable(flag)
    rx_enabled = flag
    if not flag then
        rxstr = ""
    end
...
-- @return boolean
function air4017.reset()
    if not is_inited then return false end
    uart.write(UART_ID, pack(CMD_RESET, string.char(0x00)))
    log.info("air4017", "发送程序复位 02 05 00, 等待重新初始化后自动恢复上行")
    return true
end
...
-- @param vol 音量值 0~31
-- @return boolean
function air4017.set_volume(vol)
    if not is_inited then return false end
...
function air4017.play_stream_start(vol)
    if not is_inited then return false end
    if vol then air4017.set_volume(vol) end
...
function air4017.play_stream_write(data)
...
function air4017.play_direct(data)
    if not is_inited then return false end
