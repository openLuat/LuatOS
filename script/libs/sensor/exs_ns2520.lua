--[[
@module  exs_ns2520
@summary NS2520 气压传感器扩展库（NS2520）
@version 1.0
@date    2026.07.16
@author  王世豪
@usage
...
@table init_cfg 初始化配置表
    ...
    prs_osr:number, 气压过采样率0~7，可选，不传默认4(16x)。手册Table9推荐：0=1x(5Pa/3μA)、4=16x(~1.2Pa/22μA)、6=64x(~0.6Pa/200μA)
...
@return table/nil 成功返回{pressure:气压值(number,300~1200hPa),temperature:温度值(number,-40~85℃)}，失败返回nil
@usage
...
@return number/nil 补偿后的气压值(hPa)，失败返回nil
@usage
...
function exs_ns2520.set_osr(p_osr, t_osr)
    ...
    if p_osr then
        ...
        log.info("exs_ns2520", "气压OSR", p_osr)
    end
    if t_osr then
