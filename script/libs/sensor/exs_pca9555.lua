...
-- 2、更新内容
  - 第二版，参考 AirGPIO_1000 demo 重新设计 API 接口
  - 新增：自动识别从设备地址功能
  - 新增：GPIO 中断模式支持（通过 INTR 引脚 + sys.publish 机制）
  - 新增：deinit() 接口释放资源
  - 优化：GPIO ID 编码方式（0x00-0x07 Port0，0x10-0x17 Port1）
  - 优化：统一 setup/set/get/close API 命名规范
  - 修改：PCA9555 寄存器地址与 TCA9555 相同
]]
