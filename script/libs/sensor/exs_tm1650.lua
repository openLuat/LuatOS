...
@author  江访
@usage
本文件为 TM1650 驱动芯片的 LuatOS 扩展库，核心功能为：
1、初始化 TM1650，配置 CLK/DIO 两线通信引脚
2、4 位 8 段数码管显示（支持共阴/共阳切换、7段/8段模式切换）
3、28 个按键扫描检测（轮询和回调两种方式）
4、8 级亮度调节

本文件的对外接口有 9 个：
1、exs_tm1650.setup(config)：初始化
2、exs_tm1650.set_brightness(level)：设置亮度
3、exs_tm1650.set_display(str, offset)：数码管显示字符串
4、exs_tm1650.set_mode(mode)：设置 7段/8段模式
5、exs_tm1650.clear()：清空显示
6、exs_tm1650.get_key()：轮询读取按键
7、exs_tm1650.set_key_callback(cbfunc)：设置按键回调
8、exs_tm1650.version()：获取版本号

-- 版本更新说明
-- 版本号：202607131200
-- 1、更新时间：2026-07-13 12:00
-- 2、更新内容
  - 首次发布，实现 TM1650 基础驱动功能
  - 支持数码管显示（8 段 ×4 位）
  - 支持按键扫描（轮询 + 回调两种方式）
  - 支持 8 级亮度调节
  - 支持共阴/共阳数码管切换
...
-- 生成显示控制字节
-- TM1650 位定义：X BR2 BR1 BR0 S X X D
--   BR[2:0] = 亮度编码（bits[6:4]）
--   S = 段模式（bit[3]，1=7段，0=8段）
--   D = 显示开/关（bit[0]，1=开，0=关）
local function make_display_ctrl(bright, segment_7seg, display_on)
    ...
    return (br_code << 4) | (s_bit << 3) | d_bit
end

-- 更新显示控制（刷新到芯片）
local function update_display_control()
    local ctrl = make_display_ctrl(g_current_bright, g_segment_7seg, g_display_on)
...
local function key_byte_to_code(key_byte)
    ...
    if key_code < 1 or key_code > MAX_KEYS then
        return nil
    end
...
--[[
初始化 TM1650 驱动芯片，配置通信引脚和初始参数

@api exs_tm1650.setup(config)

@table config
初始化配置表，包含以下键：

clk
CLK 时钟引脚 GPIO 编号，接 TM1650 第 6 脚（CLK）；
数据类型：number
是否必选：必选

dio
DIO 数据引脚 GPIO 编号，接 TM1650 第 5 脚（DIO）；
注意：DIO 为开漏模式，需外接 4.7kΩ~10kΩ 上拉电阻到 VCC；
数据类型：number
是否必选：必选

kp
KP 中断引脚 GPIO 编号（可选），接 TM1650 第 7 脚（KP/SEG8/DP）；
**注意：该引脚与 SEG8/DP（小数点段）硬件复用。配置 kp 后必须使用 7 段模式（set_mode(1)），否则 DP 引脚被用作中断输出，小数点无法显示。**
...
function exs_tm1650.setup(config)
    -- 参数检查
    if type(config) ~= "table" then
        log.error("exs_tm1650.setup 参数错误：config 应为 table 类型")
        return false
    end
    if not config.clk or not config.dio then
        log.error("exs_tm1650.setup 参数错误：clk、dio 均为必填")
        return false
    end
    ...
    log.info("exs_tm1650", string.format("初始化完成, clk=%d dio=%d 亮度=%d",
...
@api exs_tm1650.set_brightness(level)
...
@usage
exs_tm1650.set_brightness(7)  -- 最亮
exs_tm1650.set_brightness(0)  -- 最暗
]]
function exs_tm1650.set_brightness(level)
...
@usage
exs_tm1650.set_mode(0)  -- 8 段模式
exs_tm1650.set_mode(1)  -- 7 段模式
]]
function exs_tm1650.set_mode(mode)
...
@api exs_tm1650.set_display(str, offset)
...
exs_tm1650.set_display("25°C")
]]
function exs_tm1650.set_display(str, offset)
    if type(str) ~= "string" or #str == 0 then
        log.error("exs_tm1650.set_display 参数错误：str 应为非空字符串")
        return
    end
    offset = offset or 1
...
TM1650 内置 7×4 按键扫描矩阵，最多支持 28 个独立的按键。

按键扫描由芯片自动完成，结果存储在内部寄存器，
主控通过两线总线读取即可。

@api exs_tm1650.get_key()
...
if key then
    log.info("exs_tm1650", "检测到按键:", key)
end
]]
function exs_tm1650.get_key()
    local key_byte = read_key_byte()
    local code = key_byte_to_code(key_byte)
    g_last_key_byte = key_byte
    return code
...
设备进入低功耗休眠前，必须先调用 exs_tm1650.set_key_callback(nil)
停止内部轮询定时器，否则定时器会阻止系统进入低功耗模式。
唤醒后重新注册回调即可。

如果 setup() 时配置了 kp 引脚，将使用 KP 中断替代轮询，降低 CPU 占用。
⚠ 重要：使用 KP 中断时，芯片第 7 脚（KP/SEG8/DP）被用作中断输出，
此时必须调用 set_mode(1) 切换到 7 段模式（DP 段不可用），否则小数点无法显示。
如需显示小数点（8 段模式），请勿配置 kp 引脚，改用轮询方式读取按键。

@api exs_tm1650.set_key_callback(cbfunc)

@param function cbfunc
按键事件回调函数，接收一个参数 key_code（number 类型 1~28）。
...
local function on_key(key_code)
    log.info("exs_tm1650", "按键:", key_code)
end
...
function exs_tm1650.set_key_callback(cbfunc)
...
@api exs_tm1650.version()

@return string
版本号字符串，格式为 "yyyymmddhhmm"

@usage
local ver = exs_tm1650.version()
