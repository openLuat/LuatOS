...
-- -- UDP性能测试
-- function udp_test.test_udp_performance()
--     init()
...
--         local libnet = require "libnet"
--         local packet_size = 512
--         local packet_count = 5
--         local test_data = string.rep("X", packet_size)
        
--         -- 预热
--         safe_tx(TASK_NAME, 2000, socket_client, "WARMUP")
--         sys.wait(200)
        
--         local start_time = get_current_time_ms()
--         for i = 1, packet_count do
--             local tx_result = safe_tx(TASK_NAME, 2000, socket_client, test_data)
--             assert(tx_result == true, string.format(
--                 "适配器[%s] 性能测试发送失败: 第%d包", name or "默认", i))
--         end
--         local end_time = get_current_time_ms()
--         local send_time = end_time - start_time
--         if send_time < 0 then
--             send_time = -send_time
--         end
--         if send_time == 0 then
...
--         local total_bytes = packet_size * packet_count
--         local send_rate = (total_bytes * 1000) / send_time
        
--         log.info("udp_test", string.format(
--             "适配器[%s] 发送性能: %d 字节/%dms = %.2f KB/s", 
--             name or "默认", total_bytes, send_time, send_rate))
        
--         assert(send_rate > 0, string.format(
--             "适配器[%s] UDP性能测试失败: 发送速率为0", name or "默认"))
        
--         log.info("udp_test", string.format("✓ 适配器[%s] UDP性能测试通过", name))
--         ensure_udp_closed(socket_client)
--         sys.wait(300)
--     end
-- end
