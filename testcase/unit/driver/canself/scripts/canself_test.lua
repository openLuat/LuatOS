...
local function can_tx_test()
    ...
    local tx_result = can.tx(can_id, tx_id, id_type, false, true, tx_buf)
    -- 使用assert验证发送结果
    assert(tx_result == 0,
        string.format("CAN发送消息第%d次测试失败: 预期 0, 实际 %s", current_test_count, tx_result))
...
function canself_test.test_all_baudrates()
    ...
    for idx, baud in ipairs(baud_rates) do
        ...
        if success then
            ...
            fail_count = fail_count + 1
            table.insert(fail_details, string.format("%s-标准帧: %s", baud.name, err))
